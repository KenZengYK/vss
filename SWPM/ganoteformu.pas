...
begin
  ...
  memo2.lines.clear;
  if dbmemo1.text>'' then sm1:=dbmemo1.text else sm1:='';
  if pos('~',sm1)>0 then begin
    memo2.lines.add(trim(copy(sm1,1,pos('~',sm1)-1)));
    memo2.lines.add(trim(copy(sm1,pos('~',sm1)+1,600)));
  end else memo2.lines.add(trim(sm1));
end;
