...
begin
  ...
  str1:='update tbl_cap_wf_notepad set ';
  if not query1.fieldbyname('remarks').isnull then
  str1:=str1+'remarks='''+query1.fieldbyname('remarks').value+''' '
  else str1:=str1+'remarks='''' ';
  str1:=str1+'where tplant='''+query1.fieldbyname('tplant').value+''' and dt='''+formatdatetime('yyyy-MM-dd',query1.fieldbyname('dt').value)+''' and ws='''+query1.fieldbyname('ws').value+'''';
