...
procedure Tfrmwmcss.ComboBox1Change(Sender: TObject);
begin
  if combobox1.text>'' then begin
    ...
    with query2 do begin
      close;
      params.clear;
      commandtext:='select distinct tshop from tblline where tshop>'''' and tplant='''+combobox1.text+'''';
      open;
      first;
...
procedure Tfrmwmcss.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
  frmwmcss:=nil;
end;

procedure Tfrmwmcss.Query1NewRecord(DataSet: TDataSet);
var
  seq:integer;
  tplant:string;
begin
  ...
  query1.fieldbyname('msta').value:='2';
  query1.fieldbyname('mstt').value:=combobox2.text;
  query1.fieldbyname('seq').value:=seq;
end;

procedure Tfrmwmcss.Query1AfterPost(DataSet: TDataSet);
begin
...
procedure Tfrmwmcss.BitBtn1Click(Sender: TObject);
begin
...
procedure Tfrmwmcss.BitBtn2Click(Sender: TObject);
begin
  if query1.Active then begin
...
procedure Tfrmwmcss.BitBtn3Click(Sender: TObject);
begin
  if (query1.state=dsinsert) or (query1.state=dsedit) then query1.post;
end;

procedure Tfrmwmcss.ComboBox2Change(Sender: TObject);
begin
  if combobox2.text>'' then begin
    with query1 do begin
      close;
      params.clear;
      commandtext:='select * from tbl_mcpool where mstt='''+combobox2.text+''' and msta=''2''';
      open;
    end;
  end;
end;

procedure Tfrmwmcss.FormShow(Sender: TObject);
begin
  combobox2.items.clear;
  with query2 do begin
    close;
