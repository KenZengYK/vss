...
uses
  ...
  Dialogs, DB, DBClient, GridsEh, DBGridEh, ExtCtrls, DBCtrls, StdCtrls,
  Buttons, ppPrnabl, ppClass, ppCtrls, ppDB, ppBands, ppCache, ppProd,
  ppReport, ppComm, ppRelatv, ppDBPipe, ppVar, ppViewr, ppParameter;

type
  Tfrmswfmc = class(TForm)
    Panel1: TPanel;
    DBGridEh1: TDBGridEh;
    Panel2: TPanel;
...
procedure Tfrmswfmc.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
  frmswfmc:=nil;
end;

procedure Tfrmswfmc.BitBtn1Click(Sender: TObject);
begin
  dbgrideh1.SetFocus;
...
procedure Tfrmswfmc.BitBtn2Click(Sender: TObject);
begin
  if application.MessageBox('Want to delete this record?','Confirmation',mb_iconquestion+mb_okcancel)=idok then begin
    with query2 do begin
      ...
      commandtext:='delete from tblswf_log2 where seq=:x1';
...
procedure Tfrmswfmc.BitBtn3Click(Sender: TObject);
begin
  if (query1.state=dsedit) or (query1.state=dsinsert) then query1.post;
end;

procedure Tfrmswfmc.BitBtn4Click(Sender: TObject);
begin
  bitbtn3click(self);
...
procedure Tfrmswfmc.Query1NewRecord(DataSet: TDataSet);
var
  seq:integer;
begin
  with query2 do begin
    close;
    params.clear;
    commandtext:='select max(seq) as q1 from tblswf_log2';
    open;
...
procedure Tfrmswfmc.Query1AfterPost(DataSet: TDataSet);
begin
  if query1.ApplyUpdates(-1)>0 then begin
    with query2 do begin
      ...
      params.clear;
      params.createparam(ftinteger,'x1',ptinput);
      commandtext:='select seq from tblswf_log2 where seq=:x1';
      params[0].asinteger:=query1.fieldbyname('seq').value;
      open;
      if not fieldbyname('seq').isnull then begin
        with query3 do begin
          ...
          commandtext:='update tblswf_log2 set opt=:x1,mc=:x6,dt=:x2,sect=:x3 where seq=:x5';
          if not query1.fieldbyname('opt').isnull then
          params[0].asstring:=query1.fieldbyname('opt').value
          else params[0].asstring:='';
          if not query1.fieldbyname('mc').isnull then
          params[1].asstring:=query1.fieldbyname('mc').value
        ...
        with query3 do begin
          ...
          commandtext:='insert into tblswf_log2(opt,dt,sect,mc,seq,swfid,swfna) values(:x1,:x2,:x3,:x4,:x5,:x6,:x7)';
          if not query1.fieldbyname('opt').isnull then
          params[0].asstring:=query1.fieldbyname('opt').value
          else params[0].asstring:='';
          if not query1.fieldbyname('dt').isnull then
          ...
          if not query1.fieldbyname('mc').isnull then
          params[3].asstring:=query1.fieldbyname('mc').Value
...
procedure Tfrmswfmc.ppReport1PreviewFormCreate(Sender: TObject);
begin
