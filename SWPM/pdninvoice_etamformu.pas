...
uses
  ...
  ppParameter, frxExportPDF, frxClass, frxExportXLS, frxDBSet, RM_Common,
  RM_Class, RM_Dataset, RM_e_Xls;

type
  Tfrmpdninvoice_etam = class(TForm)
...
var
  frmpdninvoice_etam: Tfrmpdninvoice_etam;
...
procedure Tfrmpdninvoice_etam.ppReport1PreviewFormCreate(Sender: TObject);
begin
  ppReport1.PreviewForm.WindowState:=wsMaximized;
  TppViewer(ppReport1.PreviewForm.Viewer).ZoomPercentage := 100;
end;

procedure Tfrmpdninvoice_etam.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
  frmpdninvoice_etam:=nil;
end;

procedure Tfrmpdninvoice_etam.ppDetailBand1BeforePrint(Sender: TObject);
var
  job1,jno1:string;
begin
  job1:=''; jno1:='';
...
procedure Tfrmpdninvoice_etam.ppSummaryBand1BeforePrint(Sender: TObject);
var
  s:string;
  s1,s2,box:integer;
  amt:double;
begin
  ...
  with query4 do begin
    close;
    params.clear;
    commandtext:='select rmks from tbl_inv_cust2';
    open;
    ppmemo1.lines.add(fieldbyname('rmks').value);
  end;
end;
