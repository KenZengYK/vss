...
begin
  ...
  with qry2 do begin
    close;
    sql.text:='update phdb..tbl_cp_mailhead set ug=case when act_external=1 then ''Yes'' else ''No'' end where act_cmp=0';
    execsql;
  end;
  with qry1 do begin
    close;
...
begin
  ...
  with qry2 do begin
    close;
    sql.Text:='select a.act_item,b.desc_en,isnull(b.desc_cn,'''') as desc_c from phdb..tbl_cp_mailhead a,phdb..tbl_cp_mailreason b where a.act_item=b.cde and a.seq='+qry1.FieldByName('seq').asstring;
    open;
    first;
    while not eof do begin
      if category002.caption>'' then
        category002.Caption:=category002.Caption+'   '+fieldbyname('desc_en').value+'  '+fieldbyname('desc_c').Value
      else category002.Caption:='  '+fieldbyname('desc_en').value+'  '+fieldbyname('desc_c').Value;
      next;
    end;
  end;
end;
