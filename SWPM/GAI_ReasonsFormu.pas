...
begin
  ...
  if label1.caption='850GAI' then begin
    ...
    str1:=str1+'where pline='''+frmGAI_Actionlog.lblline.caption+''' ';
    str1:=str1+'and seq='+frmGAI_Actionlog.label10.caption+' ';
    str1:=str1+'and dt1='''+formatdatetime('yyyy-MM-dd',frmGAI_Actionlog.dateedit1.date)+'''';
...
procedure TfrmGAI_Reasons.FormShow(Sender: TObject);
var
  dt1:tdatetime;
begin
  if Label1.Caption='850GAI' then Begin
    Panel2.Visible:=True;
    if not frmGAI_Actionlog.dbgrideh1.fields[0].isnull then dt1:=frmGAI_Actionlog.dbgrideh1.fields[0].value
    else dt1:=frmGAI_Actionlog.dateedit1.date;
