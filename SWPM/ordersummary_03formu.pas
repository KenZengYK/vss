unit ordersummary_03formu;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ADODB, ppCtrls, ppReport, ppSubRpt, ppBands, ppClass, ppMemo,
  ppStrtch, ppRegion, ppVar, ppPrnabl, ppCache, ppProd, ppDB, ppComm,
  ppRelatv, ppDBPipe, DB, DBClient, StdCtrls, ExtCtrls, Buttons,
  ppViewr, ppParameter;

type
  Tfrmordersummary_03 = class(TForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    ComboBox1: TComboBox;
...
var
  frmordersummary_03: Tfrmordersummary_03;
  jcpo:string;
  logseq:integer;
...
uses mainformu, analysisformu, ordersummary1formu, ordersummary2formu,
  ordersummary_0formu, ordersummary_01formu;

{$R *.dfm}

procedure Tfrmordersummary_03.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
  frmordersummary_03:=nil;
end;

procedure Tfrmordersummary_03.BitBtn1Click(Sender: TObject);
var
  tm,tm1:tdatetime;
  s1,s2,s3,s4:string;
  str1:string;
begin
  ...
  tm:=now;
  try
   if combobox2.text='SALL1' then begin
     if frmordersummary_0=nil then frmordersummary_0:=tfrmordersummary_0.Create(nil);
     frmordersummary_0.ComboBox1.text:=combobox1.text;
     frmordersummary_0.ComboBox2.text:=combobox2.text;
     frmordersummary_0.ComboBox3.Text:=combobox3.text;
     frmordersummary_0.rg01.ItemIndex:=rg01.ItemIndex;
     frmordersummary_0.xh1.ItemIndex:=xh1.ItemIndex;
     frmordersummary_0.BitBtn1click(self);
   end else begin
   if xh1.ItemIndex=0 then begin
    if combobox2.text='SARA' then begin
      ...
      with query3 do begin
        ...
        params.createparam(ftstring,'x3',ptinput);
        params.createparam(ftstring,'x4',ptinput);
        commandtext:='execute procedure sp_gen_tmpordsummary_new(:x1,:x2,:x3,:x4)';
        params[0].asdatetime:=tm;
        params[1].asstring:=combobox2.text;
        params[2].asstring:=combobox1.text;
        params[3].asstring:=combobox3.text;
    ...
    with query3 do begin
      close;
      params.clear;
      commandtext:='select min(season) as s4 from tbl_tmp_ordsummary_01 where jcpo=''PJ''';
      open;
    ...
    if (((combobox1.Text<s4) and (combobox2.text='SALL')) or (combobox2.text<>'SALL')) then begin
    ...
    with query2 do begin
      ...
      params.clear;
      params.createparam(ftdatetime,'x1',ptinput);
      commandtext:='select distinct tm,xh,jhrs2 as jhrs1,fc,dcdate,shpm,count(distinct cstyle) as s1,sum(scqty) as s2 from tbl_tmp_ordsummary where tm=:x1 group by tm,xh,jhrs2,fc,dcdate,shpm order by tm,xh';
      params[0].asdatetime:=tm;
      open;
    end;
    str1:='';
    ...
    if combobox2.text='SALL' then begin
     ...
     if combobox2.text='SARA' then begin
      ...
      dx002.Caption:='Del ';
      dx003.Caption:='date ';
      ppGroupFooterBand3.Visible:=false;//true;
     end;
    end;
    with query3 do begin
      close;
    ...
    if frmordersummary1=nil then frmordersummary1:=tfrmordersummary1.create(nil);
    with frmordersummary1.query1 do begin
      close;
      params.clear;
      params.createparam(ftdatetime,'x1',ptinput);
    ...
    with frmordersummary1.query2 do begin
      close;
      params.clear;
      params.createparam(ftdatetime,'x1',ptinput);
      commandtext:='select distinct tm,xh,jhrs2 as jhrs1,fc,dcdate,shpm,count(distinct cstyle) as s1,sum(scqty) as s2 from tbl_tmp_ordsummary where tm=:x1 group by tm,xh,jhrs2,fc,dcdate,shpm order by tm,xh';
    ...
    if not frmordersummary1.Query1.fieldbyname('tm').isnull then begin
      if combobox2.text<>'SARA' then begin
        frmordersummary1.title001.Caption:=str1+' Order Summary - '+combobox2.text;
        frmordersummary1.dx001.Caption:='DC ';
        frmordersummary1.dx002.Caption:='date ';
        frmordersummary1.dx003.Caption:=' ';
      end else begin
        frmordersummary1.title001.Caption:=str1+' Order Summary - HBI';
        frmordersummary1.dx001.Caption:='SO ';
        frmordersummary1.dx002.Caption:='Del ';
        frmordersummary1.dx003.Caption:='date ';
      end;
      frmordersummary1.subtitle001.Caption:='(ORDER OUT)';
      if combobox2.text='SARA' then begin
        frmordersummary1.rx001.Caption:='Brand Name';
        frmordersummary1.rx002.Caption:='brand';
      end else begin
        frmordersummary1.rx001.Caption:='Range(Collection)';
        frmordersummary1.rx002.Caption:='range';
      end;
      frmordersummary1.ppReport1.Print;
    end;
   end;
   end;
  finally
...
procedure Tfrmordersummary_03.ppDetailBand1BeforePrint(Sender: TObject);
var
  s1,s2:string;
begin
  s1:='';
...
procedure Tfrmordersummary_03.ppSummaryBand2BeforePrint(Sender: TObject);
var
  s1:string;
begin
  s1:='';
...
procedure Tfrmordersummary_03.ppReport1PreviewFormCreate(Sender: TObject);
begin
  ppReport1.PreviewForm.WindowState:=wsMaximized;
  TppViewer(ppReport1.PreviewForm.Viewer).ZoomPercentage:=100;
end;

procedure Tfrmordersummary_03.ppSummaryBand1BeforePrint(Sender: TObject);
begin
  with query3 do begin
    close;
    params.clear;
  ...
  with query3 do begin
    ...
    params.clear;
    params.createparam(ftdatetime,'x1',ptinput);
    commandtext:='select count(distinct cstyle) as s1,sum(scqty) as s2,sum(tprc) as s3 from tbl_tmp_ordsummary where tm=:x1';
    params[0].asdatetime:=query1.fieldbyname('tm').value;
    open;
...
procedure Tfrmordersummary_03.ppDetailBand2BeforePrint(Sender: TObject);
begin
  with query3 do begin
    close;
    params.clear;
...
procedure Tfrmordersummary_03.ppGroupFooterBand1BeforePrint(Sender: TObject);
var
  i1,i2,i3:integer;
begin
  with query3 do begin
...
procedure Tfrmordersummary_03.ppGroupFooterBand3BeforePrint(Sender: TObject);
var
  i1:integer;
begin
...
procedure Tfrmordersummary_03.ppGroupHeaderBand2BeforePrint(
  Sender: TObject);
begin
  with query3 do begin
    close;
