...
procedure Tfrmdmcss.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
  frmdmcss:=nil;
end;

procedure Tfrmdmcss.BitBtn1Click(Sender: TObject);
begin
...
procedure Tfrmdmcss.BitBtn2Click(Sender: TObject);
begin
  if query1.Active then begin
...
procedure Tfrmdmcss.BitBtn3Click(Sender: TObject);
begin
  if (query1.state=dsinsert) or (query1.state=dsedit) then query1.post;
end;

procedure Tfrmdmcss.Query1NewRecord(DataSet: TDataSet);
var
  seq:integer;
begin
  with query2 do begin
  ...
  query1.fieldbyname('msta').value:='4';
  query1.fieldbyname('mstt').value:='Dead';
  query1.fieldbyname('seq').value:=seq;
end;

procedure Tfrmdmcss.Query1AfterPost(DataSet: TDataSet);
begin
...
procedure Tfrmdmcss.ComboBox1Change(Sender: TObject);
begin
  if combobox1.text>'' then begin
    with query1 do begin
      close;
      params.clear;
      commandtext:='select * from tbl_mcpool where msta=''4'' and tplant='''+combobox1.text+'''';
      open;
    end;
  end;
end;
