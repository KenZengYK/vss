...
uses
  ...
  Dialogs, StdCtrls, Buttons, GridsEh, DBGridEh, DB, ADODB, DBClient, DBGridEHImpExp;
...
procedure Tfrmrwosampleqty.BitBtn1Click(Sender: TObject);
begin
  try
  ...
  if key=#13 then begin
    ...
    with adoquery1 do begin
      ...
      sql.text:='select distinct projectno,workorderno,customerstyleno,colorcode,rwo,rtrim(custsize)+rtrim(custcup) as sizecup,sampleqty '
               +'from [ph.rwo1]..view_woc_rwo where 0=0';// where workorderno=:x1 and colorcode=:x2 and rwo=:x3 and sampleqty>0';
      if edit1.text>'' then sql.text:=sql.text+' and projectno='''+edit1.text+'''';
      if edit2.text>'' then sql.text:=sql.text+' and workorderno='''+edit2.text+'''';
      if edit3.text>'' then sql.text:=sql.text+' and colorcode='''+edit3.text+'''';
      open;
      first;
