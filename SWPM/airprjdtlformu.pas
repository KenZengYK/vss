...
begin
  //DataSetAfterPost(DataSet,0,1,2,3);//,4,5);
      str1:='update tbl_pdn_airdtl set ';
      if not query1.fieldbyname('ocharge').isnull then
      str1:=str1+'ocharge='+query1.fieldbyname('ocharge').asstring+','
      else str1:=str1+'ocharge=0,';
      if not query1.fieldbyname('bcharge').isnull then
      str1:=str1+'bcharge='+query1.fieldbyname('bcharge').asstring+','
      else str1:=str1+'bcharge=0,';
      if not query1.fieldbyname('rcharge').isnull then
      str1:=str1+'rcharge='+query1.fieldbyname('rcharge').asstring+','
      else str1:=str1+'rcharge=0,';
      if not query1.fieldbyname('vwght1').isnull then
      str1:=str1+'vwght1='+query1.fieldbyname('vwght1').asstring+','
      else str1:=str1+'vwght1=0,';
      if not query1.fieldbyname('vndr').isnull then
      str1:=str1+'vndr='''+query1.fieldbyname('vndr').value+''' '
      else str1:=str1+'vndr='''' ';
      str1:=str1+'where seq='+query1.fieldbyname('seq').asstring+' ';
      str1:=str1+'and seq1='+query1.fieldbyname('seq1').asstring+' ';
      str1:=str1+'and cstyle='''+query1.fieldbyname('cstyle').value+''' ';
