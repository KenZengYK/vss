...
procedure TfrmEWs_eff.BitBtn4Click(Sender: TObject);
begin
  frmEWs_Eff.Close;
end;
...
procedure TfrmEWs_eff.Query1AfterPost(DataSet: TDataSet);
var
  str1:string;
begin
  with query2 do begin
    ...
    if not fieldbyname('seq').isnull then begin
      ...
      str1:=str1+'fty='''+query1.fieldbyname('fty').value+''',';
      str1:=str1+'eff='+query1.fieldbyname('eff').asstring+',';
      if query1.fieldbyname('sdt').isnull then str1:=str1+'sdt=null,'
      else str1:=str1+'sdt='''+formatdatetime('yyyy-MM-dd',query1.fieldbyname('sdt').value)+''',';
      if query1.fieldbyname('edt').isnull then str1:=str1+'edt=null '
      else str1:=str1+'edt='''+formatdatetime('yyyy-MM-dd',query1.fieldbyname('edt').value)+''' ';
