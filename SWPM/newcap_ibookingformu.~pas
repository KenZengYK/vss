...
type
  Tfrmnewcap_ibooking = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Label4: TLabel;
...
var
  frmnewcap_ibooking: Tfrmnewcap_ibooking;
...
procedure Tfrmnewcap_ibooking.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
  frmnewcap_ibooking:=nil;
end;

procedure Tfrmnewcap_ibooking.ComboBox1Change(Sender: TObject);
var
  tn1,tn2:TTreeNode;
begin
...
procedure Tfrmnewcap_ibooking.FormShow(Sender: TObject);
begin
  combobox1.OnChange(self);
end;

procedure Tfrmnewcap_ibooking.TreeView1Change(Sender: TObject;
  Node: TTreeNode);
begin
  {
...
procedure Tfrmnewcap_ibooking.show_data;
begin
  if treeview1.Selected<>nil then begin
    if treeview1.Selected.Level=1 then begin
      with query1 do begin
        close;
        params.clear;
        commandtext:='select * from tbl_cap_oa_book2 where tplant='''+combobox1.text+''' and cust='''+treeview1.Selected.Text+''' and yr='+inttostr(spinedit1.Value);
        if xh1.Checked then
      ...
      with query1 do begin
        ...
        commandtext:='select distinct tplant,''ALL'' as cust,yr,wk,sum(bqty) as bqty,0.00 as buprc,sum(kqty) as kqty,0.00 as kuprc,sum(uqty) as uqty,0.00 as uuprc,sum(tqty) as tqty,sum(tprc) as tprc '
                    +'from tbl_cap_oa_book2 where tplant='''+combobox1.text+''' and yr='+inttostr(spinedit1.value);
        if xh1.Checked then
        commandtext:=commandtext+' and wk>='+inttostr(spinedit2.value)+' and wk<='+inttostr(spinedit3.value)
        else if xh2.Checked then
...
procedure Tfrmnewcap_ibooking.Query1AfterOpen(DataSet: TDataSet);
begin
...
procedure Tfrmnewcap_ibooking.Query1QtyChange(Sender: TField);
var
  q1,q2,q3:integer;
  u1,u2,u3:double;
begin
...
procedure Tfrmnewcap_ibooking.SpinEdit1Change(Sender: TObject);
begin
  show_data;
end;

procedure Tfrmnewcap_ibooking.BitBtn1Click(Sender: TObject);
begin
  if query1.state=dsedit then query1.post;
end;

procedure Tfrmnewcap_ibooking.Query1AfterPost(DataSet: TDataSet);
begin
  if query1.ApplyUpdates(-1)>0 then begin
    with query2 do begin
      ...
      commandtext:='update tbl_cap_oa_book2 set bqty=:x1,kqty=:x2,uqty=:x3,buprc=:x4,kuprc=:x5,uuprc=:x6,tqty=:x7,tprc=:x8,mr1=:x9,mr2=:x10 where tplant=:x11 and seq=:x12';
      if not query1.fieldbyname('bqty').isnull then
