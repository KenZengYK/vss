    ...
    with query2 do begin
      ...
      if not fieldbyname('seq').isnull then begin
        ...
        with query3 do begin
          ...
          str1:='update tbl_cap_wf_recruit set ';
          if not query1.fieldbyname('empno').isnull then
          str1:=str1+'empno='''+query1.fieldbyname('empno').value+''','
          else str1:=str1+'empno='''',';
          if not query1.fieldbyname('empname').isnull then
          str1:=str1+'empname='''+query1.fieldbyname('empname').value+''','
          else str1:=str1+'empname='''',';
          str1:=str1+'typ='''+query1.fieldbyname('typ').value+''',';
          if not query1.fieldbyname('effdt').isnull then
          str1:=str1+'effdt='''+formatdatetime('yyyy-MM-dd',query1.fieldbyname('effdt').value)+''' '
          else str1:=str1+'effdt='''+formatdatetime('yyyy-MM-dd',date)+''' ';
        ...
        with query3 do begin
          ...
          params.createparam(ftstring,'x5',ptinput);
          params.createparam(ftinteger,'x6',ptinput);
          commandtext:='insert into tbl_cap_wf_recruit(empno,empname,typ,effdt,tplant,seq) values(:x1,:x2,:x3,:x4,:x5,:x6)';// set empno=:x1,empname=:x2,typ=:x3,effdt=:x4 where tplant=:x5 and seq=:x6';
          if not query1.fieldbyname('empno').isnull then
          params[0].asstring:=query1.fieldbyname('empno').value
          else params[0].asstring:='';
          if not query1.fieldbyname('empname').isnull then
