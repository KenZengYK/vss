...
var
  frmswpm_Mail: Tfrmswpm_Mail;

implementation

uses mainformu, gai_notepad_chnformu;

{$R *.dfm}

function Tfrmswpm_Mail.acc_filename(const filepath: string): string;
var
  filep1:string;
begin
...
procedure Tfrmswpm_Mail.BitBtn1Click(Sender: TObject);
var
  s1,s2:string;
  i:integer;
...
function Tfrmswpm_Mail.CheckTxt(const src: string): string;
var
...
procedure Tfrmswpm_Mail.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
  frmswpm_Mail:=nil;
end;

procedure Tfrmswpm_Mail.FormShow(Sender: TObject);
begin
  edit6.Text:='';
  ...
  with query2 do begin
    close;
    sql.text:='select distinct mail from tbl_carte_user where mail>''''';
    open;
    first;
  ...
  with query2 do begin
    close;
    sql.text:='select smtphost from tbl_carte_mailserver';
    open;
...
procedure Tfrmswpm_Mail.getcclist;
var
  s1,mal:string;
begin
  mal:=edit4.text;
...
procedure Tfrmswpm_Mail.gettolist;
var
  s1,mal:string;
begin
  mal:=edit3.text;
...
procedure Tfrmswpm_Mail.ListBox1Click(Sender: TObject);
begin
...
procedure Tfrmswpm_Mail.SpeedButton1Click(Sender: TObject);
begin
  listbox1.visible:=false;
...
procedure Tfrmswpm_Mail.SpeedButton2Click(Sender: TObject);
begin
  listbox1.visible:=false;
  listbox1.visible:=true;
  label7.caption:='CC';
...
procedure Tfrmswpm_Mail.SpeedButton3Click(Sender: TObject);
begin
  if opendialog1.Execute then
    edit6.Text:=opendialog1.FileName;
end;

procedure Tfrmswpm_Mail.StrtoStrlist(Sign, Str: string;
