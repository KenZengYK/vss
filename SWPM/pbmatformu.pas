...
procedure Tfrmpbmat.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure Tfrmpbmat.Query1AfterPost(DataSet: TDataSet);
begin
  if query1.ApplyUpdates(-1)>0 then begin
    with query2 do begin
      ...
      commandtext:='update tblpbmatmatch set t_date=:date,t_marks=:marks where j_no=:jno and j2_job=:job and acol=:acol and sopno=:sopno and ordline=:ordline and t_seq=:seq';
...
procedure Tfrmpbmat.BitBtn1Click(Sender: TObject);
begin
  if query1.state=dsedit then query1.post;
end;

procedure Tfrmpbmat.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if query1.state=dsedit then query1.post;
  action:=cafree;
  frmpbmat:=nil;
