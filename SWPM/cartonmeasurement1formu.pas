    ...
    with query2 do begin
      ...
      if not fieldbyname('seq0').isnull then begin
        ...
        with query3 do begin
          ...
          str1:='update tbl_pdn_pack_dtl set ';
          if not query1.fieldbyname('cbm').isnull then
          str1:=str1+'cbm='''+query1.fieldbyname('cbm').value+''','
          else str1:=str1+'cbm='''',';
          if not query1.fieldbyname('c1').isnull then
          str1:=str1+'c1='+query1.fieldbyname('c1').asstring+','
          else str1:=str1+'c1=0,';
          if not query1.FieldByName('c2').isnull then
          str1:=str1+'c2='+query1.fieldbyname('c2').asstring+','
          else str1:=str1+'c2=0,';
          if not query1.FieldByName('c3').isnull then
          str1:=str1+'c3='+query1.fieldbyname('c3').asstring+','
          else str1:=str1+'c3=0,';
          if not query1.fieldbyname('tbox').isnull then
          str1:=str1+'tbox='+query1.fieldbyname('tbox').asstring+','
          else str1:=str1+'tbox=0,';
          if not query1.fieldbyname('tcbm1').isnull then
          str1:=str1+'tcbm1='+query1.fieldbyname('tcbm1').asstring+' '
          else str1:=str1+'tcbm1=0 ';
          str1:=str1+'where seq0='+query1.fieldbyname('seq0').asstring;
