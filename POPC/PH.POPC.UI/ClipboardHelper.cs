...
EndSelection:<<<<<<<<4";
        ...
        public static DataObject CreateDataObject(string html, string plainText, string richText)
        {
            ...
            MemoryStream stream2 = new MemoryStream(Encoding.UTF8.GetBytes(htmlFragment));
            dataObject.SetData(DataFormats.Html, stream2);

            //dataObject.SetData(DataFormats.Html, htmlFragment);
            dataObject.SetData(DataFormats.Text, plainText);
            dataObject.SetData(DataFormats.UnicodeText, plainText);
            dataObject.SetData(DataFormats.Rtf, richText);
            return dataObject;
        }
        ...
        /// <param name="html">a  html fragment</param>      
        /// <param  name="plainText">the plain text</param>      
        public static void CopyToClipboard(string html, string plainText, string richText)
        {
            var dataObject = CreateDataObject(html, plainText, richText);
    ...
    public class PHClipboard
    {
        public static void CellsCopyToClipboard(GridView view)
        {
            string htmlText = GetHtmlText(view);
            string plianText = GetPlainText(view);
            string richText = GetRichText(view);
            ClipboardHelper.CopyToClipboard(htmlText, plianText, richText);

        }

        private static string GetPlainText(GridView view)
        {
            ...
            GridColumn[] columns;
            columns = view.GetSelectedCells(selectedRows[0]);


            string htmlTable = "";
            string htmlHeader = "";
            foreach (var gCol in columns)
            {
                htmlHeader += string.Format("{0}{1}", (string.IsNullOrEmpty(htmlHeader)) ? "" : "\t", RemoveReturnString(gCol.Caption));
            }
            htmlTable += htmlHeader;
            ...
            for (int i = 0; i < selectedRows.Length; i++)
            {
                ...
                foreach (var gCol in columns)
                {
                    string text = view.GetRowCellDisplayText(selectedRows[i], gCol);
                    trString += string.Format("{0}{1}", string.IsNullOrEmpty(trString) ? "" : "\t", RemoveReturnString(text));
                }
                rowString += string.Format("{0}{1}", string.IsNullOrEmpty(rowString) ? "" : "\r\n", trString);
            }
        ...
        private static string GetHtmlText(GridView view)
        {
            //GridView view = bandedGridView1;
            ...
            string htmlTable = @"<table style=' border-style:none ;  border-color:Black; border-width:1px; border-collapse :collapse ; font-size:10px; text-align:center; word-wrap: break-word; ' width={0}>
            <tr style='color:blue'>";
            int ttl = 1;
            ...
            foreach (var gCol in columns)
            {
                ttl += gCol.VisibleWidth + 1;
                htmlHeader += string.Format("\r\n<td style=' border:solid  Black 1px;' width={0}>{1}</td>", gCol.VisibleWidth, gCol.Caption);
            }
            htmlHeader += "\r\n</tr>";
            htmlTable = string.Format(htmlTable, ttl) + htmlHeader;

            string rowString = "";
            for (int i = 0; i < selectedRows.Length; i++)
            {
                string trString = @"<tr style='background-color:white';>";
                foreach (var gCol in columns)
                {
                    string text = view.GetRowCellDisplayText(selectedRows[i], gCol);
                    trString += string.Format("\r\n<td style=' border:solid  Black 1px;'>{0}</td>", text);
                }
                trString += "\r\n</tr>";
                rowString += trString;
            }
        ...
        private static string GetRichText(GridView view)
        {
            ...
            AddRTFRow(sb, view, columns, -1, Color.Blue, true);

            for (int i = 0; i < selectedRows.Length; i++)
            {
                AddRTFRow(sb, view, columns, selectedRows[i], Color.Black, false);
            }
        ...
        private static string RemoveReturnString(string s)
        {
            return s = s.Replace("\r\n", "");
        }

        private static void AddRTFRow(RTFBuilderbase sb, GridView view, GridColumn[] columns, int rowHandle, Color textColor, bool isHeader)
        {
            int rowWidth = 0;
            foreach (var gCol in columns)
            {
                rowWidth += gCol.VisibleWidth;
            }
            rowWidth = rowWidth / 5;

            Padding p = new Padding { All = 50 };
            RTFRowDefinition rd = new RTFRowDefinition(rowWidth, RTFAlignment.MiddleCenter, RTFBorderSide.Default, 15, SystemColors.WindowText, Padding.Empty);
            RTFCellDefinition[] cds = new RTFCellDefinition[columns.Length];
            for (int i = 0; i < columns.Length; i++)
            {
                cds[i] = new RTFCellDefinition(columns[i].VisibleWidth / 5, RTFAlignment.MiddleCenter, RTFBorderSide.Default, 15, SystemColors.WindowText, Padding.Empty);
                //float w = columns[i].VisibleWidth * 3 / 20;
            }
            int pos = 0;
            foreach (RTFBuilderbase item in sb.EnumerateCells(rd, cds))
            {
                ...
                sb.FontSize(18).Font(RTFFont.Arial);

                GridColumn gCol = columns[pos++];
                string text = isHeader ? gCol.Caption : view.GetRowCellDisplayText(rowHandle, gCol);
                item.Append(text);
            }
        }
    }
