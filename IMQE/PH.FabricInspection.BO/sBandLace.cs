...
{
    public partial class sBandLaceHeader : BaseEntity
    {
        ...
        public override void Save(System.Data.Linq.DataContext ADataContext, System.Windows.Forms.BindingSource ABindingSource)
        {
            ...
            IEnumerable<sBandLaceHeader> lists = ABindingSource.List as IEnumerable<sBandLaceHeader>;
            ...
            //计算Defect扣分
            SpotDefectPointQty = sBandLaceSpotDefects.Select(p => (p.DefectPointQty ?? 0)).Sum();
            ChainDefectPointQty = Convert.ToInt32(Math.Ceiling(sBandLaceChainDefects.Select(p => (p.Length ?? 0)).Sum())) * 4;
            ...
            ChainDefectLength = this.sBandLaceChainDefects.Where(dr => !(dr.IsCutOut ?? false)).Select(p => p.Length).Sum();
            ...
            CutoutDefectDistance = this.sBandLaceChainDefects.Where(dr => (dr.IsCutOut ?? false)).Select(dr => dr.Length).Sum();
            /*---------------------------------------*/
            ...
            this.NeedEvadeMarker = this.sBandLaceChainDefects.Where(dr => !(dr.IsCutOut ?? false)).Count() > 0 ? "Y" : "N";
    ...
    public partial class sBandLaceSpotDefect : BaseEntity
    {
    ...
    public partial class sBandLaceChainDefect : BaseEntity
    {
        private PHQCDefect QCDefect
        {
            get
