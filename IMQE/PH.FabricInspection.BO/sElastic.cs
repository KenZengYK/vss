...
namespace PH.FabricInspection.BO
{
    ...
    public partial class sElasticHeader : BaseEntity
    {
        ...
        public override void Save(System.Data.Linq.DataContext ADataContext, System.Windows.Forms.BindingSource ABindingSource)
        {
            ...
            SpotDefectPointQty = this.sElasticSpotDefects.Select(p => p.DefectPointQty).Sum();

            //计算Chain Defect Ttl Pts*
            ChainDefectPointQty = Convert.ToInt32(Math.Ceiling(this.sElasticChainDefects.Select(p => (p.Length ?? 0)).Sum()));
            ...
            //计算 C.Defect (Y/N）*
            this.HasChainDefect = this.sElasticChainDefects.Count() > 0 ? "Y" : "N";
            ...
            //计算 C.Defect(m) *
            ChainDefectLength = this.sElasticChainDefects.Select(p => p.Length).Sum();
    ...
    public partial class sElasticSpotDefect : BaseEntity
    {
    ...
    public partial class sElasticChainDefect : BaseEntity
    {
        private PHQCDefect QCDefect
        {
            get
