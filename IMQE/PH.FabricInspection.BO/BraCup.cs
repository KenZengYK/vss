...
{
    public partial class BraCupHeader : BaseEntity
    {
        ...
        public int AuditFullSizeDiffByLotShow
        {
            get
            {
                int HowMany = (UnderPackingListSizeCupQtyByLot ?? 0) - (UnderPullSizeCupQtyByLot ?? 0);
                return HowMany < 0 ? 0 : HowMany;
        ...
        public override void Save(System.Data.Linq.DataContext ADataContext, System.Windows.Forms.BindingSource ABindingSource)
        {
            ...
            IEnumerable<BraCupHeader> lists = ABindingSource.List as IEnumerable<BraCupHeader>;
            ...
            //计算Defect Qty
            this.DefectQty = this.BraCupDefects.Select(p => p.Qty).Sum();
            ...
            SpotDefectPointQty = BraCupDefects.Select(p => (p.Qty ?? 0)).Sum();
            ...
            int iAdjustQtyByLot = ByLots.Where(p => p.CasingDiffQty < 0).GroupBy(p => new { p.AuditSKUCrossCup, p.PackListQtyByCrossCup })
            ...
            foreach (var o in ByLots)
            {
                ...
                int? iDefectCount = o.BraCupDefects.Where(p => DefectTop4.Contains(p.DefectCode)).Count();
                o.DefectCount = iDefectCount;

                //计算Casing Top 1 ~ Top 4中的疵点个数 Ttl Casing Defect *
                o.CasingTotalDefectQty = o.BraCupCasingDefects.Where(p => CasingDefectTop2.Contains(p.DefectCode)).Count();
            ...
            //因为在2nd或3rd Audit中，会存在一个Audit#中某些Cross Cup是AQL检查，某些Cross Cup是全检，程序需要分开计算Claim的数量 2022-01-20
            var ByCrossCupFullInspectionList = ByLots.Where(p => (p.IsByCrossCupFulInspection ?? false));
            decimal iPart1FinalClaimByLotFullInspection = ByCrossCupFullInspectionList.Select(p => p.SpotDefectPointQty ?? 0).Sum();

            var ByCrossCupAQLList = ByLots.Except(ByCrossCupFullInspectionList);

            decimal iPart1FinalClaimByLotAQL = ByCrossCupAQLList.Where(p => p.Part1Result == "B")
                                                 .GroupBy(p => new { p.AuditSKUCrossCup, p.PackListQtyByCrossCup })
                                                 .Select(g => g.Sum(x => (x.PackListQtyByCrossCup ?? 0)) / g.Count()).Sum() +

                                               ByCrossCupAQLList.Where(p => p.Part1Result == "A").Select(p => p.SpotDefectPointQty ?? 0).Sum(); //如果是A，有Spot Defect的也要Claim.

            decimal iPart1FinalClaimByLot = iPart1FinalClaimByLotFullInspection + iPart1FinalClaimByLotAQL;
            ...
            int iCasingShortLengthClaim = -1 * ByLots.Where(p => (p.CasingDiffQty ?? 0) < 0)
    ...
    public partial class BraCupDefect : BaseEntity
    {
    ...
    public partial class BraCupCasingDefect : BaseEntity
    {
        private PHQCDefect QCDefect
        {
            get
