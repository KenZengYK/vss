...
{
    public class Fabric_Insp_HeaderList : BaseEntityList<Fabric_Insp_Header, FabricInspectionDataContext>
    {
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderList()
        {
            ...
            var dataList = from c in CurrentDataContext.Fabric_Insp_Headers select c;
        ...
        /// <returns></returns>
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilter(int ss, string AStatus)
        {
            //PH.FabricInspection.BO.FabricInspectionDataContext context = PH.Platform.BO.ContextBuilder.CreateContext<FabricInspectionDataContext>();
            //this.CurrentDataContext = context;
            ...
            //var dataList = CurrentDataContext.Fabric_Insp_Headers
            //    .Where(p => Convert.ToInt32(p.Stock_In_Date) >= Convert.ToInt32(ss) && p.Status == AStatus)
            //    .OrderByDescending(p => p.RefNo).ThenBy(p => p.AuditStage).ThenBy(p => p.Supplich_Batch).ThenBy(p => p.Create_Date).ThenBy(p => p.Lot_Id);
            //return dataList;

            var dataList = CurrentDataContext.Fabric_Insp_Headers
                .Where(p => Convert.ToInt32(p.Stock_In_Date) >= ss && p.Status == AStatus)
                .OrderByDescending(p => p.RefNo).ThenBy(p => p.AuditStage).ThenBy(p => p.Supplich_Batch).ThenBy(p => p.Create_Date).ThenBy(p => p.Lot_Id);
            return dataList;
        ...
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilterByDailyDate(string ADailyDate, string AWarehouse)
        {
            var dataList = CurrentDataContext.Fabric_Insp_Headers
        ...
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilterByDateRange(string AFromDate, string AToDate, string AWarehouse)
        {
            var dataList = CurrentDataContext.Fabric_Insp_Headers
        ...
        /// <returns></returns>
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilter(string FromDateStr, string EndDateStr)
        {
            long sFromD = Convert.ToInt32(FromDateStr) * 1000 + 1;
            long eEndID = Convert.ToInt32(EndDateStr) * 1000 + 999;
            var dataList = from c in CurrentDataContext.Fabric_Insp_Headers where c.System_Install_No >= sFromD && c.System_Install_No <= eEndID select c;
            return dataList;

        }

        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilter(string fromSystemNo, string toSystemNo, string fromItemNo, string toItemNo, string fromLotId, string toLotId)
        {
            //PH.FabricInspection.BO.FabricInspectionDataContext context = PH.Platform.BO.ContextBuilder.CreateContext<FabricInspectionDataContext>();
            //this.CurrentDataContext = context;
            long fromSystemNo1 = Convert.ToInt32(fromSystemNo);
            long toSystemNo1 = Convert.ToInt32(toSystemNo);
            var dataList = from c in CurrentDataContext.Fabric_Insp_Headers where (c.System_Install_No >= fromSystemNo1) && (c.System_Install_No <= toSystemNo1) && (c.Lot_Id.CompareTo(fromLotId) >= 0) && (c.Lot_Id.CompareTo(toLotId) <= 0) && (c.Item_No.CompareTo(fromItemNo) >= 0) && (c.Item_No.CompareTo(toItemNo) <= 0) select c;
            //return dataList.ToList<Fabric_Insp_Header>();
            return dataList;
        ...
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListFilter1(string strCompanyNo, string strLotId)
        {
            //PH.FabricInspection.BO.FabricInspectionDataContext context = PH.Platform.BO.ContextBuilder.CreateContext<FabricInspectionDataContext>();
            //this.CurrentDataContext = context;
            var dataList = from c in CurrentDataContext.Fabric_Insp_Headers where c.Company.Equals(strCompanyNo) && (c.Lot_Id == strLotId) select c;
            //return dataList.ToList<Fabric_Insp_Header>();
            return dataList;


        }
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListWith(string strCompanyNo, int intSystemInstallNo)
        {
            //PH.FabricInspection.BO.FabricInspectionDataContext context = PH.Platform.BO.ContextBuilder.CreateContext<FabricInspectionDataContext>();
            //this.CurrentDataContext = context;
            var dataList = from p in CurrentDataContext.Fabric_Insp_Headers
        ...
        public IEnumerable<Fabric_Insp_Header> GetFabric_Insp_HeaderListWith(string strCompanyNo, string strItemNo, string strStockInDate)
        {
            //PH.FabricInspection.BO.FabricInspectionDataContext context = PH.Platform.BO.ContextBuilder.CreateContext<FabricInspectionDataContext>();
            //this.CurrentDataContext = context;
            var dataList = from p in CurrentDataContext.Fabric_Insp_Headers
