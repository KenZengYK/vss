...
{
    public partial class ElasticHeaderListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public ElasticHeaderListForm()
        {
            InitializeComponent();
        }
        ...
        public ElasticHeaderListForm(bool AConditionCall, string AWhse, DateTime AIDCreateDateFrom, DateTime AIDCreateDateTo)
            : this()
        {
        ...
        public ElasticHeaderListForm(bool AOnlyShowIncompleted)
            : this()
        {
        ...
        string _Status;
        public ElasticHeaderListForm(string AStatus)
            : this()
        {
            _Status = AStatus;
        }
        ...
        public override void DataBind()
        {
            ...
            if (_ConditionCall)
            {
                this.BindingSource.DataSource = (this.DataContext as FabricInspectionDataContext).ElasticHeaders
                    .Where(p => (_Whse == "" || p.Whse == _Whse) && p.IDCreateDate >= _IDCreateDateFrom && p.IDCreateDate < _IDCreateDateTo.AddDays(1))
                    .OrderByDescending(p => p.AuditNo).ThenBy(p => p.AuditStage).ThenBy(p => p.LotID);
            }
            else
            {
                IEnumerable<ElasticHeader> lists = (this.DataContext as FabricInspectionDataContext).ElasticHeaders
                    .Where(p => p.IDCreateDate >= DateTime.Now.Date.AddDays(-150) && p.IDCreateDate >= DateTime.Parse("2021-06-01") && p.Status == _Status)
                    .OrderByDescending(p => p.AuditNo).ThenBy(p => p.AuditStage).ThenBy(p => p.LotID);
            ...
            this.EditorTypeName = typeof(ElasticHeaderDetailForm).FullName;
        ...
        protected override void OnClickAddNew()
        {
            ...
            {
                SelectLotIDForm frmElasticSelectAuditID = new SelectLotIDForm(frmInputItemCode.AuditStage, frmInputItemCode.Company,
        ...
        private void repositoryItemButtonEdit_ActionLog_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            CurrentActionLogAuditNo = (this.BindingSource.Current as ElasticHeader).AuditNo;
            IEnumerable<ElasticHeader> MainSheetList = (this.BindingSource.DataSource as IEnumerable<ElasticHeader>).Where(p => p.AuditNo == obj.AuditNo && p.AuditStage == obj.AuditStage);
            AlertLogForm frmAlertLog = new AlertLogForm();
            frmAlertLog.PrepareActionLogData<ElasticHeaderListForm, ElasticHeader>(ActionLogProcessModes.Elastic, db, this, obj.ActionLogID, MainSheetList, Suppliers, Projects, "");
            frmAlertLog.ShowDialog();
        }
        ...
        private void toolStripMenuItemActionLogByLot_Click(object sender, EventArgs e)
        {
            ...
            AlertLogForm frmAlertLog = new AlertLogForm(ActionLogProcessModes.Elastic, db, MissingAudit, obj.ActionLogID, this, Suppliers, Projects, "", obj.AuditStage);
            frmAlertLog.ShowDialog();
        }
        #endregion
        ...
        void EditSmplQuantumByLot(ElasticHeader ACurrent)
        {
            if (ACurrent.AuditStage == "1st") return;

            int EditValue = ACurrent.AuditSampleSizeQtyByLot ?? 0;
            ...
            foreach (var obj in List)
            {
                ...
                if (EditValue >= TotalPackList) //全检
                {
                    ...
                    obj.AuditSampleSizeQty = Convert.ToInt32(obj.PackingListLength);
                }
                else //AQL 检查
                {
                    obj.IsFullInspection = null;

                    if (TotalPackList == 0) //处理除0错误
                    {
        ...
        {
            ElasticHeader obj = this.bandedGridView1.GetRow(e.FocusedRowHandle) as ElasticHeader;
            colAuditSampleSizeQtyByLot.OptionsColumn.AllowEdit = obj != null && obj.AuditStage != "1st";
        }
