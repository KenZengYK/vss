...
{
    public partial class AccessoryHeaderListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public AccessoryHeaderListForm()
        {
            InitializeComponent();
        }
        ...
        public AccessoryHeaderListForm(bool AConditionCall, string AWhse, DateTime AIDCreateDateFrom, DateTime AIDCreateDateTo)
            : this()
        {
        ...
        public AccessoryHeaderListForm(bool AOnlyShowIncompleted)
            : this()
        {
        ...
        string _Status;
        public AccessoryHeaderListForm(string AStatus)
            : this()
        {
            _Status = AStatus;
        }
        ...
        public override void DataBind()
        {
            ...
            if (_ConditionCall)
            {
                this.BindingSource.DataSource = (this.DataContext as FabricInspectionDataContext).AccessoryHeaders
            ...
            {
                IEnumerable<AccessoryHeader> lists = (this.DataContext as FabricInspectionDataContext).AccessoryHeaders
                    .Where(p => p.AuditType == "Accessories" && p.IDCreateDate >= DateTime.Now.AddDays(-150) && p.IDCreateDate >= DateTime.Parse("2021-06-01") && p.Status == _Status)
                    .OrderByDescending(p => p.AuditNo).ThenBy(p => p.AuditStage).ThenBy(p => p.ItemIDNO);
            ...
            this.EditorTypeName = typeof(AccessoryHeaderDetailForm).FullName;
        ...
        protected override void OnClickAddNew()
        {
            ...
            if (frmInputItemCode.ShowDialog() == DialogResult.OK)
            {
                ...
                if (frmSelectLotID.ShowDialog() == DialogResult.OK)
                {
                    ...
                    List<AccessorySelectLotIDResult> AllDataList = frmSelectLotID.GetAllData();
                    List<AccessorySelectLotIDResult> SelectedLists = frmSelectLotID.GetSelectData();
                    ...
                    foreach (AccessorySelectLotIDResult item in SelectedLists)
                    {
        ...
        private void repositoryItemButtonEdit_ActionLog_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            //AlertLogForm frmAlertLog = new AlertLogForm(ActionLogProcessModes.Accessories, db, ActionLogNO, obj.ActionLogOID, this, Suppliers, Projects, Batchs);
            //frmAlertLog.ShowDialog();

            CurrentActionLogAuditNo = (this.BindingSource.Current as AccessoryHeader).AuditNo;
            IEnumerable<AccessoryHeader> MainSheetList = (this.BindingSource.DataSource as IEnumerable<AccessoryHeader>).Where(p => p.AuditNo == obj.AuditNo && p.AuditStage == obj.AuditStage);
            AlertLogForm frmAlertLog = new AlertLogForm();
            frmAlertLog.PrepareActionLogData<AccessoryHeaderListForm, AccessoryHeader>(ActionLogProcessModes.Accessories, db, this, obj.ActionLogID, MainSheetList, Suppliers, Projects, Batchs);
            frmAlertLog.ShowDialog();
        }
        ...
        void EditSmplQuantumByLot(AccessoryHeader ACurrent)
        {
            if (ACurrent.AuditStage == "1st") return;

            int EditValue = ACurrent.AQLAuditQtyByLot ?? 0;
            ...
            var List = (this.BindingSource.DataSource as IEnumerable<AccessoryHeader>).Where(p => p.AuditNo == ACurrent.AuditNo && p.AuditStage == ACurrent.AuditStage);
            int TotalPackList = List.Select(p => p.PackListQty ?? 0).Sum();
            foreach (var obj in List)
            {
                obj.AQLAuditQtyByLot = EditValue; //修改By Lot中的一行 Smpl Quantum **，其他行都变成一样的值

                if (EditValue >= TotalPackList) //全检
                {
                    obj.IsFullInspection = true;
                    obj.AQL25StandardQtyByLot = 0;
                    obj.AQLAuditQty = obj.PackListQty;
                }
                else //AQL 检查
                {
                    obj.IsFullInspection = null;

                    if (TotalPackList == 0)
                    {
                        obj.AQLAuditQty = null;
                    }
                    else
                    {
                        obj.AQLAuditQty = Convert.ToInt32(Math.Ceiling((1.0000M * obj.PackListQty ?? 0) / TotalPackList * EditValue)); //按比例重新计算Assigned Smpl Quantum *
                    }

                    if (obj.AQLAuditQty > BanalceValue)
                    {
                        obj.AQLAuditQty = BanalceValue;
                    }
                    BanalceValue -= (obj.AQLAuditQty ?? 0);
        ...
        {
            AccessoryHeader obj = this.bandedGridView1.GetRow(e.FocusedRowHandle) as AccessoryHeader;
            colAQLAuditQtyByLot.OptionsColumn.AllowEdit = obj != null && obj.AuditStage != "1st";
        }
