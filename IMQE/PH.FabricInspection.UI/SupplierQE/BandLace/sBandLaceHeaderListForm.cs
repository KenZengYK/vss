...
{
    public partial class sBandLaceHeaderListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public sBandLaceHeaderListForm()
        {
            InitializeComponent();
        }
        ...
        public sBandLaceHeaderListForm(bool AConditionCall, string AWhse, DateTime AIDCreateDateFrom, DateTime AIDCreateDateTo)
            : this()
        {
        ...
        public sBandLaceHeaderListForm(bool AOnlyShowIncompleted)
            : this()
        {
        ...
        public override void DataBind()
        {
            ...
            if (_ConditionCall)
            {
                this.BindingSource.DataSource = (this.DataContext as FabricInspectionDataContext).sBandLaceHeaders
            ...
            {
                IEnumerable<sBandLaceHeader> lists = (this.DataContext as FabricInspectionDataContext).sBandLaceHeaders
                    .Where(p => p.IDCreateDate >= DateTime.Now.Date.AddDays(-150))
                    .OrderByDescending(p => p.AuditNo).ThenBy(p => p.BatchNo).ThenBy(p => p.LotID); ;
            ...
            this.EditorTypeName = typeof(sBandLaceHeaderDetailForm).FullName;
        ...
        {
            sInputLotIDForm frmInputLotID = new sInputLotIDForm();
        ...
        void ImportDataFromERP(string ACompany, string ALotID, string AAuditStage)
        {
            ...
            try
            {
                ...
                sBandLaceHeader obj = this.BindingSource.Current as sBandLaceHeader;
        ...
        void ByLotSavePartIFinalClaimIaIb(sBandLaceHeader ACurrent)
        {
            var List = (this.BindingSource.DataSource as IEnumerable<sBandLaceHeader>).Where(p => p.AuditNo == ACurrent.AuditNo && p.AuditStage == ACurrent.AuditStage);
        ...
        private void repositoryItemButtonEdit_ActionLog_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            AlertLogForm frmAlertLog = new AlertLogForm(ActionLogProcessModes.BandLace, db, MissingAudit, obj.ActionLogID, this, Suppliers, Projects, Batchs);
        ...
        void CalOddEvenRowFlag()
        {
            ...
            {
                sBandLaceHeader obj = bandedGridView1.GetRow(i) as sBandLaceHeader;
        ...
        private void btnConfirm_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (frmConfim.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    sBandLaceHeader CurrentObj = this.BindingSource.Current as sBandLaceHeader;
                    if (CurrentObj == null) return;

                    var List = (this.BindingSource.DataSource as IEnumerable<sBandLaceHeader>).Where(p => p.AuditNo == CurrentObj.AuditNo && p.AuditStage == CurrentObj.AuditStage);
                    foreach (var obj in List)
                    {
                ...
                {
                    IEnumerable<sBandLaceHeader> lists = bandedGridView1.FilteredList().Cast<sBandLaceHeader>();
