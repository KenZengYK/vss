...
{
    public partial class sBraCupHeaderListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public sBraCupHeaderListForm()
        {
            InitializeComponent();
        }
        ...
        public sBraCupHeaderListForm(bool AConditionCall, string AWhse, DateTime AIDCreateDateFrom, DateTime AIDCreateDateTo)
            : this()
        {
        ...
        public sBraCupHeaderListForm(bool AOnlyShowIncompleted)
            : this()
        {
        ...
        public override void DataBind()
        {
            ...
            if (_ConditionCall)
            {
                this.BindingSource.DataSource = (this.DataContext as FabricInspectionDataContext).sBraCupHeaders
            ...
            {
                IEnumerable<sBraCupHeader> lists = (this.DataContext as FabricInspectionDataContext).sBraCupHeaders
                    .Where(p => p.IDCreateDate >= DateTime.Now.AddDays(-150))
                    .OrderByDescending(p => p.AuditNo).ThenBy(p => p.ItemIDNO);
            ...
            this.EditorTypeName = typeof(sBraCupHeaderDetailForm).FullName;
        ...
        protected override void OnClickAddNew()
        {
            ...
            if (frmInputItemCode.ShowDialog() == DialogResult.OK)
            {
                ...
                if (frmSelectLotID.ShowDialog() == DialogResult.OK)
                {
                    ...
                    List<sBraCupSelectLotIDResult> AllDataList = frmSelectLotID.GetAllData();
                    List<sBraCupSelectLotIDResult> SelectList = frmSelectLotID.GetSelectData();
                    ...
                    foreach (sBraCupSelectLotIDResult item in SelectList)
                    {
        ...
        void CalOddEvenRowFlag()
        {
            ...
            {
                sBraCupHeader obj = bandedGridView1.GetRow(i) as sBraCupHeader;
        ...
        private void barEditItemDashboard_EditValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    this.BindingSource.DataSource = (SaveAllData as IEnumerable<sBraCupHeader>).Where(p => p.DueDay > 3);
                }
                else if (SelectedItem == "Action log - behind sch.") //1st Stage date 已经超期
                {
                    this.BindingSource.DataSource = (SaveAllData as IEnumerable<sBraCupHeader>).Where(p => p.IsOverDueEventLog);
                }
                else if (SelectedItem == "Action log - o/s (1st)")   //1st Stage 没有Completed
                {
                    this.BindingSource.DataSource = (SaveAllData as IEnumerable<sBraCupHeader>).Where(p => p.IsOutstandingEventLog);
                }
                else if (SelectedItem == "Action Log - o/s (last)")  //Last Stage 没有Complete的
                {
                    this.BindingSource.DataSource = (SaveAllData as IEnumerable<sBraCupHeader>).Where(p => p.IsLastStageOutstandingEventLog);
                }
                else if (SelectedItem == "Part I Grade(B, C)")      //Part I是B或C，并且超过三天没有出Action Log的
                {
                    this.BindingSource.DataSource = (SaveAllData as IEnumerable<sBraCupHeader>).Where(p => p.IsPartIGradeBC);
                }
