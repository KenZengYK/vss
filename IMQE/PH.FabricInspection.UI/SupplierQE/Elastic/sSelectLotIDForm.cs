...
namespace PH.FabricInspection.UI.SupplierQE.Elastic
{
    public partial class sSelectLotIDForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        void PrepareData(string AAuditStage, string ACompany, DateTime AIDCreateDate, string AItemCode, string AColorCode)
        {
            ...
            List<sElasticSelectAduitIDResult> lists = dt.AsEnumerable().Select(dr => new sElasticSelectAduitIDResult
            {
        ...
        public List<sElasticSelectAduitIDResult> GetSelectData()
        {
            List<sElasticSelectAduitIDResult> AllLists = GetAllData();

            List<sElasticSelectAduitIDResult> SelectLists = new List<sElasticSelectAduitIDResult>();
            for (int i = 0; i < this.gridView.RowCount; i++)
            {
                sElasticSelectAduitIDResult obj = this.gridView.GetRow(i) as sElasticSelectAduitIDResult;
        ...
        public List<sElasticSelectAduitIDResult> GetAllData()
        {
            return this.bindingSource.DataSource as List<sElasticSelectAduitIDResult>;
        }

    }

    //用于Elastic从ERP中导入检查ID
    public class sElasticSelectAduitIDResult : BaseEntity
    {
