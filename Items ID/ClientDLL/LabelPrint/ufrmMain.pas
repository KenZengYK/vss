...
uses
  ...
  Menus, cxMemo, cxCalendar, cxDateUtils;
...
type
  TfrmMain = class(TfrmBase)
    ...
    N1: TMenuItem;
    cxMemo1: TcxMemo;
    cxGrid1DBTableView1GrnDate: TcxGridDBColumn;
    Label25: TLabel;
    editGRNDate: TcxDateEdit;
...
begin
  ...
  try
    ...
    begin
      ...
      if (grnDate <> NullDate) then
        FieldByName('PrintDate').AsDateTime := grnDate;
      Post;
    end;
     //Showmessage('OK');
    WriteToSocketStream(iCopies, bCollateCopies, bcPrinter, ClientDataSet_Temp, BarcodePrint);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          GRNDate := FieldByName('GRNDate').AsDateTime;
...
*}
      ...
      sql1 := 'select distinct aa.STOCKROOM,aa.ITEM_CODE,aa.LOT_ID,aa.CUR_BAL,aa.UOM,aa.IsCheck,aa.spc235,aa.Location,aa.pgmn,aa.HDO_NO,'
        + 'aa.PHISM,aa.ColorShade,aa.Project,aa.CH2T84,aa.PO,aa.POLine1,aa.locn80,aa.CH9T84,aa.ch1t84,aa.CONO80,aa.strc80,aa.MARKER_WIDTH,a1.recd59'
        + ' from ( select strc80 as STOCKROOM,pnum80 as ITEM_CODE,locn80 as LOT_ID,sphy80 as CUR_BAL, IUNT35 as UOM,'
        ...
        + ' CH2T84,SubString(CH2T84,1,7) as PO,SubString(CH2T84,8,3) as POLine1,locn80,CH9T84,ch1t84,CONO80,strc80,SPC235 as MARKER_WIDTH '
      ...
      sql1 := sql1 + ' left join ault4f1.pmp59 a1'
        //+ '(select cono59,ordn59,Line59,item59,max(recd59) as recd59 from ault4f1.pmp59'
  //+ ' where cono59= ' + QuotedStr(ACONO)
  //+ ' and trnq59>0'
  //+ ' group by cono59,ordn59,Line59,item59) a1'
      + ' on (a1.cono59=aa.CONO80 and a1.ordn59||substr(''00''||cast(a1.Line59 as varchar(3)),length(''00''||cast(a1.Line59 as varchar(3)))-2,3)=aa.CH2T84'
        + '     and a1.item59=aa.ITEM_CODE and a1.trnq59>0)';
      ...
      sql2 := ' select  a.*,c.ORG_QTY,b.Dept,b.WorkOrderNo,b.GarmentColor,b.CustStyleNo,b.OrderLine,'
        ...
        + '[PH.POPC].dbo.fn_ErpInt2Date(19000000+a.recd59) as GrnDate';
