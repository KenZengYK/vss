...
begin
  ...
  begin
    try
      ...
      btnIDAddA.Visible :=false;// isAdmin;
...
begin
  ...
  begin
    ...
    try
      ...
      CommandText := 'Call ZPHLIB_AUL.sp_IDOSelectMaterial_Test (' + QuotedStr(FCONO) + ',' + QuotedStr(FProjectNo) + ',' + QuotedStr(FWorder) + ')';
...
begin
  ...
  cxButton4.Caption := frmMain.GetLanguagesStr(58, 'Select All', True);
  cxButton5.Caption := frmMain.GetLanguagesStr(59, 'Select None', True);
...
var
  ALotNo, ASopNo, AWorderNo, ASQL, ASQLW, AProject, AItemCode: string;
begin
  screen.cursor := crSQLWait;
  with ClientDataSet_Temp do
  begin
...
procedure TfrmIDO.cxButtonEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key = VK_RETURN) and (Shift = []) then btnOKClick(nil);
end;
