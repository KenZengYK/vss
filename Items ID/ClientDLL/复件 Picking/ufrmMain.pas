...
begin
  ...
  begin
    ...
    if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
      CommandText := 'select * from ido1  order by bh'
    else
...
begin
  ...
  try
    ...
    begin
      ...
      Params.Clear;
      CommandText := 'select a.IDO,b.ErpSuccess as bSelect from '
        + ' (select distinct IDO from PB_GATEPASS) a Left join (select IDO,ErpSuccess from PB_GATEPASS where 1=2) b'
        + ' on (a.IDO=b.IDO) order by a.IDO';
      Open;
    end; // with
    cxGrid3DBTableView1.DataController.DataSource := DataSource2;
...
begin
  try
    ...
    begin
      ...
      SQL.Add('call ZPHLIB_AUL.sp_UpdateIDOs2AS400 ('
        + QuotedStr(ACONO) + ','
        + QuotedStr(AIDO) + ','
        + QuotedStr(ALotID) + ','
        + QuotedStr(AWord) + ','
