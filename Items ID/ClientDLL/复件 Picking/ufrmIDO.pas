...
uses
  ...
  cxCurrencyEdit, cxLookAndFeelPainters, ComCtrls;
...
type
  ...
  private
    ...
    function UnlockID(AIDO, ALOTID, ACONO, AWord, Itemcode: string; bIsWarehouse: boolean): boolean;
  public
    { Public declarations }
...
uses ufrmMain, uTools, ufrmDialog;
...
procedure TfrmIDO.SelectIDOData;
  ...
  begin
    ...
    begin
     ...
     // begin
      ...
      commandtext := 'select Count(*) from ido1 a,ido2 b'
  ...
  begin
    ...
    begin
      ...
      params.createparam(ftString, 'idno', ptinput);
      params.createparam(ftString, 'sopno', ptinput);
      commandtext := 'select a.* from ido1 a,ido2 b where a.bh=b.bh and a.bh<>:bh and b.idno=:idno and a.sopno<>:sopno';
...
begin
  ...
  begin
    ...
    try
      ...
      CommandText := 'Call ZPHLIB_AUL.sp_IDOSelectMaterial (' + QuotedStr(FCONO) + ',' + QuotedStr(FProjectNo) + ',' + QuotedStr(FWorder) + ')';
      {Params.ParamByName('CONO').AsString := FCONO;
      Params.ParamByName('PrjNo').AsString := FProjectNo;
      Params.ParamByName('Worder').AsString := FWorder; }
      Open;
      ...
      {    SELECT DISTINCT B.CUSO40 AS PRJNO,A.W1RF40 AS SOPNO,ODSC41 AS OPERDESC,OPCD41 AS OPCD,
             ...
             CH0T84,CH6T84,CH8V84,CH5T84,CH4T84,CH9T84,SPC235,IUNT35,CH8T84 AS PGMN35,C.TLIN40,VCAT01 AS PHISM
          FROM AULT5F1.MSP42,AULT5F1.MSP41,AULT5F1.MSP40 A,AULT4F1.OEP40 B,
   AULT2F1.INP60
     LEFT JOIN AULT4F1.PMP01 ON (CONO01=CONO60 AND ITEM01=PNUM60 AND VNDR01=SUC160),
   AULT4F1.INP71
            LEFT JOIN  AULT4F1.APP84   ON (CONO84=CONO71 AND LOTN84=SREF71 AND PNUM84=CATN71)
            LEFT JOIN  AULT2F1.INP35   ON (CONO35=CONO71 AND PNUM35=CATN71)
            LEFT JOIN  AULT2F1.INP40CH1 C ON (C.CONO40=CONO71 AND C.TREF40=SUBSTR(CATN71,1,9) AND C.USGC40='1' AND C.TLNO40=2)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            commandtext := 'insert into ido2(bh,oper,idno,zdh,wl,sh,sz,qaresult,cd,ufd,mfd,qty,uom,wz,opdesc,aqty,uwidth,Status,StockRoom,pgmn,PHISM,SuppBatch,FUser,CreateDate) '
              + 'values(:bh,:oper,:idno,:zdh,:wl,:sh,:sz,:qaresult,:cd,:ufd,:mfd,:qty,:uom,:wz,:opdesc,:aqty,:uwidth,:Status,:StockRoom,:pgmn,:PHISM,:SuppBatch,:FUser,:CreateDate)';
          ...
          frmMain.UpdateIDOs2AS400(FCONO, BH, idno, worder, 'A');
...
var
  ALotNo, ASopNo, AWorderNo, ASQL, ASQLW, AProject, AItemCode,AIDO: string;
begin
  screen.cursor := crSQLWait;
  with ClientDataSet_Temp do
  begin
    ...
    AIDO := GetIDOFullString(editIDO.Text);
    ...
    try
      ...
      if FQueryIDO then
      begin
        //cxGrid2DBTableView1DBColumn6.Visible := True;
        ASQL := 'select a.*,b.idno,b.zdh,b.StockRoom,b.wl,b.Qty, b.status as BStatus,c.status as CStatus,c.erpsuccess, b.status as IsCheck'
          + ' from ido1 a inner join ido2 b on (a.bh=b.bh)'
          + ' left join pb_gatepass c on (b.bh=c.ido and b.idno=c.lot_id and b.zdh=c.work_order and b.wl=c.item_code and a.cono=c.company and b.stockroom=c.stockroom)'
          + ' where ';
      ...
      begin
        //cxGrid2DBTableView1DBColumn6.Visible := False;
        ASQL := 'select Lot_ID as Idno,Project_NO as Prjno,SOP as sopno,Work_Order as zdh,IDO as bh,'
          + 'Issued_Date as rq,Company as cono,StockRoom,UID as Jlr,ISSUE_QTY as QTY,ITEM_CODE as wl'
          + ' from PB_GatePass';
        ASQLW := '';
        ...
        if AIDO <> '' then ASQL := ASQL + ' and Upper(IDO) like ' + QuotedStr(AIDO + '%');
...
begin
  ...
  begin
    case AType of //
      1: result := 'select distinct b.idno from ido1 a,ido2 b where a.bh=b.bh order by b.idno';
      2: result := 'select distinct sopno,prjno from ido1   order by sopno';
      3: result := 'select distinct Zdh as worder from ido2   order by worder';
    end; // case
  end
  else
  begin
    case AType of //
      1: result := 'select distinct Lot_ID as idno from PB_GatePass order by idno';
      2: result := 'select distinct SOP as sopno,Project_NO as prjno from PB_GatePass   order by sopno';
      3: result := 'select distinct Work_Order as worder from PB_GatePass   order by worder';
    end; // case
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Params.Clear;
            Params.CreateParam(ftString, 'idno', ptInput);
            CommandText := 'update ido2 set idno=rTrim(idno)+''A'',FUser=isNull(FUser,'''')+' + QuotedStr('UnLock by ' + frmMain.UID) + ' where idno=:idno';
...
begin
  ...
  begin
    ...
    try
      ...
      ASQL := 'exec dbo.sp_PB_IDOMatching_Select '
        + QuotedStr(AIDO) + ','
        + QuotedStr(ALotNo) + ','
        + QuotedStr(ASopNo) + ','
        + QuotedStr(AWorderNo) + ','
...
var
  AIDO, ALOTID, ACONO, AWord, Itemcode: string;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if UnlockID(AIDO, ALOTID, ACONO, AWord, Itemcode, bIsWarehouse) then
          begin
...
function TfrmIDO.UnlockID(AIDO, ALOTID, ACONO, AWord, Itemcode: string; bIsWarehouse: boolean): boolean;
var
  ASQL: string;
begin
  with ClientDataSet_Temp1 do
  begin
    ...
    ASQL := 'exec dbo.sp_PB_IDOMatching_Unlock '
      + QuotedStr(AIDO) + ','
      + QuotedStr(ALOTID) + ','
      + QuotedStr(ACONO) + ','
      + QuotedStr(AWord) + ','
