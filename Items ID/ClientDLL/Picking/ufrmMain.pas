...
uses
  ...
  cxGridDBTableView, cxGrid, cxContainer, cxEdit, cxGroupBox, StdCtrls, DateUtils,
  ...
  cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, cxLabel, cxDBLabel, ShellAPI,
  cxLookAndFeels, ImgList, ADODB, RM_AsBarView, cxLookAndFeelPainters, INIFiles,
  cxCustomData, cxGraphics, cxFilter, cxData, cxDataStorage, cxDBData, cxDateUtils,
  cxCheckBox, cxBlobEdit, cxRadioGroup, cxImageComboBox;
...
type
  TfrmMain = class(TfrmBase)
    ...
    cxGrid1DBTableView1FERROR: TcxGridDBColumn;
    btnIDOMatching: TcxButton;
    cxGrid2DBTableView1InspectedFlag: TcxGridDBColumn;
    cxDBBlobEdit1: TcxDBBlobEdit;
    cxGrid2DBTableView1OrigQTY: TcxGridDBColumn;
    ClientDataSet_location: TClientDataSet;
    cxDBRadioGroup1: TcxDBRadioGroup;
    btnConfirmReserve: TcxButton;
    btnCancelReserve: TcxButton;
    ClientDataSet_Gatepass_M: TClientDataSet;
    cxGrid1DBTableView1ConfirmReserve: TcxGridDBColumn;
    Label17: TLabel;
    btnQueryReserve: TcxButton;
    cxButton5: TcxButton;
    ...
    ADOQuery5: TADOQuery;
    editIsDO: TcxDBRadioGroup;
    editIsSC: TcxDBCheckBox;
    cxGrid2DBTableView1PONO: TcxGridDBColumn;
    cxGrid2DBTableView1POLine: TcxGridDBColumn;
    editIsUPI: TcxDBCheckBox;
    cxGrid2DBTableView1EndCustColorCode: TcxGridDBColumn;
    btnSCoNo: TcxButton;
    Label18: TLabel;
    icbRequestDateNoon: TcxImageComboBox;
    cxDBDateEdit_RequestDate: TcxDBDateEdit;
    cxGrid2DBTableView1OrderLine: TcxGridDBColumn;
    btnAllIDO: TcxButton;
    ...
    procedure cxGrid1DBTableView1FERRORGetDisplayText(
      Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
      var AText: string);
    procedure btnIDOMatchingClick(Sender: TObject);
    procedure btnConfirmReserveClick(Sender: TObject);
    procedure btnCancelReserveClick(Sender: TObject);
    procedure cxGrid1DBTableView1ConfirmReserveGetDisplayText(
      Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
      var AText: string);
    procedure cxGrid1DBTableView1ConfirmReserveGetFilterDisplayText(
      Sender: TcxCustomGridTableItem; const AValue: Variant;
      var ADisplayText: string);
    procedure btnQueryReserveClick(Sender: TObject);
    procedure btnSubReportClick(Sender: TObject);
    procedure btnIDOSubReportClick(Sender: TObject);
    procedure btnSCoNoClick(Sender: TObject);
    procedure btnAllIDOClick(Sender: TObject);
  private
    ...
    FIssueSQL: string;
    fIsUPI: boolean; // 定義是IDO還是UPI

    procedure DoApplyUpdateData(bISSUED: boolean = False);
    procedure DoApplyUpdateIssueErpSuccess(conoi, wordi, trdti, istri, compi, locni, IDO: string; bErpSuccess: boolean; errfi: string);
    ...
    procedure GetIDOData(AIDO: string);
    ...
    function GetMaxIDO(isDO: boolean): string;
    ...
    function GetMarketWidthFromIDO(ido, Barcode, workorder, SKU, STOCKROOM: string): Double;
    ...
    procedure PrintIDO(ido: string);
    procedure PrintExt(ido: string; isDO, isSC: boolean; lang: string);
    function GetLang(): string;
    procedure SetIsUPI(const Value: boolean);
    function CheckSCInput: boolean;
  public
    { Public declarations }
//    FNew, FSystem21, FToHandHeld, FFromHandHeld, FIDOCompleted, FUnKnown: string;
    ...
    FsReserveStatus, FsConfirmReserve, FsCancelReserve, FsNeedReserve, FsUnReserve, FsWHReserveStatus: string;
    property IsUPI: boolean read fIsUPI write SetIsUPI;
    ...
    procedure UpdateIDOs2AS400_Exists(ACONO, AIDO, ALotID, AWord, AStatus: string);
    procedure IDOs2AS400ErrorLog(ACONO, AIDO, ALotID, AWord, AStatus: string);

    procedure DoDeleteIDODetail(AIDO: string);
  end;
...
uses ufrmDialog, ufrmIDO, ufrmDOOption;

{$R *.dfm}
var
  Fpono, FbuyerSizeCup, FbuyerSizeCups, FgarmentColor, FendCustStyle, Fdept, FworkOrderNo: string;
  Fpoline: string;
  fSuffixTable: string;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FieldByName('ISSUED_DATE').AsDateTime := GetServerDateTime;
          end;
          Post;
...
begin
  ...
  //fIsUPI := false;
  if (IsUPI) then
    fSuffixTable := '_Unplanned'
  else
    fSuffixTable := ''; //IDO: empty, UPI: _Unplanned
  ...
  ClientDataSet_location.RemoteServer := SCN;
  ClientDataSet_Gatepass_M.RemoteServer := SCN;
  ...
  // btnIDOMatching.Visible := ((urAdmin in FIDOUserRight) or (TUserRight(cUrIDOMatching) in FIDOUserRight));
  ...
  halfYear := SysUtils.IncMonth(SysUtils.Now, -6);
  with ClientDataSet_ido1 do
  begin
    Close;
    Params.Clear;
    //CommandText := 'select * from ido1' + fSuffixTable + '  where 1=2';

    if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
      CommandText := 'select * from ido1' + fSuffixTable + ' where rq>=' + QuotedStr(FormatDateTime('yyyy''/''mm''/''dd', halfYear)) + '  order by bh'
    else
      CommandText := 'select * from ido1' + fSuffixTable + ' where jlr=' + QuotedStr(UID) + ' order by bh';
...
begin
  ...
  //btnIDOSubReport.Caption := 'IDO ' + btnReport.Caption + ' SC...';
  ...
  FsConfirmReserve := GetLanguagesStr(97, 'Confirm Reserve');
  FsCancelReserve := GetLanguagesStr(98, 'Cancel Reserve');
  FsReserveStatus := GetLanguagesStr(100, 'Reserve Status');
  FsNeedReserve := GetLanguagesStr(95, 'Reserve Material');
  FsUnReserve := GetLanguagesStr(96, 'UnReserve Material');
  FsWHReserveStatus := GetLanguagesStr(99, 'WH Reserve Status:');

  btnConfirmReserve.Caption := FsConfirmReserve;
  btnCancelReserve.Caption := FsCancelReserve;
  cxDBRadioGroup1.Properties.Items[0].Caption := FsUnReserve;
  cxDBRadioGroup1.Properties.Items[1].Caption := FsNeedReserve;
  Label17.Caption := FsWHReserveStatus;
  ...
  cxGridDBColumn2.Caption := cxGrid3DBTableView1IDO.Caption;
  ...
  cxGrid2DBTableView1OrigQTY.Caption := GetLanguagesStr(139, 'Orig QTY', True);

  cxGrid1DBTableView1PARENTWO.Caption := GetLanguagesStr(93, 'Parent WO');
  cxGrid2DBTableView1PARENTWO.Caption := GetLanguagesStr(93, 'Parent WO');
  ...
  if (fIsUPI) then
  begin
    TabSheet_IDO.Caption := '      ' + 'UPI' + '      ';
    self.btnAllIDO.Caption := 'ALL UPI';
  end
  else
  begin
    TabSheet_IDO.Caption := '      ' + GetLanguagesStr(24, 'IDO') + '      ';
    self.btnAllIDO.Caption := 'ALL IDO';
  end;
  ...
  Label13.Caption := GetLanguagesStr(45, 'Notepad');
  ...
  cxGrid1DBTableView1PHISM.Caption := cxGrid2DBTableView1PHISM.Caption;
  ...
  cxGrid1DBTableView1ConfirmReserve.Caption := FsReserveStatus;
  ...
  //btnSubReport.Caption := GetLanguagesStr(3, 'Report', True) + ' SC'; ;
  ...
  cxGrid1DBTableView1FERROR.Caption := GetLanguagesStr(88, 'Error Cause', True);
  btnQueryReserve.Caption := FsReserveStatus + '-' + GetLanguagesStr(30, 'Query');
  cxButton5.Caption := btnQueryReserve.Caption;

  editIsDO.Properties.Items[0].Caption := GetLanguagesStr(125, 'IDO');
  editIsDO.Properties.Items[1].Caption := GetLanguagesStr(126, 'DO');
  editIsSC.Caption := GetLanguagesStr(127, 'Sub-contractor');
  btnSCoNo.Caption := GetLanguagesStr(140, 'SC', True);
...
begin
  ...
  begin
    Close;
    params.clear;
    CommandText := 'select  STATUS,IDO,LOT_ID  from PB_GATEPASS' + fSuffixTable + ' where ' + (frmMain.FSelectIDONo)
...
begin
  ...
  //    Close;
  //    Params.Clear;
  //    CommandText := 'select distinct IDO from PB_GATEPASS ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ' order by IDO';
  //    Open;
  //  end; // with
  ...
  try
    ...
    begin
      ...
      CommandText := 'select a.IDO,b.ErpSuccess as bSelect from '
        + ' (select distinct IDO from PB_GATEPASS' + fSuffixTable + ' ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') a'
        + ' Left join (select IDO,ErpSuccess from PB_GATEPASS' + fSuffixTable + ' where 1=2) b'
...
begin
  ...
  begin
    ...
    CommandText := 'exec sp_pb_IssueUpdate_New' + fSuffixTable + ' :@IDO,:@LOT_ID,:@Status,:@ISSUED_DATE,:@UID,:@State';
...
begin
  ...
  requestDate := DataSet.FieldByName('RequestDate').AsDateTime;
  icbRequestDateNoon.EditValue := null;
  if (not DataSet.FieldByName('RequestDate').IsNull) then
  begin
    isAfternoon := DateUtils.IsPM(requestDate);
    self.icbRequestDateNoon.EditValue := isAfternoon;
  end;
  ...
  try
    ...
    begin
      bh := DataSet.fieldbyname('bh').asstring;
      with ClientDataSet_Gatepass_M do
      begin
        close;
        params.clear;
        params.createparam(ftString, 'bh', ptinput);
        commandtext := 'select ConfirmReserve from PB_GatePass_M a where IDO=:bh';
        params[0].asstring := bh;
        open;
        bConfirmReserve := -1;
        if (RecordCount > 0) then
          bConfirmReserve := FieldByName('ConfirmReserve').AsInteger;

        case bConfirmReserve of //
          //1: AText := FsConfirmReserve;
          2: AText := FsConfirmReserve;
          3: AText := FsCancelReserve;
        else
          AText := '';
        end; // case
        Label17.Caption := FsWHReserveStatus + ':' + AText;
      end;
      ...
      begin
        ...
        params.clear;
        params.createparam(ftString, 'bh', ptinput);
        commandtext := 'select a.*,a.Qty*Cd/100 AS ActualQty from ido2' + fSuffixTable + ' a where a.bh=:bh';
        params[0].asstring := bh;
        open;
      end;
    end;
...
begin
  ...
  begin
    close;
    params.clear;
    commandtext := 'select Count(*) from ido2' + fSuffixTable
...
begin
  ...
  begin
    ...
    //      + '  and IDO in (select  IDO from PB_GATEPASS ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ')' + '  order by IDO';
    CommandText := 'select distinct a.IDO,b.ConfirmReserve from PB_GATEPASS' + fSuffixTable + ' a'
      + ' left join PB_GATEPASS_M b ON (a.IDO=b.IDO)'
      + ' where (a.status=' + IntToStr(Ord(bcsFromHandHeld))
      + ' or (a.status=' + IntToStr(Ord(bcsToSystem21)) + ' and a.ERPSuccess=0))'
      + ' and a.IDO in (select  IDO from PB_GATEPASS' + fSuffixTable + ' '
      + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ')'
      + ' and (b.ConfirmReserve=0 or b.ConfirmReserve=2 or b.ConfirmReserve is null)' //0: not need reserve ; 1: need reserve; 2: Confirm Reserve; 3: Cancel Reserve
    + ' order by a.IDO';
    Open;
  end; // with
  ...
  try
    ...
    vParam[0] := AIDONo;
    if (self.IsUPI) then
      SCN.AppServer.UpdateDataToERP(ord(ftUnplanned), vParam, bFinish)
    else
...
begin
  ...
  begin
    ...
    CommandText := 'select distinct a.IDO,b.ConfirmReserve from PB_GATEPASS' + fSuffixTable + ' a'
      + ' left join PB_GATEPASS_M b ON (a.IDO=b.IDO)'
      + ' where ((a.status=' + IntToStr(Ord(bcsToSystem21)) + ' and a.ERPSuccess=0) '
      + ' or a.status<>' + IntToStr(Ord(bcsToSystem21)) + ')'
      //      + ' and IDO not in (select IDO from PB_GATEPASS where status=' + IntToStr(Ord(bcsToSystem21)) + ')'
    + ' and a.IDO in (select IDO from PB_GATEPASS' + fSuffixTable + ' ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ')'
      + ' and (b.ConfirmReserve<>2 or b.ConfirmReserve is null)' //  ConfirmReserve=2 : Confirm Reserve
    + ' order by a.IDO';
    Open;
  end; // with
  if not DoSelectIDONo(AIDONo) then exit;
  ...
  try
    ...
    begin
      ...
      CommandText := 'exec sp_pb_CancelPickingIDO' + fSuffixTable + ' :@BH,:@UID,:@DT';
...
procedure TfrmMain.RefreshLocation();
var
  cono: string;
  sku: string;
  lotid: string;
...
begin
  Screen.Cursor := crSQLWait;
  try
    with ClientDataSet_location do
    begin
      Close;
      Params.Clear;
      CommandText := 'select distinct IDO,company,ITEM_CODE,LOT_ID from PB_GATEPASS' + fSuffixTable + ' '
        + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ' and (ERPSuccess=0 or  ERPSuccess is null)';
      Open;
      ...
      begin
        ...
        cono := FieldByName('company').AsString;
        sku := FieldByName('ITEM_CODE').AsString;
        lotid := FieldByName('LOT_ID').AsString;
        location := GetNewLocation(cono, sku, lotid);
        UpdateLocation(cono, sku, lotid, ido, location);
        Next;
      end;
    end;
  finally
...
var
  AStatus, ACase, ASQL, ASQL1: string;
  APrintType: Integer;
  //  vParam: Variant;
...
begin
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct status from PB_GATEPASS' + fSuffixTable + ' ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ' order by status';
    Open;
  end; // with
    ...
    begin
      ...
      begin
        ...
        if APrintType = 0 then
          ASQL := 'select *,' + ACase + ' from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True)
        else if APrintType = 1 then
          ASQL := 'select *,' + ACase + ' from (select * from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') a where  status=' + AStatus
        else //if APrintType=2 then
          ASQL := 'select *,' + ACase + ' from (select * from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') a where  ERPSuccess=1';

        CommandText := ASQL + ' order by ITEM_CODE,LOT_ID';
        Open;
        if RecordCount = 0 then
        begin
          Application.MessageBox(PChar(GetLanguagesStr(44, 'No Data', True)),
...
//        end; // while
      ...
      if APrintType = 0 then
        ASQL1 := 'select IDO from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True)
      else if APrintType = 1 then
        ASQL1 := 'select IDO from (select * from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') a where  status=' + AStatus
      else //if APrintType=2 then
        ASQL1 := 'select IDO from (select * from PB_GATEPASS' + fSuffixTable + '  ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') a where  ERPSuccess=1';

      with ClientDataSet_IDO1_Temp do
      begin
        Close;
        Params.Clear;
        CommandText := 'select a.Site,a.rq,a.shr,a.jlr,a.bm,a.bh from ido1' + fSuffixTable + ' a where a.bh in (' + ASQL1 + ')';
        Open;
      end; // with
...
var
  AIDO: string;
  isDO, isSC, aIsUPI: boolean;
begin
  if (not DoDOOption(isDO, isSC, aIsUPI)) then exit;
  ...
  begin
    ...
    try
      ...
      fieldbyname('IsDO').value := isDO;
      fieldbyname('IsSC').value := isSC;
      fieldbyname('IsUPI').value := IsUPI;
      fieldbyname('ReserveMaterial').value := False;
...
function TfrmMain.GetMaxIDO(isDO: boolean): string;
var
  AIDO: Integer;
  sIDO: string;
begin //IDO-0000021
  ...
  begin
    close;
    params.clear;
    commandtext := 'exec sp_GetMaxIDO' + fSuffixTable; //'select IDO from ido_max';
    open;
    AIDO := FieldByName('FID').AsInteger;
    //result:= FormatFloat('0000000', AIDO);

    //if (isDO) then
    //  result := 'DO-' + FormatFloat('0000000', AIDO)
        //else if (isUPI) then
//result := 'UPI-' + FormatFloat('0000000', AIDO)
    //else
    if (IsUPI) then
      result := 'UPI-' + FormatFloat('0000000', AIDO)
    else
...
var
  ...
  ydc, reserveMaterial: integer;
  isDO, isSC, aisUPI: integer;
  requestDate: tdatetime;
  isAfternoon: boolean;
  y, m, d: word;
begin
  ...
  reserveMaterial := Ord(ClientDataSet_ido1.FieldByName('reserveMaterial').AsBoolean);
  bh := ClientDataSet_ido1.FieldByName('bh').AsString;
  isDO := Ord(ClientDataSet_ido1.FieldByName('isDO').AsBoolean);
  isSC := Ord(ClientDataSet_ido1.FieldByName('isSC').AsBoolean);
  aisUPI := Ord(ClientDataSet_ido1.FieldByName('isUPI').AsBoolean);
  requestDate := ClientDataSet_ido1.FieldByName('RequestDate').AsDateTime;
  if (not ClientDataSet_ido1.FieldByName('RequestDate').IsNull) then
  begin
    if (icbRequestDateNoon.EditValue = null) then
      isAfternoon := false
    else
      isAfternoon := self.icbRequestDateNoon.EditValue;

    DecodeDate(requestDate, y, m, d);
    if (isAfternoon) then
    begin
      requestDate := EncodeDateTime(y, m, d, 15, 0, 0, 0);
    end
    else
    begin
      requestDate := EncodeDateTime(y, m, d, 10, 0, 0, 0);
    end;

    if not (ClientDataSet_ido1.State in [dsEdit, dsInsert]) then ClientDataSet_ido1.Edit;
    ClientDataSet_ido1.FieldByName('RequestDate').AsDateTime := requestDate;
  end;
  ...
  begin
    ...
    begin
      ...
      params.createparam(ftBoolean, 'reserveMaterial', ptinput);
      params.createparam(ftBoolean, 'isDO', ptinput);
      params.createparam(ftBoolean, 'isSC', ptinput);
      params.createparam(ftBoolean, 'isUPI', ptinput);
      params.createparam(ftDateTime, 'RequestDate', ptinput);
      CommandText := ' Insert into  IDO1' + fSuffixTable + '(cono,bh,rq,sj,site,jlr,ydc,ReserveMaterial,IsDO,IsSC,IsUPI,RequestDate) values(:cono,:bh,:rq,:sj,:site,:jlr,:ydc,:reserveMaterial,:IsDO,:IsSC,:IsUPI,:RequestDate)';
      ...
      Params.ParamByName('reserveMaterial').AsBoolean := ClientDataSet_ido1.FieldByName('reserveMaterial').AsBoolean;
      Params.ParamByName('isDO').AsBoolean := ClientDataSet_ido1.FieldByName('isDO').AsBoolean;
      Params.ParamByName('isSC').AsBoolean := ClientDataSet_ido1.FieldByName('isSC').AsBoolean;
      Params.ParamByName('isUPI').AsBoolean := ClientDataSet_ido1.FieldByName('isUPI').AsBoolean;
      if (not ClientDataSet_ido1.FieldByName('RequestDate').IsNull) then Params.ParamByName('RequestDate').Clear
      else Params.ParamByName('RequestDate').AsDateTime := requestDate;
      Execute;
  ...
  begin
    ...
    begin
      ...
      params.createparam(ftBoolean, 'ydc', ptinput);
      params.createparam(ftString, 'bh', ptinput);  }
      ASQL := 'Update IDO1' + fSuffixTable + ' set';
      ...
      ASQL := ASQL + 'Bm=' + QuotedStr(Bm) + ',Shr=' + QuotedStr(AShr) + ',Bmscx=' + QuotedStr(Bmscx) + ',Mem=' + QuotedStr(Mem)
        + ',reserveMaterial=' + IntToStr(reserveMaterial) + ',Bm1=' + QuotedStr(Bm1)
        + ',isDO=' + IntToStr(isDO)
        + ',isSC=' + IntToStr(isSC)
        + ',isUPI=' + IntToStr(aisUPI); // + ',ydc=' + IntToStr(ydc)
      if (not ClientDataSet_ido1.FieldByName('RequestDate').IsNull) then
        ASQL := ASQL + ',RequestDate=' + QuotedStr(DateTimeToStr(requestDate));
...
begin
  ...
  begin
    ...
    begin
      ...
      close;
      params.clear;
      sql := 'update ido2' + fSuffixTable + ' set qty=' + FloatToStr(ClientDataSet_ido2.fieldbyname('qty').AsFloat)
        + ',Status=' + IntToStr(ClientDataSet_ido2.fieldbyname('Status').AsInteger)
        + ',PHISM=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('PHISM').asstring))
        + ',PONO=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('PONO').asstring));
      if (not ClientDataSet_ido2.fieldbyname('POLine').IsNull) then
        sql := sql + ',POLine=' + IntToStr(ClientDataSet_ido2.fieldbyname('POLine').AsInteger)
      else sql := sql + ',POLine=null';
      sql := sql + ',EndCustColorCode=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('EndCustColorCode').asstring))
        + 'where bh=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('bh').asstring))
        + ' and idno=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('idno').asstring))
        + ' and zdh=' + QuotedStr(Trim(ClientDataSet_ido2.fieldbyname('zdh').asstring));
      commandtext := sql;
      {
     ...
     params.createparam(ftInteger, 'POLine', ptinput);
     params.createparam(ftString, 'EndCustColorCode', ptinput);
     sql := 'update ido2' + fSuffixTable + ' set qty=:qty,Status=:Status,PHISM=:PHISM, PONO=:PONO, POLine=:POLine, EndCustColorCode=:EndCustColorCode '
       + 'where bh=:bh and idno=:idno and zdh=:zdh ';
     commandtext := sql;
     if not ClientDataSet_ido2.fieldbyname('qty').isnull then
       params[0].asfloat := ClientDataSet_ido2.fieldbyname('qty').value
     else
       params[0].asfloat := 0;
     params[1].asInteger := ClientDataSet_ido2.fieldbyname('Status').AsInteger;
     params[2].asstring := ClientDataSet_ido2.fieldbyname('PHISM').asstring;
     params[3].asstring := ClientDataSet_ido2.fieldbyname('PONO').asstring;
     if (not ClientDataSet_ido2.fieldbyname('POLine').IsNull) then
       params[4].AsInteger := ClientDataSet_ido2.fieldbyname('POLine').AsInteger
     else
       params[4].Clear;
     params[5].asstring := ClientDataSet_ido2.fieldbyname('EndCustColorCode').asstring;

     params[6].asstring := ClientDataSet_ido2.fieldbyname('bh').value;
     params[7].asstring := ClientDataSet_ido2.fieldbyname('idno').value;
     params[8].asstring := ClientDataSet_ido2.fieldbyname('zdh').value;
       }
      execute;
    end
    else
    begin
      ...
      params.createparam(ftInteger, 'Status', ptinput);
      commandtext := 'insert into ido2' + fSuffixTable + '(bh,oper,idno,zdh,wl,sh,sz,qaresult,cd,ufd,mfd,qty,uom,wz,opdesc,aqty,StockRoom,Status) '
...
var
  AYDC, isDO, bHasIDO2Confirm: boolean;
begin
  ...
  begin
    ...
    begin
      ...
      isDO := fieldbyname('isDO').AsBoolean;
      ...
      SetcxEditReadOnly(cxDBBlobEdit1, AYDC);

      SetcxEditReadOnly(cxDBDateEdit_RequestDate, AYDC);
      SetcxEditReadOnly(self.icbRequestDateNoon, AYDC);
      ...
      SetcxEditReadOnly(cxDBRadioGroup1, AYDC);

      btnSelectMaterial.Enabled := not AYDC;
      btnAffirm.Enabled := not AYDC;
      btnSCoNo.Enabled := isDO;
      //btnPrint.Enabled :=  AYDC;
      btnDeleteIDO.Enabled := (not AYDC) and (not bHasIDO2Confirm);
      self.btnAllIDO.Visible := (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight);
...
begin
  ...
  try
    ...
    begin
      DoSelectMaterial(ACONO, AProjectNo, AWorder, (urAdmin in FIDOUserRight), IsUPI); //not btnDeleteIDO.Enabled
    end
    else
    begin
...
begin
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct b.zdh,a.cono from ido1' + fSuffixTable + ' a, ido2' + fSuffixTable + ' b where a.bh=b.bh and a.bh=' + QuotedStr(IDO)
...
var
  ...
  BH, worder, sTips: string;
  //  ISSUE_QTY: Double;
  bReserveMaterial: boolean;
  notepad: string;
begin
  inherited;
  //1.User Right
  if not CheckUserRight(cUrConfirm, TButton(Sender).Caption, FIDOUserRight) then exit;
  notepad := trim(ClientDataSet_ido1.FieldByName('Mem').AsString);

  bReserveMaterial := ClientDataSet_ido1.FieldByName('ReserveMaterial').AsBoolean;
  if (bReserveMaterial) then
    sTips := cxDBRadioGroup1.Properties.Items[1].Caption
  else
    sTips := cxDBRadioGroup1.Properties.Items[0].Caption;

  if Application.MessageBox(PChar(GetLanguagesStr(78, 'Are you sure to Confirm this IDO?') + '(' + sTips + ')?'),
    PChar(GetLanguagesStr(5, 'Tips', True)), MB_YESNO + MB_ICONInformation) = IDNO then exit;
  ...
  try
    ...
    else if (IsUPI and (notepad = '')) then
    begin
      Application.MessageBox(PChar('Must input UPI reason.'),
        PChar(GetLanguagesStr(5, 'Tips', True)), MB_OK + MB_ICONInformation);
      cxDBBlobEdit1.setfocus;
    end
    else
    begin
        ...
        begin
          ...
          try
            ...
            begin
              ...
              Params.CreateParam(ftBoolean, '@ydc', ptInput);
              Params.CreateParam(ftBoolean, '@reserveMaterial', ptInput);
              CommandText := 'exec sp_pb_IDOConfirm_Reserve_MC' + fSuffixTable + ' :@IDO,:@UID,:@Status_Issue,:@Status_ido2,:@ydc,:@reserveMaterial';
              //CommandText := 'exec sp_pb_IDOConfirm_Unplanned :@IDO,:@UID,:@Status_Issue,:@Status_ido2,:@ydc,:@State';
              ...
              Params.ParamByName('@reserveMaterial').AsBoolean := bReserveMaterial;
              Execute;
...
begin
  ...
  DoDeleteIDODetail(ClientDataSet_IDO1.fieldbyname('bh').asstring);
  {  with ClientDataSet_IDO2_Temp do
    begin
      close;
      params.clear;
  ...
  begin
    ...
    params.clear;
    params.createparam(ftString, 'bh', ptinput);
    commandtext := 'delete from ido1' + fSuffixTable + ' where bh=:bh and (ydc is null or ydc=0)';
    params[0].asstring := ClientDataSet_IDO1.fieldbyname('bh').value;
    execute;
  end;
  try
...
procedure TfrmMain.DoDeleteIDODetail(AIDO: string);
begin
  with ClientDataSet_IDO2_Temp do
  begin
    close;
    params.clear;
    params.createparam(ftString, 'bh', ptinput);
    commandtext := 'delete from ido2' + fSuffixTable + ' where bh=:bh and status<>' + IntToStr(Ord(bcsIDOAffirm));
    params[0].asstring := AIDO;
    execute;
  end;
  with ClientDataSet_IDO2 do
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        params.createparam(ftString, 'Zdh', ptinput);
        params.createparam(ftString, 'wl', ptinput);
        commandtext := 'delete from ido2' + fSuffixTable + ' where bh=:bh  and idno=:idno and Zdh=:Zdh and wl=:wl and status<>' + IntToStr(Ord(bcsIDOAffirm));
        params[0].asstring := ClientDataSet_IDO2.fieldbyname('bh').value;
      ...
      try
        if (ClientDataSet_IDO2.FieldByName('status').AsInteger <> Ord(bcsIDOAffirm)) then
        begin
          UpdateIDOs2AS400(
            ClientDataSet_IDO1.FieldByName('CONO').AsString,
            ClientDataSet_IDO1.FieldByName('BH').AsString,
...
var
  //  vParam: Variant;
  aCURR_QTY, IssueQty: Double;
  IDO, LOT_ID, ItemCode, CompanyNo, StockRoom, lang, receiveDept: string;
  bHaveCurrQty: boolean;
  isSC, isDO: boolean;
  //afilter: TRMExportFilter;
  //Report: TRMReport;
begin
  inherited;
  ...
  IDO := ClientDataSet_IDO1.fieldbyname('bh').asstring;
  receiveDept := ClientDataSet_IDO1.fieldbyname('bm1').asstring;
  isSC := ClientDataSet_IDO1.fieldbyname('IsSC').AsBoolean;
  isDO := ClientDataSet_IDO1.fieldbyname('IsDO').AsBoolean;
  //if (receiveDept = 'Suntree') then isDO := false;
  lang := GetLang;

  if (isSC and (not CheckSCInput)) then exit;

  PrintExt(IDO, isDO, isSC, lang);
  //PrintIDO(IDO);

end;

function TfrmMain.CheckSCInput: boolean;
var
  sco, lotid: string;
begin
  with ClientDataSet_IDO2 do
  begin
    ...
    while not eof do
    begin
      sco := FieldByName('PONO').AsString;
      lotid := FieldByName('idno').AsString;
      if (Trim(sco) = '') then
      begin
        ShowMessage('Please input ScO no. of Lot ID: ' + lotid);
        result := false;
        exit;
      end;
      next;
...
function TfrmMain.GetLang(): string;
begin
  if (LanguageID = 134) or (LanguageID = 136) then
    result := 'CN'
  else if (LanguageID = 222) then
    result := 'TH'
  else //if (LanguageID=0 or LanguageID=1) then
    result := 'EN';
end;

procedure TfrmMain.PrintExt(ido: string; isDO, isSC: boolean; lang: string);
var
  params: string;
  AIniFile: TIniFile;
  sPath, ASection, sIsDO, sIsSC, sIsUPI: string;
begin
  AIniFile := TIniFile.Create(ExtractFilePath(Application.ExeName) + cClientIniFile);
  try
    ASection := 'WinAppPlatformPath';
    sPath := AIniFile.ReadString(ASection, 'Path', 'D:\live\PH.Platform_LIVE\Release');
  finally // wrap up
    FreeAndNil(AIniFile);
  end; // try/finally

  if (not FileExists(sPath + '\PH.Platform.PHWinApp.exe')) then
  begin
    ShowMessage('Please use Citrix to Print DO/IDO report.');
    exit;
  end;

  if (isDO) then sIsDO := 'true' else sIsDO := 'false';
  if (isSC) then sIsSC := 'true' else sIsSC := 'false';
  if (IsUPI) then sIsUPI := 'true' else sIsUPI := 'false';

  params := '"Joew" "123" "EN" "PH.RWO.UI3.0.dll" "PH.RWO.UI.Report.ExReport" "PH.Barcode.BackEnd.dll,PH.Barcode.BackEnd.JobIDOReport,'
    + ido + ';' + sIsDO + ';' + sIsSC + ';' + sIsUPI + ';' + lang;
  //showmessage(params);
  ShellExecute(0, 'open', pchar('PH.Platform.PHWinApp.exe'), pchar(params), pchar(sPath), sw_show);

end;

procedure TfrmMain.PrintIDO(ido: string);
var
  //  vParam: Variant;
  aCURR_QTY, IssueQty: Double;
...
begin
  ...
  begin
    ...
    try
      ...
      commandtext := 'select a.*,b.*,a.cono as COMPANY,b.idno as LOT_ID,Qty as CurQty,Stockroom as Split,b.wl as ITEM_CODE,c.ConfirmUser as WHConfirmUser,c.ConfirmTime as WHConfirmTime'
        + ',b.wl as pono'
        + ',b.wl as buyerSizeCup'
        + ',b.Fuser as buyerSizeCups'
        + ',b.wl as garmentColor'
        + ',b.wl as endCustStyle'
        + ',t.Description as EN'
        + ',t.CN'
        + ',t.TH'
        + ' from ido1' + fSuffixTable + ' a'
        + ' left join PB_GatePass_M c on (a.bh=c.IDO and c.ConfirmReserve=2),ido2' + fSuffixTable + ' b'
        + ' left join [PH.MIDc].dbo.[MaterialType] t on (Left(b.wl,3)=t.MaterialType)'
        + ' where b.bh=a.bh and a.bh=:bh order by b.wl,b.oper,b.idno';
      params[0].asstring := ido;
      open;

      isSC := fieldbyname('IsSC').AsBoolean;
      isDO := fieldbyname('IsDO').AsBoolean;
      wdesc := fieldbyname('Bm1').AsString;
      //get the select record's curQty from ERP
      ...
      begin
        ...
        GetPOInfor(CompanyNo, LOT_ID, ItemCode);
        //pono,poline,buyerSizeCup,buyerSizeCups,garmentColor,endCustStyle

        Edit;
        FieldByName('pono').AsString := Fpono;
        ...
        FieldByName('endCustStyle').AsString := FendCustStyle;
  ...
  begin
    ...
    if (isDO) then
      LoadFromResourceName(hInstance, 'DO')
    else
    ...
    Dictionary.Variables.AsString['FIDO'] := fIDO;
    if (not isDO) then
      Dictionary.Variables.AsString['FReportTitle'] := GetLanguagesStr(70, 'IDO')
    else
    begin
      Dictionary.Variables.AsString['FReportTitle'] := GetLanguagesStr(94, 'DO');
      Dictionary.Variables.AsString['FIDO'] := StringReplace(fIDO, 'IDO', 'DO', []);
    end;
    //if (isSubContract) then
    //  Dictionary.Variables.AsString['FReportSubTitle'] := GetLanguagesStr(116, '- Subcontracting')
    //else
    Dictionary.Variables.AsString['FReportSubTitle'] := '';
    Dictionary.Variables.AsString['FDate'] := GetLanguagesStr(23, 'Date', True);
    Dictionary.Variables.AsString['FTime'] := GetLanguagesStr(38, 'Time', True);
    Dictionary.Variables.AsString['FPage'] := GetLanguagesStr(47, 'Page', True);
    Dictionary.Variables.AsString['FStatus'] := GetLanguagesStr(12, 'Status', True);
    ...
    Dictionary.Variables.AsString['FStockRoom'] := GetLanguagesStr(16, 'Stock Room', True);
    Dictionary.Variables.AsString['FCompany'] := GetLanguagesStr(17, 'Company', True);
    Dictionary.Variables.AsString['FSuppRef'] := GetLanguagesStr(79, 'Supp Refer No.', True);
    ...
    Dictionary.Variables.AsString['FSh'] := GetLanguagesStr(52, 'Color');
    Dictionary.Variables.AsString['FSz'] := GetLanguagesStr(53, 'Color Group/Shade');
    ...
    Dictionary.Variables.AsString['FOrigQTY'] := GetLanguagesStr(139, 'Orig QTY', True);
    Dictionary.Variables.AsString['FNotepad'] := GetLanguagesStr(45, 'Notepad');

    if (isSC) then
      Dictionary.Variables.AsString['FPO'] := GetLanguagesStr(140, 'SC', True)
    else
      Dictionary.Variables.AsString['FPO'] := GetLanguagesStr(130, 'PO', True);
    Dictionary.Variables.AsString['FGarmentColor'] := GetLanguagesStr(133, 'Garment Color', True);
    Dictionary.Variables.AsString['FEndCustStyle'] := GetLanguagesStr(134, 'End Cust Style#', True);
    Dictionary.Variables.AsString['FBuyerSizeCup'] := GetLanguagesStr(138, 'Buyer Size/Cup', True);
    Dictionary.Variables.AsString['FBuyerSizeCups'] := GetLanguagesStr(135, 'Buyer Size/Cups', True);

    Dictionary.Variables.AsString['FPrepareBy'] := GetLanguagesStr(101, 'Prepare By');
    Dictionary.Variables.AsString['FWHConfirmBy'] := GetLanguagesStr(102, 'WH Confirm By');
    Dictionary.Variables.AsString['FWHTranBy'] := GetLanguagesStr(103, 'WH Tran By');
    Dictionary.Variables.AsString['FReceivingBy'] := GetLanguagesStr(104, 'Receiving By');
    Dictionary.Variables.AsString['FPrepareDept'] := GetLanguagesStr(105, 'Prepare Dept');
    Dictionary.Variables.AsString['FPickingDept'] := GetLanguagesStr(106, 'Picking Dept');
    if (isSC) then
    begin
      Dictionary.Variables.AsString['FConsigneeCode'] := GetLanguagesStr(130, 'Sub-contractor Code');
      Dictionary.Variables.AsString['FConsigneeName'] := GetLanguagesStr(129, 'Sub-contractor/Consignee Name');
    end
    else
    begin
      Dictionary.Variables.AsString['FReceivingDept'] := GetLanguagesStr(107, 'Receiving Dept');
    end;
    Dictionary.Variables.AsString['FPickingUse'] := GetLanguagesStr(108, 'Picking Use');
    Dictionary.Variables.AsString['FTransUse'] := GetLanguagesStr(109, 'Trans Use');

    Dictionary.Variables.AsString['F1ststep'] := GetLanguagesStr(110, '1st step');
    Dictionary.Variables.AsString['F2ndstep'] := GetLanguagesStr(111, '2nd step');
    Dictionary.Variables.AsString['FParty'] := GetLanguagesStr(112, 'Party Name/Code');
    Dictionary.Variables.AsString['FPerson'] := GetLanguagesStr(113, 'Person');
    Dictionary.Variables.AsString['FPersonAppointed'] := GetLanguagesStr(114, 'Person/Appointed Person*');
    Dictionary.Variables.AsString['FReceivingDesc'] := GetLanguagesStr(115, '**Mat''l receive party has inter-party and incl. sub-contractor also.');

    //if (isSubContract) then
    //  Dictionary.Variables.AsString['FCopy'] := GetLanguagesStr(117, 'Sub-contractor Copy')
    //else
    Dictionary.Variables.AsString['FCopy'] := GetLanguagesStr(118, 'PH Copy');
    Dictionary.Variables.AsString['FSubcontractorNameAddress'] := GetLanguagesStr(119, 'Sub-contractor Name/Address');
    Dictionary.Variables.AsString['FSign'] := GetLanguagesStr(120, 'Sign');
    Dictionary.Variables.AsString['FMaterialDesc'] := GetLanguagesStr(121, 'Item Desc');
    Dictionary.Variables.AsString['FEN'] := GetLanguagesStr(122, 'English');
    Dictionary.Variables.AsString['FCN'] := GetLanguagesStr(123, 'Chinese');
    Dictionary.Variables.AsString['FDeliveryAddress'] := GetLanguagesStr(128, 'Delivery Address');

    with ClientDataSet_Temp do
    begin
      Close;
      Params.Clear;
      CommandText := 'select distinct Wcode ,Wdesc ,WName ,Consignee ,DeliveryAddress from IDO4  where Wdesc='
        + QuotedStr(wdesc);
      Open;
      Dictionary.Variables.AsString['FConsigneeValue'] := FieldByName('Wcode').AsString;
      Dictionary.Variables.AsString['FDeliveryAddressValue'] := FieldByName('DeliveryAddress').AsString;
    end; // with
    ...
    //ExportTo(RMllPDFExport1,'c:\temp\test.pdf');
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Params.Clear;
      if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
        CommandText := 'select distinct Bh from ido1' + fSuffixTable + ' where cono=' + QuotedStr(UpperCase(cxTextEdit1.Text)) + ' order by bh'
      else
        CommandText := 'select distinct Bh from ido1' + fSuffixTable + ' where cono=' + QuotedStr(UpperCase(cxTextEdit1.Text)) + 'and jlr=' + QuotedStr(UID) + ' order by bh';
...
begin
  ...
  try
    ...
    begin
      ...
      Params.Clear;
      CommandText := 'select a.IDO,b.ErpSuccess as bSelect from '
        + ' (select distinct IDO from PB_GATEPASS' + fSuffixTable + ') a Left join (select IDO,ErpSuccess from PB_GATEPASS' + fSuffixTable + ' where 1=2) b'
        + ' on (a.IDO=b.IDO) order by a.IDO';
      Open;
    end; // with
    cxGrid3DBTableView1.DataController.DataSource := DataSource2;
...
begin
  ...
  begin
    ...
    try
      //1.get sql
      sWhere := bcGetWhereSQL(FIssueSQL, 'IDO', (AIDO));
      //2.show data
      Close;
      aSQL := 'SELECT * FROM PB_GatePass' + fSuffixTable + ' ';
      aSQL := aSQL + ' ' + sWhere;
      //aSQL := aSQL + ' ORDER BY IDO';
      FIssueSQL := aSQL;
      aSQL := 'select a.*,b.ConfirmReserve from (' + aSQL + ') a left join  PB_GatePass_M b on (a.IDO=b.IDO) ORDER BY a.IDO';
      CommandText := aSQL;
      Open;
    finally // wrap up
      EnableControls;
...
begin
  ...
  begin
    ...
    begin
      if (IsUPI) then
        AIDO := GetIDOFullString(AIDO, 'UPI')
      else
        AIDO := GetIDOFullString(AIDO, 'IDO');
...
begin
  ...
  btnConfirmReserve.Enabled := bEnable;
  btnCancelReserve.Enabled := bEnable;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if (IsUPI) then
            SQL.Add('and a.ISSFI3=''1'' ')
          else
            SQL.Add('and a.ISSFI3=''0'' ');

          SQL.Add('order by a.errfi3 desc');
          ...
          begin
            ...
            DoApplyUpdateIssueErpSuccess(conoi, wordi, trdti, istri, compi, locni, IDO, bErpSuccess, errfi);
            if bErpSuccess then
    ...
    begin
      ...
      try
        ...
        begin
          Close;
          Params.Clear;
          CommandText := 'select distinct COMPANY,LOT_ID,LOT_ID_Sub,ITEM_CODE,STOCKROOM from PB_GatePass' + fSuffixTable + ' '
            + bcGetWhereSQL(FIssueSQL, 'IDO', '', True)
...
procedure TfrmMain.DoApplyUpdateIssueErpSuccess(conoi, wordi, trdti, istri, compi, locni, IDO: string; bErpSuccess: boolean; errfi: string);
var
  sDT: string;
begin
  ...
  begin
    ...
    Params.CreateParam(ftString, '@IDO', ptInput);
    Params.CreateParam(ftString, '@UID', ptInput);
    Params.CreateParam(ftString, '@FERROR', ptInput);
    CommandText := 'exec sp_pb_Update_ErpSuccess_New :@COMPANY,:@WORK_ORDER,:@DATE,:@Stockroom,:@ITEM_CODE,:@LOT_ID,:@ErpSuccess,:@Type,:@IDO,:@UID,:@FERROR';
    ...
    if (IsUPI) then
      Params.ParamByName('@Type').AsInteger := 3 // UPI @Type=3
    else
      Params.ParamByName('@Type').AsInteger := 1;

    Params.ParamByName('@IDO').AsString := IDO;
    Params.ParamByName('@UID').AsString := UID;
    Params.ParamByName('@FERROR').AsString := errfi;
    Execute;
  end; // with
end;
...
begin
  ...
  begin
    ...
    CommandText := 'exec sp_pb_Update_IssueSubLotNo' + fSuffixTable + ' :@COMPANY,:@DATE,:@Stockroom,:@ITEM_CODE,:@LOT_ID,:@LOT_ID_Sub';
    Params.ParamByName('@COMPANY').AsString := conoi;
...
begin
  ...
  ClientDataSet1.Close;
  FIssueSQL := '';
  ClientDataSet1.CommandText := FIssueSQL;
end;
...
begin
  ...
  if clientdataset_ido1.state in [dsEdit, dsInsert] then clientdataset_ido1.post;
  if clientdataset_ido2.state in [dsEdit, dsInsert] then clientdataset_ido2.post;
  //clientdataset_ido2.ApplyUpdates(0);
  //btnIDO2Post.Enabled := False;
end;
...
var
  //  vParam: Variant;
  bcPrinter: string;
  SKU, HDONO, CompanyNo, STOCKROOM, Barcode, UOM, SubLot, PGMN, Location, PHISM, ColorShade, Project, PO, Dept: string;
  aCURR_QTY, aORIG_QTY, aIssueQty, MarketWidth: Double;
  iCopies: Integer;
  bCollateCopies, bHaveCurrQty: boolean;
  DT: Integer;
  TM: string;
  ido, WorkOrderNo, GarmentColor, CustStyleNo, BuyerSizeCups, BuyerSizeCup, SuppColor, workorder: string;
begin
  ...
  begin
    ...
    //        + ' (select * from PB_GatePass ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') b'
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Project := FieldByName('PROJECT_NO').AsString;
        ido := FieldByName('IDO').AsString;
        workorder := FieldByName('WORK_ORDER').AsString;
        MarketWidth := GetMarketWidthFromIDO(ido, Barcode, workorder, SKU, STOCKROOM);
        GetPOInfor(CompanyNo, Barcode, SKU);
        ...
        begin
          ...
          begin
            ...
            FieldByName('WorkOrderNo').AsString := workorder;
            FieldByName('GarmentColor').AsString := FgarmentColor;
...
function TfrmMain.GetMarketWidthFromIDO(ido, Barcode, workorder, SKU, STOCKROOM: string): Double;
begin
  with ClientDataSet_Temp do
  begin
    Close;
    ...
    Params.CreateParam(ftString, 'STOCKROOM', ptInput);
    Params.CreateParam(ftString, 'Zdh', ptInput);
    CommandText := 'select mfd from IDO2' + fSuffixTable
      + ' where bh=:bh and wl=:wl  and Idno=:Idno and STOCKROOM=:STOCKROOM and Zdh=:Zdh';
    Params.ParamByName('bh').AsString := ido;
    Params.ParamByName('wl').AsString := SKU;
    Params.ParamByName('Idno').AsString := Barcode;
    Params.ParamByName('STOCKROOM').AsString := STOCKROOM;
    Params.ParamByName('Zdh').AsString := workorder;
    Open;
    result := FieldByName('mfd').AsFloat;
  end; // with
end;
...
begin
  ...
  begin
    ...
    CommandText := 'select distinct a.HDO_NO,a.COMPANY from PB_FINALID a, '
      + ' (select * from PB_GatePass' + fSuffixTable + ' ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + ') b'
      + ' where b.Item_Code=a.Item_Code  and b.LOT_ID=a.LOT_ID and b.STOCKROOM=a.STOCKROOM and b.Company=a.Company'
      + ' and b.ERPSuccess=1 and b.status=' + IntToStr(Ord(bcsToSystem21));
    Open;
    AHDOs := '';
    ...
    try
      ...
      begin
        ...
        begin
          ...
          FieldByName('ConfirmBy').AsString := UID;
          FieldByName('ConfirmDate').AsDateTime := Date;
...
var
  //  vParam: Variant;
  aCURR_QTY, IssueQty: Double;
  LOT_ID, ItemCode, CompanyNo, StockRoom, AIDONo, lang, receiveDept: string;
  bHaveCurrQty: boolean;
  isSC, isDO: boolean;
begin
  inherited;
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct IDO from PB_GATEPASS' + fSuffixTable + ' ' + bcGetWhereSQL(FIssueSQL, 'IDO', '', True) + '  order by IDO';
    Open;
  end; // with
  if not DoSelectIDONo(AIDONo) then exit;
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select *  from IDO1' + fSuffixTable + ' where bh= ' + QuotedStr(AIDONo);
    Open;
    isSC := fieldbyname('IsSC').AsBoolean;
    isDO := fieldbyname('IsDO').AsBoolean;

    receiveDept := fieldbyname('bm1').asstring;
    if (receiveDept = 'Suntree') then isDO := false;
  end; // with

  lang := GetLang;

  PrintExt(AIDONo, isDO, isSC, lang);
  ...
  begin
    ...
    try
      ...
      commandtext := 'select a.*,b.*,a.cono as COMPANY,b.idno as LOT_ID,Qty as CurQty,Stockroom as Split,b.wl as ITEM_CODE,c.ConfirmUser as WHConfirmUser,c.ConfirmTime as WHConfirmTime'
        + ' from ido1 a'
        + ' left join PB_GatePass_M c on (a.bh=c.IDO and c.ConfirmReserve=2),ido2 b'
        + ' where b.bh=a.bh and a.bh=:bh order by b.wl,b.oper,b.idno';
      params[0].asstring := AIDONo;
      open;
  ...
  begin
    ...
    Dictionary.Variables.AsString['FReportTitle'] := GetLanguagesStr(70, 'Material Issue Work List');
    Dictionary.Variables.AsString['FDate'] := GetLanguagesStr(23, 'Date', True);
    Dictionary.Variables.AsString['FTime'] := GetLanguagesStr(38, 'Time', True);
    Dictionary.Variables.AsString['FPage'] := GetLanguagesStr(47, 'Page', True);
    Dictionary.Variables.AsString['FStatus'] := GetLanguagesStr(12, 'Status', True);
...
{*
procedure TfrmMain.btnIDOReportClick1(Sender: TObject);
begin
  inherited;
  if not CheckUserRight(cUrReport, TButton(Sender).Caption, UserRights) then exit;
  FShowIDOReport := True;
  ...
  begin
    ...
    try
      ...
      commandtext := 'select a.*,b.*,a.cono as COMPANY,b.idno as LOT_ID,Qty as CurQty,Stockroom as Split,b.wl as ITEM_CODE'
        + '  from ido1' + fSuffixTable + ' a,ido2' + fSuffixTable + ' b where b.bh=a.bh and a.bh=:bh order by b.wl,b.oper,b.idno';
      params[0].asstring := IDO;
      open;
...
procedure TfrmMain.btnChangToYellowClick(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      Params.CreateParam(ftString, '@IDO', ptInput);
      Params.CreateParam(ftString, '@LOT_ID', ptInput);
      CommandText := 'update PB_GATEPASS' + fSuffixTable + ' set Status=:@Status, ISSUED_DATE=:@ISSUED_DATE,ERPSuccess=null where IDO=:@IDO  and LOT_ID=:@LOT_ID';
      Params.ParamByName('@Status').AsInteger := ClientDataSet1.FieldByName('Status').AsInteger;
...
begin
  ...
  begin
    ...
    {
  GetIDOData(AIDO);
  cxButtonEdit_BH.Text := AIDO;
  }

    if LocateBH(AIDO) then cxButtonEdit_BH.Text := AIDO
    else
    begin
      AIDO := GetIDOFullString(AIDO, 'DO');
      if LocateBH(AIDO) then cxButtonEdit_BH.Text := AIDO;
    end;
...
procedure TfrmMain.GetIDOData(AIDO: string);
var
  aSQL: string;
begin
  with ClientDataSet_IDO1 do
  begin
    ...
    try
      Close;
      if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
        aSQL := 'SELECT * FROM IDO1' + fSuffixTable + ' where BH=' + QuotedStr(AIDO)
      else
        aSQL := 'SELECT * FROM IDO1' + fSuffixTable + ' where BH=' + QuotedStr(AIDO) + ' and jlr=' + QuotedStr(UID);
      CommandText := aSQL;
      Open;
    finally // wrap up
      EnableControls;
...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add('call ZPHLIB_AUL.sp_UpdateIDOs2AS400_NEW ('
        + QuotedStr(ACONO) + ','
        + QuotedStr(AIDO) + ','
        + QuotedStr(ALotID) + ','
        + QuotedStr(AWord) + ','
...
procedure TfrmMain.UpdateIDOs2AS400_Exists(ACONO, AIDO, ALotID, AWord, AStatus: string);
var
  s: string;
begin
  if (IsUPI) then s := 'U' else s := 'P';
  ...
  try
    ...
    begin
      ...
      SQL.Clear;
      //IN i_cono varchar(2),IN i_IDO varchar(12),IN i_LOTID varchar(15),IN i_Word varchar(7),IN i_Status varchar(1)
      SQL.Add('call ZPHLIB_AUL.sp_UpdateIDOs2AS400_Exists_NEW ('
        + QuotedStr(ACONO) + ','
        + QuotedStr(AIDO) + ','
        + QuotedStr(ALotID) + ','
        + QuotedStr(AWord) + ','
...
procedure TfrmMain.cxGrid1DBTableView1FERRORGetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: string);
var
  AStr, AStr1: string;
...
begin
  ...
  begin
    AStr1 := AStr[I];
    if AStr1 = '1' then
      //AText := AText + GetErrorCause(I, self.LanguageID) + '; ';
      AText := AText + GetLanguagesStr(3000 + I, '', True) + '; ';
  end;
  AText := Trim(AText);
end;

procedure TfrmMain.btnIDOMatchingClick(Sender: TObject);
begin
  inherited;
  if RzPageControl1.ActivePageIndex = 0 then
  begin
    if not CheckUserRight(cUrIDOMatching, TButton(Sender).Caption, FIDOUserRight) then exit;
  end
  else
  begin
    if not CheckUserRight(cUrIDOMatching, TButton(Sender).Caption, UserRights) then exit;
  end;
...
procedure TfrmMain.btnConfirmReserveClick(Sender: TObject);
var
  sWhere, aSQL, AIDONo: string;
begin
  inherited;
  if not CheckUserRight(cUrReserveMaterial, TButton(Sender).Caption, UserRights) then exit;
  //0: not need reserve ; 1: need reserve; 2: Confirm Reserve; 3: Cancel Reserve
  with ClientDataSet2 do
  begin
    sWhere := bcGetWhereSQL(FIssueSQL, 'IDO', '', True);
    aSQL := 'SELECT * FROM PB_GatePass' + fSuffixTable;
    aSQL := aSQL + ' ' + sWhere;
    aSQL := 'select distinct a.IDO from (' + aSQL + ') a ,PB_GatePass_M b '
      + ' where (a.IDO=b.IDO) and (b.ConfirmReserve=1 or b.ConfirmReserve=3)'
      + ' and not exists (select * from (' + aSQL + ') c where c.IDO=a.IDO '
      + ' and c.status=' + IntToStr(Ord(bcsToSystem21)) + ')'
      + ' ORDER BY a.IDO';
  ...
  begin
    ...
    Params.CreateParam(ftString, '@IDO', ptInput);
    Params.CreateParam(ftString, '@UID', ptInput);
    CommandText := 'exec sp_pb_IDOConfirm_Reserve_WH :@IDO,:@UID ';
    Params.ParamByName('@IDO').AsString := AIDONo;
    Params.ParamByName('@UID').AsString := UID;
    Execute;
  end; // with
  ClientDataSet1.Open;
  Application.MessageBox(PChar(FsConfirmReserve + ' ' + GetLanguagesStr(66, 'Success', True) + '.'),
    PChar(GetLanguagesStr(5, 'Tips', True)), MB_OK + MB_ICONInformation);

end;

procedure TfrmMain.btnCancelReserveClick(Sender: TObject);
var
  sWhere, aSQL, AIDONo: string;
begin
  inherited;
  if not CheckUserRight(cUrCancelReserve, TButton(Sender).Caption, UserRights) then exit;
  //0: not need reserve ; 1: need reserve; 2: Confirm Reserve; 3: Cancel Reserve
  with ClientDataSet2 do
  begin
    sWhere := bcGetWhereSQL(FIssueSQL, 'IDO', '', True);
    ...
    aSQL := aSQL + ' ' + sWhere;
    aSQL := 'select distinct a.IDO from (' + aSQL + ') a ,  PB_GatePass_M b '
      + ' where (a.IDO=b.IDO) and (b.ConfirmReserve=2)'
      + ' ORDER BY a.IDO';
  ...
  begin
    ...
    Params.CreateParam(ftString, '@IDO', ptInput);
    Params.CreateParam(ftString, '@UID', ptInput);
    CommandText := 'exec sp_pb_IDOCancel_Reserve_WH :@IDO,:@UID ';
    Params.ParamByName('@IDO').AsString := AIDONo;
    Params.ParamByName('@UID').AsString := UID;
    Execute;
  end; // with
  ...
  Application.MessageBox(PChar(FsCancelReserve + ' ' + GetLanguagesStr(66, 'Success', True) + '.'),
    PChar(GetLanguagesStr(5, 'Tips', True)), MB_OK + MB_ICONInformation); end;

procedure TfrmMain.cxGrid1DBTableView1ConfirmReserveGetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: string);
//FsReserveStatus, FsConfirmReserve, FsCancelReserve, FsNeedReserve, FsUnReserve
//0: not need reserve ; 1: need reserve; 2: Confirm Reserve; 3: Cancel Reserve
var
  AInt: Integer;
begin
...
procedure TfrmMain.cxGrid1DBTableView1ConfirmReserveGetFilterDisplayText(
  Sender: TcxCustomGridTableItem; const AValue: Variant;
  var ADisplayText: string);
var
  AInt: Integer;
begin
  ...
  AInt := AValue;
  case AInt of //
    0: ADisplayText := FsUnReserve;
    1: ADisplayText := FsNeedReserve;
    2: ADisplayText := FsConfirmReserve;
    3: ADisplayText := FsCancelReserve;
  end; // case
end;

procedure TfrmMain.btnQueryReserveClick(Sender: TObject);
begin
  inherited;
  if RzPageControl1.ActivePageIndex = 0 then
  begin
...
procedure TfrmMain.btnSubReportClick(Sender: TObject);
begin
  if not CheckUserRight(cUrReport, TButton(Sender).Caption, FIDOUserRight) then exit;
  //if not (ClientDataSet_ido1.FieldByName('YDC').AsBoolean) then
  //begin
  ...
  if ClientDataSet_IDO2.state in [dsEdit, dsInsert] then ClientDataSet_IDO2.post;
  PrintIDO(ClientDataSet_IDO1.fieldbyname('bh').asstring);

end;

procedure TfrmMain.btnIDOSubReportClick(Sender: TObject);
var
  //  vParam: Variant;
  aCURR_QTY, IssueQty: Double;
  LOT_ID, ItemCode, CompanyNo, StockRoom, AIDONo: string;
  bHaveCurrQty: boolean;
begin
  inherited;
  if not CheckUserRight(cUrReport, TButton(Sender).Caption, UserRights) then exit;
...
procedure TfrmMain.btnSCoNoClick(Sender: TObject);
var
  sc: string;
begin
  inherited;
  with clientdataset_ido2 do
  begin
    sc := FieldByName('PONO').AsString;
    if (InputQuery('Please Input SC order No.', 'SCo No.', sc)) then
    begin
      First;
      while not eof do
      begin
        Edit;
        FieldByName('PONO').AsString := sc;
        Next;
      end;
      btnIDO2PostClick(nil);
    end;
  end;
end;

procedure TfrmMain.SetIsUPI(const Value: boolean);
begin
  fIsUPI := Value;
end;

procedure TfrmMain.btnAllIDOClick(Sender: TObject);
begin
  inherited;
  if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
    with ClientDataSet_ido1 do
    begin
      Close;
      Params.Clear;
      CommandText := 'select * from ido1' + fSuffixTable + '  order by bh';
      Open;
      First;
    end; // with
end;
