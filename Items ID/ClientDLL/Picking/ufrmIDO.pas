...
type
  TfrmIDO = class(TForm)
    ...
    procedure cxGridDBColumn16GetFilterDisplayText(
      Sender: TcxCustomGridTableItem; const AValue: Variant;
      var ADisplayText: string);
    procedure cxGrid2DBTableView1Column9GetDisplayText(
      Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
      var AText: String);
  private
...
// ---changed to sql server Openquery
{*
...
*}
  if (fIsUPI) then  isUPI :=  1
  else isUPI := 0;

 // ---changed to sql server Openquery
  with ClientDataSet1 do
  begin
    Screen.Cursor := crSQLWait;                                                                                          // fIsUPI := frmMain.IsUPI;
    DisableControls;
    ...
    ASQL := 'exec [PHGDB2].[dbo].[sp_pb_MaterialSelectFromAS400]  '+ QuotedStr(FCONO) + ',' + QuotedStr(FProjectNo)+ ',' + IntToStr(isUPI);
...
begin
  ...
  ClientDataSet1.ProviderName:=  'dspPHG';
  ClientDataSet1.RemoteServer := frmMain.SCN;   //changed to sqlserver(openquery) from as400
  ...
  begin
    ...
    try
      ...
      begin
        ...
        salesOrderLine:=StrToIntDef(fieldbyname('OrderLine').AsString, 0);
        ...
        begin
          ...
          begin
            ...
            params.createparam(ftInteger, 'OrderLine', ptinput);
            commandtext := 'insert into ido2' + fSuffixTable + '(bh,oper,idno,zdh,wl,sh,sz,qaresult,cd,ufd,mfd,qty,uom,wz,opdesc,aqty,uwidth,Status,StockRoom,pgmn,PHISM,SuppBatch,FUser,CreateDate,InspectedFlag,OrigQTY,PARENTWO,PONO,POLine,QCResult,QCAction,MOVR,OrderLine) '
              + 'values(:bh,:oper,:idno,:zdh,:wl,:sh,:sz,:qaresult,:cd,:ufd,:mfd,:qty,:uom,:wz,:opdesc,:aqty,:uwidth,:Status,:StockRoom,:pgmn,:PHISM,:SuppBatch,:FUser,:CreateDate,:InspectedFlag,:OrigQTY,:PARENTWO,:PONO,:POLine,:QCResult,:QCAction,:MOVR,:OrderLine)';
            ...
            Params.ParamByName('OrderLine').AsInteger := salesOrderLine;
            execute;
          end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ASQL := 'select a.*,b.idno,b.zdh,b.StockRoom,b.wl,b.Qty, b.status as BStatus,c.status as CStatus,c.erpsuccess,c.FERROR,'
          + ' b.status as IsCheck,a.bm1 as gatepass,b.opdesc,b.UFD,b.MFD,b.PHISM as SuppRef,b.wz as Location,b.ParentWO '
      ...
      begin
        ...
        ASQL := 'select Lot_ID as Idno,Project_NO as Prjno,SOP as sopno,Work_Order as zdh,IDO as bh,  b.UFD,b.MFD,b.status as BStatus,c.status as CStatus,c.erpsuccess,b.Mem,'
          + 'Issued_Date as rq,Company as cono,c.StockRoom,UID as Jlr,ISSUE_QTY as QTY,ITEM_CODE as wl,c.PHISM as SuppRef,c.Location,c.ParentWO,c.FERROR'
...
procedure TfrmIDO.cxGrid2DBTableView1Column9GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
var
  AStr, AStr1: string;
...
begin
  ...
  begin
    AStr1 := AStr[I];
    if AStr1 = '1' then
      //AText := AText + GetErrorCause(I, frmMain.LanguageID) + '; ';
      AText := AText + frmMain.GetLanguagesStr(3000+I, '', True) + '; ';
