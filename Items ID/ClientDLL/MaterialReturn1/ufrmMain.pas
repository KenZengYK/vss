...
var
  bAdd: boolean;
  ABatchNo: string;
  ALotNo, ACono, ARETURN_NO, AITEM_CODE, AWORK_ORDER, ASTOCKROOM, AUOM, ALocation: string;
  //  AStr :PAnsiChar ;
begin
  ...
  case EMR of //
    ...
    emrAdd:
      begin
        ...
        ClientDataSet1.FieldByName('STATUS').AsInteger := Ord(bcsNew);
        ClientDataSet1.FieldByName('RECORD_CREATED_DATE').AsDateTime := Now;
...
//            ShowMessage(GetLanguagesStr(45, 'Please Enter STOCKROOM'))
          ...
          begin
            ...
            //if not (ValiateLocation(ACono, ALocation)) then
            //begin
...
begin
  ...
  begin
    ...
    if bAdd then
    begin
      sSQL := 'Insert into PB_RETURN (RETURN_NO,ITEM_CODE,WORK_ORDER,LOT_ID,RETURN_QTY,UOM,RETURN_DATE,STOCKROOM,COMPANY,STATUS,Location,RECORD_CREATED_DATE,ErpSuccess)'
        + ' Values(:RETURN_NO,:ITEM_CODE,:WORK_ORDER,:LOT_ID,:RETURN_QTY,:UOM,:RETURN_DATE,:STOCKROOM,:COMPANY,:STATUS,:Location,:RECORD_CREATED_DATE,:ErpSuccess)'
    end
    else
    begin
      sSQL := 'update PB_RETURN set RETURN_NO=:RETURN_NO,ITEM_CODE=:ITEM_CODE,'
        ...
        + 'STOCKROOM=:STOCKROOM,COMPANY=:COMPANY,STATUS=:STATUS,Location=:Location,ErpSuccess=:ErpSuccess';
...
begin
  ...
  begin
    ...
    begin
      ...
      ASQL := 'select *,' + ACase + ' from PB_RETURN';
...
var
  Qtyed: double;
  ACount: Integer;
  ASQL, ASQL1, ASQL2, IDO, Location1: string;
begin
  result := False;
  ASQL1 := 'select * from PB_GatePass where LOT_ID=' + QuotedStr(ALotNo) + ' and Company=' + QuotedStr(ACono) + ' and (ERPSuccess=1 and status=1)';
  ASQL2 := 'select * from PB_GATEPASS_Unplanned where LOT_ID=' + QuotedStr(ALotNo) + ' and Company=' + QuotedStr(ACono) + ' and (ERPSuccess=1 and status=1)';
  ...
  begin
    ...
    if (not isHandheld) and (ACount > 1) then
      if not (DoSelectData(ClientDataSet_Temp)) then exit;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FieldByName('RECORD_CREATED_DATE').AsDateTime := Date;
            FieldByName('RETURN_DATE').AsDateTime := Date;
