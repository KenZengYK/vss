...
uses
  ...
  cxCurrencyEdit, cxLookAndFeelPainters, ComCtrls, RzButton, Menus, cxDateUtils,
...
type
  TfrmIDO = class(TForm)
    ...
    mnWorkOrder: TMenuItem;
    mnMaterial: TMenuItem;
    mnColor: TMenuItem;
    mnColorShade: TMenuItem;
    mnOperDesc: TMenuItem;
    ...
    Panel6: TPanel;
    Label16: TLabel;
    btnReserveOK: TcxButton;
    editReserveIDO: TcxTextEdit;
    btnReserveExcel: TcxButton;
    Label10: TLabel;
    Label11: TLabel;
    editReserveDateFrom: TcxDateEdit;
    Label12: TLabel;
    editReserveDateTo: TcxDateEdit;
    cxGrid5: TcxGrid;
    ...
    procedure btnReserveOKClick(Sender: TObject);
    procedure btnReserveExcelClick(Sender: TObject);
    procedure cxGridDBColumn14GetDisplayText(
      Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
      var AText: string);
    procedure cxGridDBColumn14GetFilterDisplayText(
      Sender: TcxCustomGridTableItem; const AValue: Variant;
      var ADisplayText: string);
    procedure cxGridDBColumn16GetDisplayText(
      Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
      var AText: string);
    procedure cxGridDBColumn16GetFilterDisplayText(
      Sender: TcxCustomGridTableItem; const AValue: Variant;
      var ADisplayText: string);
  private
    ...
    procedure DoSelectByMenu;
    procedure CreateSubMenu(ParentMenu: TMenuItem; FieldName: string);
    procedure SelectByClick(Sender: TObject);
    function GetSelectByFieldName(ATag: Integer): string;
    procedure SubMenuSelect(FieldName: string; AValue: string);
  ...
  public
    ...
    constructor Create(Sender: TObject; isUPI: boolean); overload;
  end;

var
  frmIDO: TfrmIDO;
function DoSelectMaterial(ACONO, AProjectNo, AWorder: string; isAdmin, isUPI: boolean): boolean;
function DoIDOQuery(bIDO: boolean): boolean;
function DoSplitIDList(): boolean;
function DoIDOMatching: boolean;
function DoIDOReserveQuery(): boolean;
...
function DoSelectMaterial(ACONO, AProjectNo, AWorder: string; isAdmin, isUPI: boolean): boolean; //bIDO2Confirm: boolean;
begin
  result := False;

  frmIDO := TfrmIDO.Create(nil);
  //frmIDO := TfrmIDO.Create(nil, isUPI);
  with frmIDO do
  begin
    try
      fIsUPI := isUPI;
      Caption := frmMain.GetLanguagesStr(24, 'IDO') + '-' + frmMain.GetLanguagesStr(29, 'Select Material');
      //+ frmMain.GetLanguagesStr(89, '(Tips: the Record where BackGround Color is Red Cannot Selected)');
      cxPageControl.ActivePageIndex := 0;
      cxGridPopupMenu1.Grid := frmIDO.cxGrid1;
      FCONO := ACONO;
...
begin
  ...
  begin
    try
      ...
      Caption := frmMain.GetLanguagesStr(24, 'IDO') + '-' + frmMain.GetLanguagesStr(30, 'Query');
      cxPageControl.ActivePageIndex := 1;
      cxGridPopupMenu1.Grid := frmIDO.cxGrid2;
      if ShowModal = IDOK then
      begin
        result := True;
      end;
...
begin
  ...
  begin
    try
      //fIsUPI := isUPI;
      Caption := frmMain.FsReserveStatus + '-' + frmMain.GetLanguagesStr(30, 'Query');
      cxPageControl.ActivePageIndex := 3;
      cxGridPopupMenu1.Grid := frmIDO.cxGrid5;
      if ShowModal = IDOK then
      begin
        result := True;
      end;
...
begin
  ...
  begin
    try
      ...
      cxPageControl.ActivePageIndex := 1;
      cxGridPopupMenu1.Grid := frmIDO.cxGrid4;
      if ShowModal = IDOK then
      begin
        result := True;
      end;
...
begin
  ...
  begin
    try
      Caption := frmMain.GetLanguagesStr(24, 'IDO') + '-' + frmMain.GetLanguagesStr(90, 'Split ID List');
...
procedure TfrmIDO.SelectIDOData;
  ...
  begin
    ...
    begin
     ...
     // begin
      ...
      commandtext := 'select Count(*) from ido1' + fSuffixTable + ' a,ido2' + fSuffixTable + ' b'
        ...
        + ' and not exists (select * from PB_GATEPASS' + fSuffixTable + ' C where ERPSuccess=1 and B.BH=C.IDO AND B.IDNO=C.LOT_ID'
      ...
      // end; select Count(*) from ido2 where bh<>:bh and sop=:sop and lotid=:lotid and zdh=:zdh
      ...
      begin
        ...
        //        params.createparam(ftString, 'BH', ptinput);
        //        params.createparam(ftString, 'BH1', ptinput);
        //        commandtext := 'select sum(qty) as tqty from ido2 a'
        //          + ' where  zdh=:zdh and idno=:idno'
        //          + ' and ((bh<>:bh) or (bh=:BH1 and status=' + IntToStr(Ord(bcsIDOAffirm)) + '))'
        //          + ' and NOT EXISTS(select * from PB_GatePass b'
  ...
  begin
    ...
    begin
      ...
      commandtext := 'select a.* from ido1' + fSuffixTable + ' a,ido2' + fSuffixTable + ' b where a.bh=b.bh and a.bh<>:bh and b.idno=:idno and a.sopno<>:sopno';
...
begin
  ...
  try
    ...
    ASQL := 'Call ZPHLIB_AUL.sp_IDOSelectMaterial' + fSuffixTable + '_New (' + IntToStr(DT) + ',' + QuotedStr(TM) + ','
  ...
  begin
    Screen.Cursor := crSQLWait;
    DisableControls;
    //ASQL := 'select a.*,SubString(CH2T84,1,7) as PONO,SubString(CH2T84,8,3) as POLine  from ZPHLIB_AUL.IDOUPI a where DT=' + IntToStr(DT) + ' and TM=' + QuotedStr(TM);
    ASQL :=
      ' select '
      + 'a.SSG_ProjectNum as PRJNO '
      + ',a.ProdId as WORDER '
      + ',a.SalesId as SOPNO '
      + ','''' as OPERDESC --,a.dispOperateDesc() as '  + char(13)
      + ',10 as OPER --,a.dispOperateDesc() as '   + char(13)
      + ',a.InventLocationId as StockRoom '
      + '--,a.dispSKU() as MCODE' + char(13)
      + ',b.ExternalItemId as PHISM '
      + ',a.ItemId as ItemCode '
      + ',a.InventColorId as ColorCode '
      + ',a.InventSizeId as SizeCode '
      + ',a.configId as CupCode'
      + ',a.SSG_VatDyeId as CH0T84 '
      + ',a.inventBatchId as IDNO '
      + ',a.LockedQty as QTY_ALLOC'
      + ',a.IssuedQty as QTY'
      + ',a.OriginalQty as OrigQTY'
      + ',a.UnitId as iunt35 '
      + ',a.ColorGroup '
      + ',a.ColorShade as ch4t84'
      + ','''' as InspectedFlag --,a.dispInspectedFlag() as '   + char(13)
      + ',a.Useful as ch8v84'
      + ',a.CollectRefProdId as PARENTWO '
      + ',a.ActualWidth as ch5t84'
      + ',a.MarkerWidth as spc235'
      + ',a.wMSLocationId as ch9t84 '
      + ',a.SSG_MaterialTypeId as MaterialType '
      + ',a.PurchId as PONO '
      + ',a.POLineNum as POLine '
      + ',a.QcAction as ch7t84'
      + ',a.QcResult as ch6t84'
      + ',case when c.SSG_ReserveItemType = 0 then ''PI'' else '''' end as MOVR  '
      + ',1 as IsCheck'
      + ',CAST('' AS VARCHAR(50)) as LOCKBH'
      + ',CAST('' AS VARCHAR(10)) as PGMN35'
      + ' from [AXERP].[AXERPDB].dbo.SSG_LockedMaterialView a '
      + ' left join  [AXERP].[AXERPDB].dbo.CustVendExternalItem b '
      + '   on   b.ItemId           = a.ItemId '
      + '   and  b.ModuleType       = 3 '
      + '   and  b.CustVendRelation = a.VendAccount '
      + ' inner join  [AXERP].[AXERPDB].dbo.SSG_ReserveTable c '
      + '   on c.RecId = a.ReserveTableRecId'
      + ' where ('+QuotedStr(FProjectNo)+'='''' or  a.SSG_ProjectNum=' + QuotedStr(FProjectNo) + ')'
      + ' and (' + QuotedStr(FWorder) + '='''' or a.ProdId=' + QuotedStr(FWorder) + ')'
      ;
...
procedure TfrmIDO.DoSelectByMenu;
begin
  //1. Clear all subMenus
  mnWorkOrder.Clear;
  mnMaterial.Clear;
  mnColor.Clear;
  mnColorShade.Clear;
  ...
  CreateSubMenu(mnWorkOrder, 'WORDER');
  CreateSubMenu(mnMaterial, 'MaterialCode');
  CreateSubMenu(mnColor, 'ColorCode');
  CreateSubMenu(mnColorShade, 'ch4t84');
  CreateSubMenu(mnOperDesc, 'Operdesc');
end;

procedure TfrmIDO.CreateSubMenu(ParentMenu: TMenuItem; FieldName: string);
var
  I: Integer;
  ASubMenu: TMenuItem;
  AStr: string;
...
begin
  ...
  begin //with
    ...
    try
      ...
      while not eof do
      begin //while
        AStr := Trim(FieldByName(FieldName).AsString);
        if (AList.IndexOf(AStr) = -1) then
          AList.Add(AStr);
      ...
      begin
        ASubMenu := TMenuItem.Create(PopupMenu1);
        ASubMenu.Caption := AList.Strings[I];
        ASubMenu.Tag := ParentMenu.Tag;
        ASubMenu.OnClick := SelectByClick;
        ParentMenu.Add(ASubMenu);
      end;
      ParentMenu.Enabled := ParentMenu.Count > 0;
    finally
      FreeAndNil(AList);
      EnableControls;
    end;
...
procedure TfrmIDO.SelectByClick(Sender: TObject);
var
  ASubMenu: TMenuItem;
  ATag: Integer;
  AStr: string;
begin
  ASubMenu := Sender as TMenuItem;
  ATag := ASubMenu.Tag;
  AStr := ASubMenu.Caption;
  SubMenuSelect(GetSelectByFieldName(ATag), AStr);
end;

function TfrmIDO.GetSelectByFieldName(ATag: Integer): string;
begin
  case ATag of
    1: result := 'WORDER';
    2: result := 'MaterialCode';
    3: result := 'ColorCode';
    4: result := 'ch4t84';
    5: result := 'Operdesc';
  else
...
procedure TfrmIDO.SubMenuSelect(FieldName: string; AValue: string);
var
  oper: Integer;
  idno, AStr: string;
begin
  ...
  begin
    ...
    try
      ...
      begin
        AStr := Trim(FieldByName(FieldName).AsString);
        if (FieldByName('LOCKBH').AsString = '') and (FieldByName('IsCheck').AsInteger <> -1) and (AStr = AValue) then
        begin
          Edit;
          FieldByName('IsCheck').AsInteger := -1;
          Post;
...
begin
  ...
  ClientDataSet1.RemoteServer := frmMain.SCN;
...
begin
  ...
  cxGrid1DBTableView1Column4.Caption := frmMain.GetLanguagesStr(56, 'Actual Width');
  cxGrid1DBTableView1Column5.Caption := frmMain.GetLanguagesStr(57, 'Mark Width');
  cxGrid2DBTableView1Column1.Caption := frmMain.GetLanguagesStr(10, 'GATEPASS');
  cxGrid2DBTableView1Column2.Caption := frmMain.GetLanguagesStr(41, 'Opdesc');
  cxGrid1DBTableView1OrigQTY.Caption := frmMain.GetLanguagesStr(92, 'Orig QTY', True);
  cxGrid2DBTableView1Column3.Caption := frmMain.GetLanguagesStr(56, 'Actual Width');
  cxGrid2DBTableView1Column4.Caption := frmMain.GetLanguagesStr(57, 'Mark Width');

  cxGrid2DBTableView1Column7.Caption := frmMain.GetLanguagesStr(93, 'Parent WO');
  cxGrid1DBTableView1Column6.Caption := frmMain.GetLanguagesStr(93, 'Parent WO');

  mnWorkOrder.Caption := cxGrid1DBTableView1DBColumn1.Caption;
  mnMaterial.Caption := cxGrid1DBTableView1DBColumn4.Caption;
  mnColor.Caption := frmMain.GetLanguagesStr(52, 'Color');
  mnOperDesc.Caption := cxGrid1DBTableView1DBColumn3.Caption;
  mnColorShade.Caption := frmMain.GetLanguagesStr(39, 'Color Shade');
  //cxGrid1DBTableView1Column2.Caption := cxGrid1DBTableView1DBColumn4.Caption;
  //cxGrid1DBTableView1Column3.Caption := mnColor.Caption;
  ...
  if (self.fIsUPI) then
    cxGrid2DBTableView1DBColumn4.Caption := frmMain.GetLanguagesStr(6, 'UPI No.')
  else
    cxGrid2DBTableView1DBColumn4.Caption := frmMain.GetLanguagesStr(59, 'IDO');
  ...
  cxGridDBColumn14.Caption := frmMain.FsReserveStatus;
...
procedure TfrmIDO.btnSelectClick(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      params.createparam(ftString, 'BH', ptinput);
      params.createparam(ftString, 'BH1', ptinput);
      //      commandtext := 'select sum(qty) as tqty from ido2 '
      //        + ' where  zdh=:zdh and idno=:idno and ((bh<>:bh) or (bh=:BH1 and status=' + IntToStr(Ord(bcsIDOAffirm)) + '))';
      commandtext := 'select sum(qty) as tqty from ido2' + fSuffixTable + ' a'
        + ' where  zdh=:zdh and idno=:idno'
        + ' and ((bh<>:bh) or (bh=:BH1 and status=' + IntToStr(Ord(bcsIDOAffirm)) + '))'
        + ' and NOT EXISTS(select * from PB_GatePass' + fSuffixTable + ' b'
        + '   where work_order=zdh and Lot_ID=idno'
        + ' and Item_code=wl and b.stockroom=a.stockroom and ERPSuccess=1 and COMPANY=' + QuotedStr(FCONO) + ')';
...
var
  //  oper: Integer;
  BH, idno, pgmn, worder, pwo, pono, MOVR: string;
  qaresult, qCAction, ufd, mfd, ColorShade, wz, uwidth, uom, AStockRoom, PHISM, SuppBatch: string;
  cd, AQty: double;
  ItemCode,ColorCode,SizeCode,CupCode,SKU:string;
  NotSuccessStr, InspectedFlag: string;
  poline: integer;
  isSC: boolean;
begin
  NotSuccessStr := '';
  ...
  begin
    ...
    try
      ...
      Filtered := True;
      BH := frmMain.ClientDataSet_ido1.fieldbyname('BH').AsString;
      isSC := frmMain.ClientDataSet_ido1.fieldbyname('isSC').AsBoolean;
      //delete current ido detail(table ido2)of 'BH'
      ...
      begin
        ...
        { with ClientDataSet_Temp do
         begin
           ...
           params.clear;
           params.createparam(ftString, 'bh', ptinput);
           commandtext := 'delete from ido2 where bh=:bh  and status<>' + IntToStr(Ord(bcsIDOAffirm));
           params[0].AsString := BH;
           execute;
         end;  }
      end;
      ...
      begin
        ...
        ItemCode:= ClientDataSet1.fieldbyname('ItemCode').AsString;
        ColorCode:= ClientDataSet1.fieldbyname('ColorCode').AsString;
        SizeCode:= ClientDataSet1.fieldbyname('SizeCode').AsString;
        CupCode:= ClientDataSet1.fieldbyname('CupCode').AsString;
        SKU:= trim(ItemCode)+trim(ColorCode)+trim(SizeCode)+trim(CupCode);
        ...
        qCAction := fieldbyname('ch7t84').AsString; //pass  15
        ufd := fieldbyname('ch5t84').AsString; //actual width  15
        ColorShade := Trim(fieldbyname('ch4t84').AsString); //color shade 15
        wz := fieldbyname('ch9t84').AsString; //location 15
        mfd := fieldbyname('spc235').AsString;
        uom := fieldbyname('iunt35').AsString; //2
        pgmn := Copy(ItemCode, 1, 3); //  fieldbyname('pgmn35').AsString; //3
        ...
        begin
          ...
          poline := StrToIntDef(fieldbyname('POLine').AsString, 0);
        end;
        ...
        begin
          ...
          begin
            ...
            params.createparam(ftString, 'ColorCode', ptinput);
            params.createparam(ftString, 'SizeCode', ptinput);
            params.createparam(ftString, 'CupCode', ptinput);
            ...
            params.createparam(ftString, 'QCResult', ptinput);
            params.createparam(ftString, 'QCAction', ptinput);
            params.createparam(ftString, 'MOVR', ptinput);
            commandtext := 'insert into ido2' + fSuffixTable + '(bh,oper,idno,zdh,wl,ItemCode,ColorCode,SizeCode,CupCode'+',sh,sz,qaresult,cd,ufd,mfd,qty,uom,wz,opdesc,aqty,uwidth,Status,StockRoom,pgmn,PHISM,SuppBatch,FUser,CreateDate,InspectedFlag,OrigQTY,PARENTWO,PONO,POLine,QCResult,QCAction,MOVR) '
              + 'values(:bh,:oper,:idno,:zdh,:wl,:ItemCode,:ColorCode,:SizeCode,:CupCode,'+':sh,:sz,:qaresult,:cd,:ufd,:mfd,:qty,:uom,:wz,:opdesc,:aqty,:uwidth,:Status,:StockRoom,:pgmn,:PHISM,:SuppBatch,:FUser,:CreateDate,:InspectedFlag,:OrigQTY,:PARENTWO,:PONO,:POLine,:QCResult,:QCAction,:MOVR)';
            ...
            Params.ParamByName('ItemCode').AsString := ItemCode;
            Params.ParamByName('ColorCode').AsString := ColorCode;
            Params.ParamByName('SizeCode').AsString := SizeCode;
            Params.ParamByName('CupCode').AsString := CupCode;
            Params.ParamByName('sh').AsString :=ColorCode;// ClientDataSet1.fieldbyname('ColorCode').value;
            ...
            if (poline < 100) then
              Params.ParamByName('POLine').AsInteger := poline;
            Params.ParamByName('QCresult').AsString := qaresult;
            Params.ParamByName('QCAction').AsString := QCAction;
...
begin
  ...
  begin
    ...
    if (fIsUPI) then
      ...
      AIDO := GetIDOFullString(editIDO.Text, 'IDO');
    ...
    try
      ...
      if FQueryIDO then
      begin
        cxGrid2DBTableView1Column3.Visible := True;
        cxGrid2DBTableView1Column4.Visible := True;
        ASQL := 'select a.*,b.idno,b.zdh,b.StockRoom,b.wl,b.Qty, b.status as BStatus,c.status as CStatus,c.erpsuccess, b.status as IsCheck,a.bm1 as gatepass,b.opdesc,b.UFD,b.MFD,b.PHISM as SuppRef,b.wz as Location,b.ParentWO '
          + ' from ido1' + fSuffixTable + ' a inner join ido2' + fSuffixTable + ' b on (a.bh=b.bh)'
          + ' left join pb_gatepass' + fSuffixTable + ' c on (b.bh=c.ido and b.idno=c.lot_id and b.zdh=c.work_order and b.wl=c.item_code and a.cono=c.company and b.stockroom=c.stockroom)'
          + ' where ';
      ...
      begin
        cxGrid2DBTableView1Column3.Visible := False;
        cxGrid2DBTableView1Column4.Visible := False;
        //ASQL := 'select Lot_ID as Idno,Project_NO as Prjno,SOP as sopno,Work_Order as zdh,IDO as bh,'
        //  + 'Issued_Date as rq,Company as cono,StockRoom,UID as Jlr,ISSUE_QTY as QTY,ITEM_CODE as wl,GatePass,PHISM as SuppRef,Location,ParentWO '
        //  + ' from PB_GatePass' + fSuffixTable;
        ...
        ASQL := 'select Lot_ID as Idno,Project_NO as Prjno,SOP as sopno,Work_Order as zdh,IDO as bh,  b.UFD,b.MFD,b.status as BStatus,c.status as CStatus,c.erpsuccess,b.Mem,'
          ...
          + ' from PB_GATEPASS' + fSuffixTable + ' c'
          + ' left join (select b.*,a.cono,a.Mem from ido1' + fSuffixTable + ' a inner join ido2' + fSuffixTable + ' b on (a.bh=b.bh)) b'
...
begin
  ...
  begin
    ...
    try
      ...
      while not Eof do
      begin
        if ((FieldByName('LOCKBH').AsString = '') and (FieldByName('IsCheck').AsInteger <> bAll)) then
        begin
          Edit;
...
begin
  ...
  begin
    case AType of //
      1: result := 'select distinct b.idno from ido1' + fSuffixTable + ' a,ido2' + fSuffixTable + ' b where a.bh=b.bh order by b.idno';
      2: result := 'select distinct sopno,prjno from ido1' + fSuffixTable + '   order by sopno';
      3: result := 'select distinct Zdh as worder from ido2' + fSuffixTable + '   order by worder';
    end; // case
  end
  else
  begin
    case AType of //
      1: result := 'select distinct Lot_ID as idno from PB_GatePass' + fSuffixTable + ' order by idno';
      2: result := 'select distinct SOP as sopno,Project_NO as prjno from PB_GatePass' + fSuffixTable + '   order by sopno';
      3: result := 'select distinct Work_Order as worder from PB_GatePass' + fSuffixTable + '   order by worder';
    end; // case
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          IDNO := FieldByName('IDNO').AsString;
          Word := FieldByName('Zdh').AsString;
          with ClientDataSet1 do
          begin
            if (Locate('IDNO;Worder', varArrayOf([IDNO, Word]), []) and (FieldByName('LOCKBH').AsString = '')
              and (FieldByName('IsCheck').AsInteger <> -1)) then
            begin
              Edit;
              FieldByName('IsCheck').AsInteger := -1;
              Post;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Params.Clear;
            Params.CreateParam(ftString, 'idno', ptInput);
            CommandText := 'update ido2' + fSuffixTable + ' set idno=rTrim(idno)+''A'',FUser=isNull(FUser,'''')+' + QuotedStr('UnLock by ' + frmMain.UID) + ' where idno=:idno';
...
begin
  ...
  begin
    ...
    try
      ...
      ASQL := 'exec dbo.sp_PB_IDOMatching_Select' + fSuffixTable + ' '
        + QuotedStr(AIDO) + ','
        + QuotedStr(ALotNo) + ','
        + QuotedStr(ASopNo) + ','
        + QuotedStr(AWorderNo) + ','
...
begin
  ...
  begin
    ...
    ASQL := 'exec dbo.sp_PB_IDOMatching_Log' + fSuffixTable + ' '
      + QuotedStr(AIDO) + ','
      + QuotedStr(ALOTID) + ','
      + QuotedStr(ACONO) + ','
      + QuotedStr(AWord) + ','
...
begin
  ...
  begin
    ...
    ASQL := 'exec dbo.sp_PB_IDOMatching_Unlock' + fSuffixTable + ' '
      + QuotedStr(AIDO) + ','
      + QuotedStr(ALOTID) + ','
      + QuotedStr(ACONO) + ','
      + QuotedStr(AWord) + ','
...
procedure TfrmIDO.cxGrid1DBTableView1DataControllerSummaryFooterSummaryItemsSummary(
  ASender: TcxDataSummaryItems; Arguments: TcxSummaryEventArguments;
  var OutArguments: TcxSummaryEventOutArguments);
var
  AValue: Variant;
  isCheck: boolean;
begin
  AValue := cxGrid1DBTableView1.DataController.Values[Arguments.RecordIndex, cxGrid1DBTableView1DBColumn9.Index];
  if VarIsNull(AValue) then isCheck := false
  else isCheck := AValue;

  OutArguments.Done := not isCheck;
end;
...
procedure TfrmIDO.btnReserveOKClick(Sender: TObject);
var
  aIDO, aSQL, ASQLW, sFormDT, sToDT: string;
  dtFrom, dtTO: TDateTime;
begin
  AIDO := GetIDOFullString(editReserveIDO.Text, 'IDO');
  if AIDO <> '' then editReserveIDO.Text := AIDO;

  dtFrom := editReserveDateFrom.Date;
  dtTo := editReserveDateTo.Date;
  if (dtFrom = NullDate) then dtFrom := dtTo;
  if (dtTo = NullDate) then dtTo := dtFrom;
  sFormDT := '';
  sToDT := '';

  if (AIDO = '') and (dtFrom = NullDate) then exit;
  if (dtFrom <> NullDate) then sFormDT := FormatDateTime('yyyy''-''mm''-''dd', dtFrom) + ' 00:00:01';
  if (dtTo <> NullDate) then sToDT := FormatDateTime('yyyy''-''mm''-''dd', dtTo) + ' 23:59:59';

  //aSQL := 'select * from PB_GatePass_M';
  //ASQLW := '';
  //if (AIDO <> '') then ASQLW := ASQLW + 'and IDO=' + QuotedStr(AIDO);
  //if (dtFrom <> NullDate) then ASQLW := ASQLW + 'and ConfirmTime between ' + QuotedStr(FormatDateTime('yyyy''-''mm''-''dd', dtFrom) + ' 00:00:01')
  //  + ' and ' + QuotedStr(FormatDateTime('yyyy''-''mm''-''dd', dtTo) + ' 23:59:59');
  //ASQLW := Trim(ASQLW);
  //if ASQLW <> '' then ASQL := ASQL + ' where ' + Copy(ASQLW, 4, MaxInt);

  with ClientDataSet_Temp do
  begin
    //cxGridDBTableView2.OnCustomDrawCell := nil;
    //cxGridDBTableView2.OnFocusedRecordChanged := nil;
    cxGridDBTableView2.DataController.DataSource := nil;
    try
      close;
      Params.Clear;
      Params.CreateParam(ftString, '@IDO', ptInput);
      Params.CreateParam(ftString, '@fromDT', ptInput);
      Params.CreateParam(ftString, '@toDT', ptInput);
      commandtext := 'exec sp_pb_IDOReserveQuery :@IDO,:@fromDT,:@toDT';
      Params.ParamByName('@IDO').AsString := AIDO;
      Params.ParamByName('@fromDT').AsString := sFormDT;
      Params.ParamByName('@toDT').AsString := sToDT;
      open;

      btnReserveExcel.Enabled := True;

      //cxGridDBTableView2.OnCustomDrawCell := frmIDO.cxGridDBTableView_CustomDrawCell;
      //cxGridDBTableView2.OnFocusedRecordChanged := cxGrid2DBTableView1FocusedRecordChanged;

    finally
      screen.cursor := crDefault;
      cxGridDBTableView2.DataController.DataSource := DataSource_Temp;
    end;

  end;
end;

procedure TfrmIDO.btnReserveExcelClick(Sender: TObject);
begin
  cxGridSaveAs(cxGrid5, 0);
end;

procedure TfrmIDO.cxGridDBColumn14GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: string);
var
  AInt: Integer;
begin
  ...
  case AInt of //
    -1: AText := '';
    0: AText := frmMain.FsUnReserve;
    1: AText := frmMain.FsNeedReserve;
    2: AText := frmMain.FsConfirmReserve;
    3: AText := frmMain.FsCancelReserve;
  else
    AText := frmMain.FUnKnown;
  end; // case
end;

procedure TfrmIDO.cxGridDBColumn14GetFilterDisplayText(
  Sender: TcxCustomGridTableItem; const AValue: Variant;
  var ADisplayText: string);
var
  AInt: Integer;
begin
  ...
  AInt := AValue;
  case AInt of //
    0: ADisplayText := frmMain.FsUnReserve;
    1: ADisplayText := frmMain.FsNeedReserve;
    2: ADisplayText := frmMain.FsConfirmReserve;
    3: ADisplayText := frmMain.FsCancelReserve;
  end; // case
end;

procedure TfrmIDO.cxGridDBColumn16GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: string);
begin
  AText := UpperCase(AText);
...
procedure TfrmIDO.cxGridDBColumn16GetFilterDisplayText(
  Sender: TcxCustomGridTableItem; const AValue: Variant;
  var ADisplayText: string);
var
  bIssue: boolean;
...
constructor TfrmIDO.Create(Sender: TObject; isUPI: boolean);
begin
  //inherited Create;
  self.fIsUPI := isUPI;
end;
