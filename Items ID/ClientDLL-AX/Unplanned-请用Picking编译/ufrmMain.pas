...
type
  TfrmMain = class(TfrmBase)
    ...
    btnPrintUnplanned: TcxButton;
    cxGrid2DBTableView1MOVR: TcxGridDBColumn;
    cxGrid1DBTableView1MOVR: TcxGridDBColumn;
    ...
    procedure btnPrintUnplannedClick(Sender: TObject);
...
begin
  ...
  //    Close;
  //    Params.Clear;
  //    CommandText := 'select * from PB_GATEPASS_Unplanned   order by IDO';
  //    Open;
  //  end; // with
  ...
  begin
    ...
    if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
      CommandText := 'select * from ido1_Unplanned  order by bh'
    else
      CommandText := 'select * from ido1_Unplanned where jlr=' + QuotedStr(UID) + ' order by bh';
    Open;
    First;
  end; // with
...
begin
  ...
  btnIDOReport.Caption := 'UPI ' + btnReport.Caption + '...';
  ...
  cxGrid1DBTableView1IDO.Caption := GetLanguagesStr(6, 'UPI No.');
  cxGroupBox5.Caption := GetLanguagesStr(6, 'UPI No.');
  cxGrid3DBTableView1IDO.Caption := GetLanguagesStr(6, 'UPI No.');
  ...
  cxGrid1DBTableView1MOVR.Caption := GetLanguagesStr(91, 'Movement Reason', True);
  cxGrid2DBTableView1OrigQTY.Caption := GetLanguagesStr(92, 'Orig QTY', True);
  ...
  FIDOCompleted := GetLanguagesStr(116, 'UPI Completed', True);
  FUnKnown := GetLanguagesStr(105, 'UPI Completed', True);
  FProgressed := 'UPI ' + GetLanguagesStr(118, 'Progressed', True);
  ...
  FsIDOConfirmed := GetLanguagesStr(121, 'UPI Confirmed', True);
  ...
  TabSheet_IDO.Caption := '      ' + GetLanguagesStr(24, 'UPI') + '      ';
  ...
  btnDeleteIDO.Caption := GetLanguagesStr(32, 'Delete UPI');
  btnDeleteIDODetail.Caption := GetLanguagesStr(33, 'Delete UPI Detail');
  btnDeleteIDODetail.Hint := GetLanguagesStr(73, 'Delete UPI Detail of Current Datail');
  ...
  cxGrid2DBTableView1MOVR.Caption := GetLanguagesStr(91, 'Movement Reason', True);

  Label3.Caption := GetLanguagesStr(59, 'UPI');
...
begin
  ...
  begin
    Close;
    params.clear;
    CommandText := 'select STATUS,IDO,LOT_ID  from PB_GATEPASS_Unplanned where ' + (frmMain.FSelectIDONo)
...
begin
  ...
  //    Close;
  //    Params.Clear;
  //    CommandText := 'select distinct IDO from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ' order by IDO';
  //    Open;
  //  end; // with
  ...
  try
    ...
    begin
      ...
      CommandText := 'select a.IDO,b.ErpSuccess as bSelect from '
        + ' (select distinct IDO from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') a'
        + ' Left join (select IDO,ErpSuccess from PB_GATEPASS_Unplanned where 1=2) b'
...
begin
  ...
  begin
    ...
    CommandText := 'exec sp_pb_IssueUpdate_Unplanned :@IDO,:@LOT_ID,:@Status,:@ISSUED_DATE,:@State';
  ...
  try
    if not DataSet.fieldbyname('bh').IsNull then
      ...
      begin
        ...
        commandtext := 'select a.*,a.Qty*Cd/100 AS ActualQty from ido2_Unplanned a where a.bh=:bh';
...
begin
  ...
  begin
    close;
    params.clear;
    commandtext := 'select Count(*) from ido2_Unplanned'
...
begin
  ...
  begin
    Close;
    Params.Clear;
    //    CommandText := 'select distinct IDO from PB_GATEPASS_Unplanned where status=' + IntToStr(Ord(bcsFromHandHeld))
    //      + '  and IDO in (select  IDO from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ')' + '  order by IDO';
    CommandText := 'select distinct IDO from PB_GATEPASS_Unplanned where (status=' + IntToStr(Ord(bcsFromHandHeld))
      + ' or (status=' + IntToStr(Ord(bcsToSystem21)) + ' and ERPSuccess=0))'
      + '  and IDO in (select  IDO from PB_GATEPASS_Unplanned '
...
begin
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct IDO from PB_GATEPASS_Unplanned '
      + ' where ((status=' + IntToStr(Ord(bcsToSystem21)) + ' and ERPSuccess=0) '
      + ' or status<>' + IntToStr(Ord(bcsToSystem21)) + ')'
      //      + ' and IDO not in (select IDO from PB_GATEPASS_Unplanned where status=' + IntToStr(Ord(bcsToSystem21)) + ')'
    + ' and IDO in (select IDO from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ')' + '  order by IDO';
    Open;
  end; // with
  ...
  begin
    ...
    try
      ...
      begin
        ...
        CommandText := 'exec sp_pb_CancelPickingIDO_Unplanned :@BH,:@UID,:@DT';
      ...
      {
      //1.update ido2_Unplanned
      with ClientDataSet_IDO2_Temp do
      begin
        Close;
        Params.Clear;
        CommandText := 'Update  ido2_Unplanned set status=' + IntToStr(Ord(bcsNew))
      ...
      //2.update ido1_Unplanned
      with ClientDataSet_IDO2_Temp do
      begin
        Close;
        Params.Clear;
        CommandText := 'Update ido1_Unplanned set ydc=0 where bh=' + QuotedStr(AIDONo);
        Execute;
      end; // with
      //3.delete Issue
      ...
      begin
        Close;
        params.clear;
        CommandText := 'delete from PB_GATEPASS_Unplanned where IDO=' + QuotedStr(AIDONo)
      ...
      ClientDataSet1.Close;
      ClientDataSet1.Open;
      //4.if find,then update ido1_Unplanned.ydc and refresh ido2_Unplanned
      with ClientDataSet_ido1 do
      begin
...
begin
  ...
  try
    ...
    begin
      Close;
      Params.Clear;
      CommandText := 'select distinct IDO,company,ITEM_CODE,LOT_ID from PB_GATEPASS '
        + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ' and (ERPSuccess=0 or  ERPSuccess is null)';
      Open;
...
begin
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct status from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ' order by status';
    Open;
  end; // with
    ...
    begin
      ...
      begin
        ...
        if APrintType = 0 then
          CommandText := 'select *,' + ACase + ' from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True)
        else if APrintType = 1 then
          CommandText := 'select *,' + ACase + ' from (select * from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') a where  status=' + AStatus
        else //if APrintType=2 then
          CommandText := 'select *,' + ACase + ' from (select * from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') a where  ERPSuccess=1';
        Open;
        if RecordCount = 0 then
        begin
          Application.MessageBox(PChar(GetLanguagesStr(44, 'No Data', True)),
...
//        end; // while
      ...
      if APrintType = 0 then
        ASQL := 'select IDO from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True)
      else if APrintType = 1 then
        ASQL := 'select IDO from (select * from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') a where  status=' + AStatus
      else //if APrintType=2 then
        ASQL := 'select IDO from (select * from PB_GATEPASS_Unplanned  ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') a where  ERPSuccess=1';

      with ClientDataSet_IDO1_Temp do
      begin
        Close;
        Params.Clear;
        CommandText := 'select a.Site,a.rq,a.shr,a.jlr,a.bm,a.bh from ido1_Unplanned a where a.bh in (' + ASQL + ')';
        Open;
      end; // with
...
begin
  ...
  begin
    ...
    try
      ...
      AIDO := GetMaxIDO;
      Append;
      fieldbyname('cono').value := 'P1'; // uppercase(cxTextEdit1.text);
...
begin //IDO-0000021
  ...
  begin
    close;
    params.clear;
    commandtext := 'exec sp_GetMaxIDO_Unplanned'; //'select IDO from ido_max';
    open;
...
begin
  ...
  begin
    ...
    begin
      ...
      CommandText := ' Insert into  ido1_Unplanned (cono,bh,rq,sj,site,jlr,ydc) values(:cono,:bh,:rq,:sj,:site,:jlr,:ydc)';
  ...
  begin
    ...
    begin
      ...
      params.createparam(ftBoolean, 'ydc', ptinput);
      params.createparam(ftString, 'bh', ptinput);  }
      ASQL := 'Update ido1_Unplanned set';
...
begin
  ...
  begin
    ...
    begin
      ...
      commandtext := 'update ido2_Unplanned set qty=:qty,Status=:Status,PHISM=:PHISM where bh=:bh and idno=:idno and zdh=:zdh';
    ...
    begin
      ...
      params.createparam(ftInteger, 'Status', ptinput);
      commandtext := 'insert into ido2_Unplanned(bh,oper,idno,zdh,wl,sh,sz,qaresult,cd,ufd,mfd,qty,uom,wz,opdesc,aqty,StockRoom,Status) '
...
begin
  ...
  begin
    Close;
    Params.Clear;
    CommandText := 'select distinct b.zdh,a.cono from ido1_Unplanned a, ido2_Unplanned b where a.bh=b.bh and a.bh=' + QuotedStr(IDO)
...
begin
  ...
  try
    ...
    else if (Trim(ClientDataSet_IDO1.fieldbyname('Mem').AsString) = '') then
    begin
      Application.MessageBox(PChar(GetLanguagesStr(96, 'Notepad Can not Empty!')),
        PChar(GetLanguagesStr(5, 'Tips', True)), MB_OK + MB_ICONInformation);
      ...
      begin
        ...
        begin
          ...
          try
            ...
            FilterIssueData(BH);
            //5.change ido2_Unplanned status
            with ClientDataSet_IDO2 do
            begin
              First;
              while not eof do
            ...
            //6.set ido1_Unplanned ydc is true
            with ClientDataSet_IDO1 do
            begin
...
begin
  ...
  {  with ClientDataSet_IDO2_Temp do
    begin
      ...
      params.clear;
      params.createparam(ftString, 'bh', ptinput);
      commandtext := 'delete from ido2_Unplanned where bh=:bh';
  ...
  begin
    ...
    params.clear;
    params.createparam(ftString, 'bh', ptinput);
    commandtext := 'delete from ido1_Unplanned where bh=:bh and (ydc is null or ydc=0)';
    params[0].asstring := ClientDataSet_IDO1.fieldbyname('bh').value;
    execute;
  end;
  try
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        params.createparam(ftString, 'Zdh', ptinput);
        params.createparam(ftString, 'wl', ptinput);
        commandtext := 'delete from ido2_Unplanned where bh=:bh  and idno=:idno and Zdh=:Zdh and wl=:wl and status<>' + IntToStr(Ord(bcsIDOAffirm));
        params[0].asstring := ClientDataSet_IDO2.fieldbyname('bh').value;
...
begin
  ...
  begin
    ...
    try
      ...
      commandtext := 'select a.*,b.*,a.cono as COMPANY,b.idno as LOT_ID,Qty as CurQty,Stockroom as Split,b.wl as ITEM_CODE'
        + '  from ido1_Unplanned a,ido2_Unplanned b where b.bh=a.bh and a.bh=:bh order by b.wl,b.oper,b.idno';
  ...
  begin
    ...
    Dictionary.Variables.AsString['FOrigQTY'] := GetLanguagesStr(92, 'OrigQTY', True);
...
begin
  ...
  try
    ...
    begin
      ...
      Params.Clear;
      if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
        CommandText := 'select distinct Bh from ido1_Unplanned where cono=' + QuotedStr(UpperCase(cxTextEdit1.Text)) + ' order by bh'
      else
        CommandText := 'select distinct Bh from ido1_Unplanned where cono=' + QuotedStr(UpperCase(cxTextEdit1.Text)) + 'and jlr=' + QuotedStr(UID) + ' order by bh';
...
procedure TfrmMain.GetUPIUserRights;
var
  FID: Integer;
begin
  FIDOUserRight := [];
...
begin
  ...
  try
    ...
    begin
      ...
      Params.Clear;
      CommandText := 'select a.IDO,b.ErpSuccess as bSelect from '
        + ' (select distinct IDO from PB_GATEPASS_Unplanned) a Left join (select IDO,ErpSuccess from PB_GATEPASS_Unplanned where 1=2) b'
        + ' on (a.IDO=b.IDO) order by a.IDO';
      Open;
    end; // with
    cxGrid3DBTableView1.DataController.DataSource := DataSource2;
...
begin
  ...
  begin
    ...
    try
      ...
      //2.show data
      Close;
      aSQL := 'SELECT * FROM PB_GATEPASS_Unplanned';
      aSQL := aSQL + ' ' + sWhere;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          SQL.Add('and b.StockRoom=istri3 and b.worder=wordi3 and b.FDate=trdti3 and a.ISSFI3=''1''');
    ...
    begin
      ...
      try
        ...
        begin
          Close;
          Params.Clear;
          CommandText := 'select distinct COMPANY,LOT_ID,LOT_ID_Sub,ITEM_CODE,STOCKROOM from PB_GATEPASS_Unplanned '
...
begin
  ...
  begin
    ...
    CommandText := 'exec sp_pb_Update_ErpSuccess :@COMPANY,:@WORK_ORDER,:@DATE,:@Stockroom,:@ITEM_CODE,:@LOT_ID,:@ErpSuccess,:@Type,:@IDO,:@FERROR';
    ...
    Params.ParamByName('@Type').AsInteger := 3;
    Params.ParamByName('@IDO').AsString := IDO;
...
begin
  ...
  begin
    ...
    CommandText := 'exec sp_pb_Update_IssueSubLotNo_Unplanned :@COMPANY,:@DATE,:@Stockroom,:@ITEM_CODE,:@LOT_ID,:@LOT_ID_Sub';
    Params.ParamByName('@COMPANY').AsString := conoi;
...
begin
  ...
  begin
    ...
    //        + ' (select * from PB_GATEPASS_Unplanned ' + bcGetWhereSQL(ClientDataSet1.CommandText, 'IDO', '', True) + ') b'
...
begin
  ...
  begin
    ...
    Params.CreateParam(ftString, 'STOCKROOM', ptInput);
    Params.CreateParam(ftString, 'Zdh', ptInput);
    CommandText := 'select mfd from IDO2 '
...
begin
  ...
  try
    ...
    begin
      Close;
      Params.Clear;
      CommandText := 'select BH AS IDO from ido1_Unplanned order by BH';
      Open;
    end; // with
    cxGrid3DBTableView1.DataController.DataSource := DataSource2;
  finally // wrap up
...
procedure TfrmMain.btnChangToYellowClick(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      Params.CreateParam(ftString, '@IDO', ptInput);
      Params.CreateParam(ftString, '@LOT_ID', ptInput);
      CommandText := 'update PB_GATEPASS_Unplanned set Status=:@Status, ISSUED_DATE=:@ISSUED_DATE,ERPSuccess=null where IDO=:@IDO  and LOT_ID=:@LOT_ID';
      Params.ParamByName('@Status').AsInteger := ClientDataSet1.FieldByName('Status').AsInteger;
...
begin
  ...
  try
    ...
    //      Params.Clear;
    //      if (urAdmin in FIDOUserRight) or (urBrowseAllIDO in FIDOUserRight) then
    //        CommandText := 'select distinct Bh from ido1_Unplanned '
    //      else
    //        CommandText := 'select distinct Bh from ido1_Unplanned where  jlr=' + QuotedStr(UID) + ' order by bh';
    //
    //      Open;
    //    end; // with
...
procedure TfrmMain.btnPrintUnplannedClick(Sender: TObject);
var
  AStartDate, AEndDate: TDate;
  aCURR_QTY, IssueQty: Double;
  LOT_ID, ItemCode, CompanyNo, StockRoom, ASQL: string;
  bHaveCurrQty: boolean;
begin
  inherited;
  if not CheckUserRight(cUrReport, TButton(Sender).Caption, FIDOUserRight) then exit;
  if (not DoSelectPrintDate(AStartDate, AEndDate)) then exit;
  ...
  begin
    ...
    {ASQL := 'select a.Bh,a.Rq,a.Sj,a.Bm,a.Bm1,a.Shr,a.Jlr,a.Sopno,a.Bmscx,a.Worder,a.Cono,a.Prjno,a.Site,a.ConfirmTime,'
      + 'cast(a.Mem as Varchar(1000)) as Notepad,'
      + 'case when a.Ydc is null or a.Ydc=0 then ' + QuotedStr(FsNew) + ' else ' + QuotedStr(FIDOCompleted) + ' end as YDC'
      + ' from ido1_Unplanned a'
      + ' where a.RQ between :Date1 and :Date2'
      + ' and exists (select * from ido2_Unplanned b where a.BH=b.BH) order by a.BH';
     }
    ASQL := 'select a.Bh,a.Rq,a.Sj,a.Bm,a.Bm1,a.Shr,a.Jlr,a.Sopno,a.Bmscx,a.Worder,a.Cono,a.Prjno,a.Site,a.ConfirmTime,'
      + 'cast(a.Mem as Varchar(1000)) as Notepad,'
      + 'case when a.Ydc is null or a.Ydc=0 then ' + QuotedStr(FsNew) + ' else ' + QuotedStr(FIDOCompleted) + ' end as YDC,'
      + 'b.Oper,b.Idno,b.Zdh,b.Wl,b.Uwidth,b.StockRoom,b.Sh,b.Sz,b.Qaresult,b.Cd,b.Ufd,b.Mfd,b.Qty,b.Uom,b.pgmn,b.Wz,b.Opdesc,'
      + 'b.Sy,b.Gh,b.Aqty,b.Status,b.MOVR,b.PHISM,b.SuppBatch,b.FUser,b.CreateDate,b.InspectedFlag'
      + ' from ido1_Unplanned a,ido2_Unplanned b'
      + ' where a.RQ between :Date1 and :Date2 and a.bh=b.bh'
      //+ ' and exists (select * from ido2_Unplanned b where a.BH=b.BH)'
    + ' order by a.BH,b.wl,b.oper,b.idno';
    commandtext := ASQL;
    params[0].AsDate := AStartDate;
    params[1].AsDate := AEndDate;
    open;
  end;

  with RMGridReport1 do
  begin
    //LoadFromFile(ExtractFilePath(Application.ExeName) + 'UPINotepad.rls');
    LoadFromResourceName(hInstance, 'UPINotepad');
    Dictionary.Clear;
    Dictionary.Variables.Insert(0, ' Global');
    Dictionary.Variables.AsString['FReportTitle'] := GetLanguagesStr(95, 'UPI Notepad Listing Report');
    Dictionary.Variables.AsString['FDate'] := GetLanguagesStr(23, 'Date', True);
    Dictionary.Variables.AsString['FTime'] := GetLanguagesStr(38, 'Time', True);
    Dictionary.Variables.AsString['FPage'] := GetLanguagesStr(47, 'Page', True);
    Dictionary.Variables.AsString['FStatus'] := GetLanguagesStr(12, 'Status', True);
