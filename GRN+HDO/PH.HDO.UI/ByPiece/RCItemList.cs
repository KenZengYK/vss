...
{
    public partial class RCItemList : ListForm
    {
        public List<ByPiece_RCItem> ImportList = new List<ByPiece_RCItem>();
        public RCItemList()
        {
            InitializeComponent();
        }
        ...
        public override void ProcessFunctionRight()
        {
            ...
            PH.Platform.AuthMgr.BO.Auth_FunRight obj = PH.Platform.AuthMgr.BO.Environment.Instance.CurrentUser.GetFunRight(this.MenuID, "AddRCNews");
        ...
        public override void DataBind()
        {
            ...
            this.DataSource = (from m in context.ByPiece_RCItems where m.Flag == this.Flag  select m).Take(0);

            PH.Platform.UI.CS.DataQuery.DataQueryParameter param = new PH.Platform.UI.CS.DataQuery.DataQueryParameter();
            param.BoType = typeof(ByPiece_RCItem);
        ...
        protected override void OnClickDelete()
        {
            ...
            Dictionary<int, ByPiece_RCItem> _dict = this.GetCheckData<ByPiece_RCItem>();
            foreach (var item in _dict)
            {
                _ctx.ByPiece_RCItems.DeleteOnSubmit(item.Value);
        ...
        {
            FrmDateRange frm = new FrmDateRange(4, this, this.Flag);
            frm.StartPosition = FormStartPosition.CenterParent;
            DialogResult dlg = frm.ShowDialog();
            if (dlg == DialogResult.OK)
            {
                (this.DataContext as PHGDB2DataContext).ByPiece_RCItems.AttachAll(ImportList);
        ...
        {
            HandMovement_Rc frm = new HandMovement_Rc(this, this.Flag);
            frm.StartPosition = FormStartPosition.CenterParent;
            DialogResult dlg = frm.ShowDialog();
            if (dlg == DialogResult.OK)
            {
