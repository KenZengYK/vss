  ...
  try
    ...
    for i:=0 to dbg.Columns.Count-1 do begin
      ...
      if pos('|',s)=0 then begin
        sht.cells(5,i+1):=s;
        if maxLevel>2 then
          sht.range[format('%s5:%s%d',[getXlsCol(i+1),getXlsCol(i+1),maxLevel+2])].merge;
        continue;
      end;
      //四层
      getBeginEndCol(dbg,i,3,j,k);
      if i=j then begin
        sht.cells(5,i+1):=copy(s,1,pos('|',s)-1);
        if j<>k then
          sht.range[format('%s5:%s5', [getXlsCol(j+1),getXlsCol(k+1)])].merge;
      end;
      sht.cells(6,i+1):=copy(s,pos('|',s)+1,maxInt);
    end;
    //导出数据
