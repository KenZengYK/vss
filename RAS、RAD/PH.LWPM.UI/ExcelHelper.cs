...
namespace PH.LWPM.UI.LB4
{
    public class ExcelHelper
    {
        ...
        private IList<Matching> MatchList;
        #endregion

        #region 構造函數
        public ExcelHelper(string AExcelFilePath,StyleMaster AMaster)
        {
            ...
            try
            {
                ...
                if (this._PrintMainTable)
                {
                    ...
                    //this.MergeCell("H" + row, "J" + (row + 1), "", 6);  //優化的顏色
                      this.MergeCell("H" + row, "J" + (row + 1), "", 39); //極佳的顏色 Color.FromArgb(148, 0, 211);
                  //  this.MergeCell_RGB("H" + row, "J" + (row + 1), "", Color.Violet); //FromArgb(148, 0, 211)); //極佳的顏色用Color方式賦值
        ...
        void ExportLB4Info()
        {
            try
            {
                ...
                _WorkBook = _Excel.ActiveWorkbook;
                _WorkSheet = (PHExcel.Worksheet)workbook.ActiveSheet;
                this.SetAdjTablePrintFormat();

                this._WorkRange = _WorkSheet.get_Range("A1", "B1");
                this._WorkRange.Select();
                this._WorkRange.Merge(Missing.Value);
                this._WorkRange.FormulaR1C1 = "Cust Style: " + this._Master.KHKH;

                this._WorkRange = _WorkSheet.get_Range("D1", "E1");
                this._WorkRange.Select();
                this._WorkRange.Merge(Missing.Value);
                this._WorkRange.FormulaR1C1 = "PH Style: " + this._Master.PHKH;

                this._WorkRange = _WorkSheet.get_Range("G1", "H1");
                this._WorkRange.Select();
                this._WorkRange.Merge(Missing.Value);
                this._WorkRange.FormulaR1C1 = "Class: " + this._Master.ZH;

                this._WorkRange = _WorkSheet.get_Range("J1", "K1");
                this._WorkRange.Select();
                this._WorkRange.Merge(Missing.Value);
                this._WorkRange.FormulaR1C1 = "Edition: " + this._Master.Edition;

                this._WorkRange = _WorkSheet.get_Range("M1", "N1");
                this._WorkRange.Select();
                this._WorkRange.Merge(Missing.Value);
                this._WorkRange.FormulaR1C1 = "Line: " + this._Master.Edition;

                _WorkSheet.Cells[2,1] = "FAE";
                _WorkSheet.Cells[2,2] = "Code";
                _WorkSheet.Cells[2,3] = "Operation Name";
                _WorkSheet.Cells[2,4] = "GSD SAH";
                _WorkSheet.Cells[2,5] = "WF No.";
                _WorkSheet.Cells[2,6] = "WF Name";
                _WorkSheet.Cells[2,7] = "Machine Id";
                _WorkSheet.Cells[2,8] = "Machine Type";
                _WorkSheet.Cells[2,9] = "Move Type";
                for (int i = 0; i < MatchList.Count;i++ )
                {
                    Matching mct =MatchList[i];
                    _WorkSheet.Cells[3 + i, 1] = mct.FAE;
                    _WorkSheet.Cells[3 + i, 2] = mct.OperationCode;
                    _WorkSheet.Cells[3 + i, 3] = mct.OperationName;
                    _WorkSheet.Cells[3 + i, 4] = mct.GSD_SAH;
                    _WorkSheet.Cells[3 + i, 5] = mct.WorkerId;
                    _WorkSheet.Cells[3 + i, 6] = mct.WorkerName;
                    _WorkSheet.Cells[3 + i, 7] = mct.MachineId;
                    _WorkSheet.Cells[3 + i, 8] = mct.MCType;
                    _WorkSheet.Cells[3 + i, 9] = mct.Move;
