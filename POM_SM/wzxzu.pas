  ...
  begin
    ...
    begin
      ...
      if xh1.Checked=false then begin
        ...
        if not query1.fieldbyname('operation').isnull then
        frmsiztbl.table3operation.value:=query1.fieldbyname('operation').value;
        if not query1.fieldbyname('garment').isnull then
        frmsiztbl.table3garment.value:=query1.fieldbyname('garment').value;
        if not query1.fieldbyname('newdt').isnull then
        frmsiztbl.table3crdt.value:=query1.fieldbyname('newdt').value;
      end;
...
begin
  ...
  begin
    ...
    query1.commandtext:='select cde,cmeas,emeas,uom,methd,tolerence,parts,operation,garment,newdt from SZ_bastbl where ((statu is null) or (statu='''')) order by cmeas';
//    query1.prepare;
  ...
  begin
    ...
    query1.params.clear;
    //query1.params.createparam(ftstring,'cde',ptinput);
    query1.commandtext:='select cde,cmeas,emeas,uom,methd,tolerence,parts,operation,garment,newdt from SZ_bastbl where cde='''+frmsampletbl.table3cde.value+'''';
//    query1.prepare;
