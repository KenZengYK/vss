...
{
    public partial class ESSEListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public ESSEListForm()
        {
            InitializeComponent();
            this.EditorTypeName = typeof(ESSEDetailForm).FullName;
        ...
        public override void DataBind()
        {
            ...
            {
                this.BindingSource.DataSource = helper.GetESSEList();
            }
            else
            {
        ...
        private void ESSEListForm_Load(object sender, EventArgs e)
        {
            this.DataContext = DBController.SPCDB;
            if (_sampleOrder != null)
                this.BindingSource.DataSource = from c in DBController.SPCDB.ESSEs where c.SampleOrderID == this._sampleOrder.SampleOrderID select c;
            else
                this.BindingSource.DataSource = from c in DBController.SPCDB.ESSEs select c;
            Tools.DateTimeFormatterControls(this, "yyyy'/'MM'/'dd");
        ...
        {
            ESSE spcESSE = this.objListGridView.GetRow(this.objListGridView.FocusedRowHandle) as ESSE;
            if (spcESSE != null)
            {
                XtraTabControl tabControl;
                Control c;
                OpenSampleOrder(spcESSE, out tabControl, out c);

                OpenStyle(tabControl, spcESSE);
            }
        }
        ...
        private void OpenStyleSord2(string styleSortID)
        {
            ...
            {
                bool exists = false;
                foreach (StyleSort styleSort in spcESSE.Style.StyleSorts)
                {
        ...
        private void OpenStyle(XtraTabControl tabControl, ESSE spcESSE)
        {
            Control c = null;
            ExistsAControl(tabControl.SelectedTabPage, typeof(SampleOrderDetailForm), ref c);
            SampleOrderDetailForm frmSampleEdit = c as SampleOrderDetailForm;
        ...
        private void OpenSampleOrder(ESSE spcESSE, out XtraTabControl tabControl, out Control c)
        {
            SampleOrder sampleOrder = spcESSE.Style.SampleOrder;
        ...
        public ESSEListForm(SampleOrder sampleOrder)
        {
            InitializeComponent();
            ...
            foreach (Style style in _sampleOrder.Styles)
                _spcList.Add(style.ESSE);
            // _spcList.AddRange(style.ESSE);
        ...
        List<ESSE> _spcList = new List<ESSE>();
        ...
        {
            List<ESSE> list = new List<ESSE>();
            for (int i = 0; i < this.advBandedGridView1.DataRowCount; i++)
            {
                ESSE esse = this.advBandedGridView1.GetRow(i) as ESSE;
                if (esse != null)
                    list.Add(esse);
            }
            SPCPrintHelper.PrintESSE(list);
        }
