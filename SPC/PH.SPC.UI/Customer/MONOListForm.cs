...
{
    public partial class MONOListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public MONOListForm()
        {
            InitializeComponent();
            this.EditorTypeName = typeof(MONODetailForm).FullName;
        ...
        private void MONOListForm_Load(object sender, EventArgs e)
        {
            //this.DataContext = DBController.SPCDB;
            //if (_sampleOrder != null)
            //    this.BindingSource.DataSource = from c in DBController.SPCDB.MONOs where c.SampleOrderID == this._sampleOrder.SampleOrderID select c;
            //else
            //    this.BindingSource.DataSource = from c in DBController.SPCDB.MONOs select c;
            Tools.DateTimeFormatterControls(this, "yyyy'/'MM'/'dd");
        ...
        {
            MONO spcMONO = this.objListGridView.GetRow(this.objListGridView.FocusedRowHandle) as MONO;
            if (spcMONO != null)
            {
                XtraTabControl tabControl;
                Control c;
                OpenSampleOrder(spcMONO, out tabControl, out c);

                OpenStyle(tabControl, spcMONO);
            }
        }
        ...
        private void OpenStyleSord2(string styleSortID)
        {
            ...
            {
                bool exists = false;
                foreach (StyleSort styleSort in spcMONO.Style.StyleSorts)
                {
        ...
        private void OpenStyle(XtraTabControl tabControl, MONO spcMONO)
        {
            Control c = null;
            ExistsAControl(tabControl.SelectedTabPage, typeof(SampleOrderDetailForm), ref c);
            SampleOrderDetailForm frmSampleEdit = c as SampleOrderDetailForm;
        ...
        private void OpenSampleOrder(MONO spcMONO, out XtraTabControl tabControl, out Control c)
        {
            SampleOrder sampleOrder = spcMONO.Style.SampleOrder;
        ...
        public MONOListForm(SampleOrder sampleOrder)
        {
            InitializeComponent();
            ...
            foreach (Style style in _sampleOrder.Styles)
                _spcList.Add(style.MONO);
            // _spcList.AddRange(style.MONO);
        ...
        List<MONO> _spcList = new List<MONO>();
        ...
        {
            List<MONO> list = new List<MONO>();
            for (int i = 0; i < this.advBandedGridView1.DataRowCount; i++)
            {
                MONO mono = this.advBandedGridView1.GetRow(i) as MONO;
                if (mono != null)
                    list.Add(mono);
            }
            SPCPrintHelper.PrintMONO(list);
        }
