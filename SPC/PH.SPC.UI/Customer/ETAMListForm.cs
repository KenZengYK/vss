...
{
    public partial class ETAMListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public ETAMListForm()
        {
            InitializeComponent();
        ...
        public override void DataBind()
        {
            ...
            {
                this.BindingSource.DataSource = helper.GetETAMList();
            }
            else
            {
                this.BindingSource.DataSource = helper.GetETAMList(_sampleOrder.SampleOrderID);
            }
            base.DataBind();
        }
        ...
        private void ETAMListForm_Load(object sender, EventArgs e)
        {
            ...
            //    this.BindingSource.DataSource = from c in context.ETAMs where c.SampleOrderID == this._sampleOrder.SampleOrderID select c;
            //else
            //    this.BindingSource.DataSource = from c in context.ETAMs select c;
        ...
        public ETAMListForm(SampleOrder sampleOrder)
        {
            InitializeComponent();
        ...
        List<ETAM> _spcList = new List<ETAM>();
        ...
        {
            List<ETAM> list = new List<ETAM>();
            for (int i = 0; i < this.advBandedGridView1.DataRowCount; i++)
            {
                ETAM lard = this.advBandedGridView1.GetRow(i) as ETAM;
                if (lard != null)
                    list.Add(lard);
            }
            SPCPrintHelper.PrintETAM(list);
        }
