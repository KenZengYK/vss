...
{
    public partial class AdditionalInformationInterimForm : Form
    {
        List<CustQCMaster_Interim> DataList;
        public AdditionalInformationInterimForm(List<CustQCMaster_Interim> ADataList)
        {
            InitializeComponent();
            DataList = ADataList;

            PrepareDataAdditional();
            PrepareDataCatLog();
        }

        void PrepareDataAdditional()
        {
        ...
        void PrepareDataCatLog()
        {
            string SqlStr = "select code, act_item, act_item_e from PHDB..tbl_carte_actitem where code in ('C08a','C08b','C08c', 'C09a', 'C09b', 'C09c', 'C09d', 'C09e') order by code";
            MobileQCDataContext db = ContextBuilder.CreateContext<MobileQCDataContext>();
            this.gcCatLog.DataSource = db.ExecuteDataTable(SqlStr, "dtCatLog");
        }
