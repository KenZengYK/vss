...
{
    public partial class RptAuditAQL_Sub21 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public RptAuditAQL_Sub21()
        {
            InitializeComponent();
        }

        DataTable DbTimes_Cust, DbTimes_Ws;
        public RptAuditAQL_Sub21(DataTable dbheader, DataTable reportdata, Dictionary<string, string[]> dict_wssum, Dictionary<string, string[]> dict_custsum,
             Dictionary<string, string[]> _defect_dict, string daterange, int prodtotal, int chktotal, string rtype, string wknum, DataTable DbTimes_Cust, DataTable DbTimes_Ws)//int wknum)
        {
            InitializeComponent();
        ...
        public void Close_AQL_Sub21(int i)
        {
            if (i == 1)
            {
                ReportHeader.Visible = false;
        ...
        private void Detail_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (_typeid == "100")
            {
                ...
                string defectCount = DbTimes_Ws.AsEnumerable().Where(da => da["Workshop"].ToString() == _wscu).Select(da => da["DefectCodeCount"]).FirstOrDefault().ToString();
                xrTableCell41.Text = defectCount;
            ...
            else if (_typeid == "111" || _typeid == "222")
            {
                ...
                string defectCount = DbTimes_Cust.AsEnumerable().Select(da => da["DefectCodeType"]).FirstOrDefault().ToString();
                xrTableCell41.Text = defectCount;
            ...
            else if (_typeid == "200")
            {
                ...
                string defectCount = DbTimes_Cust.AsEnumerable().Where(da => da["customer"].ToString() == _wscu).Select(da => da["DefectCodeCount"]).FirstOrDefault().ToString();
                xrTableCell41.Text = defectCount;
            }
            ...
            //if (_F01 = dr["F01"] != DBNull.Value)
            //{
            //         _F01 = int.Parse(dr["F01"].ToString();
            //}
            //else 
            //{
            //     _F01 = 0;
            ...
            this.xrTableCell87.Text = _mfail_ttl.ToString();
            this.xrTableCell61.Text = (_fail_ttl - _mfail_ttl).ToString();


            //计算 -- 细
            int _mttl_Minor = (_F01 - _MF01) + (_F02 - _MF02) + (_F03 - _MF03) + (_F04 - _MF04) + (_F05 - _MF05) +
                                    (_F06 - _MF06) + (_F07 - _MF07) + (_F08 - _MF08) + (_F09 - _MF09) + (_other_ttl - _mother_ttl);

            //int _mttl = _MF01 + _MF02 + _MF03 + _MF04 + _MF05 + _MF06 + _MF07 + _MF08 + _MF09 + _MF10 + _MF11 + _othermain;
            xrTableCell31_Xi.Text = _mttl_Minor.ToString();

            double ChkTotal_Xi=Convert.ToDouble( string.IsNullOrEmpty(this.GetCurrentColumnValue("ChkTotal").ToString())==true?0.00:Convert.ToDouble(this.GetCurrentColumnValue("ChkTotal").ToString()) );
            double aa_Minor = ChkTotal_Xi == 0 ? 0 : Convert.ToDouble(_mttl_Minor * 100.00 / ChkTotal_Xi);
            this.xrTableCell42_Xi.Text = String.Format("{0:F}", aa_Minor);


            ////计算Group时 ，疵点种类
            //int defectTypeSum = (this.xrTableCell35.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell28.Text.Replace("/", "") == "" ? 0 : 1) +
            //                                 (this.xrTableCell36.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell22.Text.Replace("/", "") == "" ? 0 : 1) +
            //                                 (this.xrTableCell37.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell29.Text.Replace("/", "") == "" ? 0 : 1) +
            //                                 (this.xrTableCell38.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell25.Text.Replace("/", "") == "" ? 0 : 1) +
            //                                 (this.xrTableCell39.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell23.Text.Replace("/", "") == "" ? 0 : 1);

            //xrTableCell41.Text = defectTypeSum.ToString();
        ...
        private void ReportFooter_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            foreach (var item in Dict_SumWs)
            {
                ...
                {
                    xrWs2.Text = _ws;
                    xrWAcc2.Text = _acc.ToString();
                    //xrWRej2.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 /_ttl);
                    //xrPer12.Text = String.Format("{0:F}", aa) + "%";
                    xrWH21.Text = "Ws"; xrWH22.Text = "FPY";
                    //xrWH23.Text = "REJ";
                }
                else if (i == 3)
                {
                    xrWs3.Text = _ws;
                    xrWAcc3.Text = _acc.ToString();
                    //xrWRej3.Text = _rej.ToString();
                    //double aa = _ttl== 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer13.Text = String.Format("{0:F}", aa) + "%";
                    xrWH31.Text = "Ws"; xrWH32.Text = "FPY";
                    //xrWH33.Text = "REJ";
                }
                else if (i == 4)
                {
                    xrWs4.Text = _ws;
                    xrWAcc4.Text = _acc.ToString();
                    //xrWRej4.Text = _rej.ToString();
                    //double aa = _ttl== 0 ? 0 : Convert.ToDouble(_rej * 100.00 /_ttl);
                    //xrPer14.Text = String.Format("{0:F}", aa) + "%";
                    xrWH41.Text = "Ws"; xrWH42.Text = "FPY";
                    //xrWH43.Text = "REJ";
                }
                else if (i == 5)
                {
                    xrWs5.Text = _ws;
                    xrWAcc5.Text = _acc.ToString();
                    //xrWRej5.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer15.Text = String.Format("{0:F}", aa) + "%";
                    xrWH51.Text = "Ws"; xrWH52.Text = "FPY";
                    //xrWH53.Text = "REJ";
                }
                else if (i == 6)
                {
                    xrWs6.Text = _ws;
                    xrWAcc6.Text = _acc.ToString();
                    //xrWRej6.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer16.Text = String.Format("{0:F}", aa) + "%";
                    xrWH61.Text = "Ws"; xrWH62.Text = "FPY";
                    //xrWH63.Text = "REJ";
                }
                else if (i == 7)
                {
                    xrWs7.Text = _ws;
                    xrWAcc7.Text = _acc.ToString();
                    //xrWRej7.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer17.Text = String.Format("{0:F}", aa) + "%";
                    xrWH71.Text = "Ws"; xrWH72.Text = "FPY";
                    //xrWH73.Text = "REJ";
                }
                else if (i == 8)
                {
                    xrWs8.Text = _ws;
                    xrWAcc8.Text = _acc.ToString();
                    //xrWRej8.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer18.Text = String.Format("{0:F}", aa) + "%";
                    xrWH81.Text = "Ws"; xrWH82.Text = "FPY";
                    //xrWH83.Text = "REJ";
                }
                i++;
            ...
            foreach (var item in Dict_SumCust)
            {
                ...
                {
                    xrCust1.Text = _cust;
                    xrCAcc1.Text = _acc.ToString();
                    //xrCRej1.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer21.Text = String.Format("{0:F}", aa) + "%";
                    xrCH11.Text = "Cust"; xrCH12.Text = "FPY";
                    //xrCH13.Text = "REJ";
                ...
                {
                    xrCust2.Text = _cust;
                    xrCAcc2.Text = _acc.ToString();
                    //xrCRej2.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer22.Text = String.Format("{0:F}", aa) + "%";
                    xrCH21.Text = "Cust"; xrCH22.Text = "FPY";
                    //xrCH23.Text = "REJ";
                }
                else if (i == 3)
                {
                    xrCust3.Text = _cust;
                    xrCAcc3.Text = _acc.ToString();
                    //xrCRej3.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer23.Text = String.Format("{0:F}", aa) + "%";
                    xrCH31.Text = "Cust"; xrCH32.Text = "FPY";
                    //xrCH33.Text = "REJ";
                }
                else if (i == 4)
                {
                    xrCust4.Text = _cust;
                    xrCAcc4.Text = _acc.ToString();
                    //xrCRej4.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer24.Text = String.Format("{0:F}", aa) + "%";
                    xrCH41.Text = "Cust"; xrCH42.Text = "FPY";
                    //xrCH43.Text = "REJ";
                }
                else if (i == 5)
                {
                    xrCust5.Text = _cust;
                    xrCAcc5.Text = _acc.ToString();
                    //xrCRej5.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer25.Text = String.Format("{0:F}", aa) + "%";
                    xrCH51.Text = "Cust"; xrCH52.Text = "FPY";
                    //xrCH53.Text = "REJ";
                }
                else if (i == 6)
                {
                    xrCust6.Text = _cust;
                    xrCAcc6.Text = _acc.ToString();
                    //xrCRej6.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer26.Text = String.Format("{0:F}", aa) + "%";
                    xrCH61.Text = "Cust"; xrCH62.Text = "FPY";
                    //xrCH63.Text = "REJ";
                }
                else if (i == 7)
                {
                    xrCust7.Text = _cust;
                    xrCAcc7.Text = _acc.ToString();
                    //xrCRej7.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer27.Text = String.Format("{0:F}", aa) + "%";
                    xrCH71.Text = "Cust"; xrCH72.Text = "FPY";
                    //xrCH73.Text = "REJ";
                }
                else if (i == 8)
                {
                    xrCust8.Text = _cust;
                    xrCAcc8.Text = _acc.ToString(); 
                    ////xrCRej8.Text = _rej.ToString();
                    ////double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    ////xrPer28.Text = String.Format("{0:F}", aa) + "%";
                    xrCH81.Text = "Cust"; xrCH82.Text = "FPY";
                    //xrCH83.Text = "REJ";
                }
                ...
                {
                    xrCust9.Text = _cust;
                    xrCAcc9.Text = _acc.ToString(); 
                    //xrCRej9.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer31.Text = String.Format("{0:F}", aa) + "%";
                    xrCH91.Text = "Cust"; xrCH92.Text = "FPY"; 
                    //xrCH93.Text = "REJ";
                }
                else if (i == 10)
                {
                    xrCustA.Text = _cust;
                    xrCAcc10.Text = _acc.ToString(); 
                    //xrCRej10.Text = _rej.ToString();
                    //double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    //xrPer31.Text = String.Format("{0:F}", aa) + "%";
                    xrCHA1.Text = "Cust"; xrCHA2.Text = "FPY";
                    //xrCHA3.Text = "REJ";
                }
                i++;
