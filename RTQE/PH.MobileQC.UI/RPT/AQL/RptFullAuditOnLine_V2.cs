...
{
    public partial class RptFullAuditOnLine_V2 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        RptFullAuditOnLine_Sub2 SubRpt;

        int ProdTotal = 0, ChkTotal = 0;
        string OnlyCust = "";


        public RptFullAuditOnLine_V2()
        {
            InitializeComponent();
        ...
        public RptFullAuditOnLine_V2(string fac, int sumtype, string startdate, string enddate, string flag, string rtype, string custlist, string wolist, string wslist, string linelist, string wknum)//int wknum)
        {
            InitializeComponent();
            ...
            string _s1 = "", _s2 = "";
            Rtype = rtype;
            bool _onlyone = !custlist.Contains(",")&&custlist!="";
            if (_onlyone)
                OnlyCust = custlist;
            ...
            else if (rtype == "2")
            {
                ...
                _s2 ="W"+WeekNum.ToString()+"周離線全審定品質表現報告摘要成衣主要次要暇疵和表現級數";
            }
            else 
            {
                _s1 = "Monthly";
                if (_onlyone)
                    _s1 = custlist + "- Monthly";
        ...
        public void PrepareData()
        {
           // DataSet ds = SqlDataHelper.GetFullAuditQuality(Factory, SumType, StartDate, EndDate, Flag);
            DataSet ds = SqlDataHelper.GetFullAuditQuality_New(Factory, SumType, StartDate, EndDate, Flag,CustomerList,WoList,WsList,LineList);
          ...
          //  if (ds.Tables.Count>2)
            ...
            if (DbDetail.Columns.Count > 14)
            {
                for (int j = 14; j < DbDetail.Columns.Count; j++)
                {
            ...
            for (int i = 0; i < DbDetail.Rows.Count; i++)
            {
              ...
              //  quaper.MPer = dr["MPer"].ToString();
               ...
               // quaper.DefectTotal = dr["Defecttotal"] != DBNull.Value ? int.Parse(dr["Defecttotal"].ToString()) : 0;
                quaper.OtherTotal = dr["ZZZZ"] != DBNull.Value ? int.Parse(dr["ZZZZ"].ToString()) : 0;
            ...
            int _tt = lstquality.Where(p => p.Workshop == "3A").Sum(P => P.ChkTotal);

            //獲取子報表數據源
            DataTable _dbsub = GetSubRptData(lstquality);
            
            SubRpt = new RptFullAuditOnLine_Sub2(_dbsub, DbHeader, DateRange, Factory, Dict_Defect,ProdTotal,ChkTotal,Rtype,WeekNum,OnlyCust);
            SubRpt.GetReportHeader_Sub2.Visible = false;
            SubRpt.GetPageHeader_Sub2.Visible = false;
        }
        ...
        private DataTable GetSubRptData(List<FullAuditOnLineResult_V1> masterlist)
        {
            ...
            for (int m = 0; m < WorkShopList.Count;m++)
            {
               ...
               string _mper = String.Format("{0:F}", _defecttotal * 100.00 / _prodtotal);
               ...
               newrow["ZL"] = _typettl;
               newrow["ZJ"] = _F01 + _F02 + _F03 + _F04 + _F05 + _F06 + _F07 + _F08 + _F09 + _F10 + _F11 + _other;
                newrow["ChkTotal"] = _checktotal; newrow["ProductTotal"] = _prodtotal;
               newrow["MPer"] = _mper; newrow["Other"] = _other;
            ...
            if (dtsub.Rows.Count > 0)
            {
               ...
               //string _mper = String.Format("{0:F}", _defecttotal * 100.00 / _checktotal);--change by  shulin20190111
               string _mper = String.Format("{0:F}", _defecttotal * 100.00 / _prodtotal);
               ...
               newrow1["Other"] = _other; newrow1["ZL"] = _typettl; 
                //newrow1["ZJ"] = _defecttotal;
               newrow1["ZJ"] = _F01 + _F02 + _F03 + _F04 + _F05 + _F06 + _F07 + _F08 + _F09 + _F10 + _F11 + _other;  
            ...
            for (int m = 0; m < CustList.Count; m++)
            {
                ...
                newrow["Other"]=_other;newrow["ZL"] = _typettl; 
               // newrow["ZJ"] = _defecttotal;
                newrow["ZJ"] = _F01 + _F02 + _F03 + _F04 + _F05 + _F06 + _F07 + _F08 + _F09 + _F10 + _F11 + _other;
                newrow["ChkTotal"] = _checktotal; newrow["ProductTotal"] = _prodtotal;
                newrow["MPer"] = _mper;
        ...
        private void ReportHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            { 
                ...
                xrChkTotal.Text = this.ChkTotal.ToString();
                xrQty.Text = ProdTotal.ToString();
