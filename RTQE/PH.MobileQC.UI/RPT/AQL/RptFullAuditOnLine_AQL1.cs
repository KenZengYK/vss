...
{
    public partial class RptFullAuditOnLine_AQL1 : DevExpress.XtraReports.UI.XtraReport
    {
        string Factory = "", StartDate = "", EndDate = "", CurWorkshop = "", Flag = "",Rtype = "";
        
        int SumType = 0, DefectTotal = 0, ProductTotal = 0;
        DataTable DbHeader, DbDetail, DbDefect, DbSub;
        Dictionary<string, string[]> Dict_Defect = new Dictionary<string, string[]>();
        ArrayList WorkShopList = new ArrayList();
        ArrayList CustList = new ArrayList();
        RptAuditAQL_Sub1 _subRpt;


        public RptFullAuditOnLine_AQL1()
        {
            InitializeComponent();
        }

        public RptFullAuditOnLine_AQL1(string fac, int sumtype, string startdate, string enddate, string flag,string daterange,string rtype)
        {
            InitializeComponent();
            Factory = fac;
            StartDate = startdate;
            ...
            {
                _s1 = "每周離線AQL審定品質表現報告摘要 -  成衣(主要/次要)暇疵和審定結果";
        ...
        public void PrepareData()
        {
            ...
            _subRpt = new RptAuditAQL_Sub1(DbHeader, _dbsub, Dict_WorkshopSum, Dict_CustomerSum, Dict_Defect, xrllDate.Text.Trim(),ProdTotal,ChkTotal,Rtype);
        ...
        private DataTable GetSubRptData(List<AuditOffLine_AQL> masterlist)
        {
            ...
            for (int m = 0; m < WorkShopList.Count;m++)
            {
             ...
             //  int _other = (from aa in masterlist where aa.Workshop == _wk select aa.OtherTotal).Sum();
               ...
               newrow["Other"] = _defecttotal - _F01 - _F02 - _F03 - _F04 - _F05 - _F06 - _F07 - _F08 - _F09 - _F10 - _F11;
            ...
            if (dtsub.Rows.Count > 0)
            {
              ...
              // int _other = (from aa in masterlist select aa.OtherTotal).Sum();
               ...
               newrow["F06"] = _F06; newrow["F07"] = _F07; newrow["F08"] = _F08; newrow["F09"] = _F09; newrow["F10"] = _F10; newrow["F11"] = _F11;
               newrow["ZL"] = _typettl; newrow["ZJ"] = _defecttotal; newrow["ChkTotal"] = _checktotal; newrow["ProductTotal"] = _prodtotal;
               newrow["FailQty"] = _failtotal; newrow["MPer"] = _mper;
               ...
               //newrow["Other"] = _other; 
                ...
                newrow1["MPer"] = _mper;
                newrow1["Other"] = _defecttotal - _F01 - _F02 - _F03 - _F04 - _F05 - _F06 - _F07 - _F08 - _F09 - _F10 - _F11;
            ...
            for (int m = 0; m < CustList.Count; m++)
            {
               ...
               // int _other = (from aa in masterlist where aa.Customer == _cust select aa.OtherTotal).Sum();
                ...
                string _cper = String.Format("{0:F}", _defecttotal * 100.00 / _checktotal);
        ...
        private DataTable CreateSchema()
        {
          ...
          //  coltype.DefaultValue = 0;
            ...
            DataColumn[] _columns = new DataColumn[] { colType, colWKCU, colF01, colF02, colF03, colF04, colF05, colF06, colF07, colF08, colF09, colF10, colF11, colOther, colZL, colZJ, colQty1, colQty2, colFailQty, colPer};
