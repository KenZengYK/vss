...
namespace PH.MobileQC.UI.AQL
{
    public partial class RptFullAuditOnLine_AQL21 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        Dictionary<string, string[]> Dict_WorkshopSum = new Dictionary<string, string[]>();
        Dictionary<string, string[]> Dict_CustomerSum = new Dictionary<string, string[]>();
        ...
        public void PrepareData()
        {
            ...
            _subRpt = new RptAuditAQL_Sub21(DbHeader, DbSub, Dict_WorkshopSum, Dict_CustomerSum, Dict_Defect, xrllDate.Text.Trim(), ProdTotal, ChkTotal, Rtype, WeekNum, DbTimes_Cust, DbTimes_Ws);
        ...
        private void GetCheckTimes_ByCust(DataTable dbcust)
        {
            for (int i = 0; i < dbcust.Rows.Count; i++)
            {
                ...
                string _accsum = dr["FPY"] != DBNull.Value ? dr["FPY"].ToString() + "%" : "0%";
                ...
                Dict_CustomerSum.Add(_cust, new string[] { _accsum });
            }
        ...
        private void GetCheckTimes_ByWs(DataTable dbws)
        {
            for (int i = 0; i < dbws.Rows.Count; i++)
            {
                ...
                string  _accsum = dr["FPY"] != DBNull.Value ? dr["FPY"].ToString() +"%": "0%";
                //int _rejsum = dr["RejTimes"] != DBNull.Value ? int.Parse(dr["RejTimes"].ToString()) : 0;
                Dict_WorkshopSum.Add(_ws, new string[] { _accsum });
            }
        ...
        private DataTable GetSubRptData(List<AuditOffLine_AQL> masterlist)
        {
            ...
            //   Dict_WorkshopSum.Add(_wk, new int[] { _accsum, _rejsum });
               ...
               newrow["MF06"] = _MF06; newrow["MF07"] = _MF07; newrow["MF08"] = _MF08; newrow["MF09"] = _MF09; 
                //newrow["MF10"] = _MF10; 
                //newrow["MF11"] = _MF11;
               
               newrow["F01"] = _F01; newrow["F02"] = _F02; newrow["F03"] = _F03; newrow["F04"] = _F04; newrow["F05"] = _F05;
               newrow["F06"] = _F06; newrow["F07"] = _F07; newrow["F08"] = _F08; newrow["F09"] = _F09;
                //newrow["F10"] = _F10; 
               //newrow["F11"] = _F11;                 
               ...
               int _zj = _MF01 + _MF02 + _MF03 + _MF04 + _MF05 + _MF06 + _MF07 + _MF08 + _MF09 +   _othermainttl;
              // int _zj = _MF01 + _MF02 + _MF03 + _MF04 + _MF05 + _MF06 + _MF07 + _MF08 + _MF09 + _MF10 + _othermainttl;
            ...
            if (dtsub.Rows.Count > 0)
            {
              ...
              // int _other = (from aa in masterlist select aa.OtherTotal).Sum();
               ...
               newrow1["F06"] = _F06; newrow1["F07"] = _F07; newrow1["F08"] = _F08; newrow1["F09"] = _F09; 
                //newrow1["F10"] = _F10; 
                //newrow1["F11"] = _F11;

               newrow1["MF01"] = _MF01; newrow1["MF02"] = _MF02; newrow1["MF03"] = _MF03; newrow1["MF04"] = _MF04; newrow1["MF05"] = _MF05;
               newrow1["MF06"] = _MF06; newrow1["MF07"] = _MF07; newrow1["MF08"] = _MF08; newrow1["MF09"] = _MF09; 
                //newrow1["MF10"] = _MF10; 
                //newrow1["MF11"] = _MF11;
        ...
        private DataTable CreateSchema()
        {
          ...
          //  coltype.DefaultValue = 0;
            ...
            DataColumn[] _columns = new DataColumn[] { colType, colWKCU, colWorkShop,colF01, colF02, colF03, colF04, colF05, colF06, colF07, colF08, colF09,
                //colF10, 
                //colF11, 
                colMF01, colMF02, colMF03, colMF04, colMF05, colMF06, colMF07, colMF08, colMF09,
                //colMF10, 
                //colMF11, 
            ...
            { 
             ...
             //   xrChkTotal.Text = dr["checktotal"] != DBNull.Value ? dr["checktotal"].ToString() : "";
               // xrQty.Text = dr["producttotal"] != DBNull.Value ? dr["producttotal"].ToString() : "";
                ...
                xrFPYPer.Text = Math.Round((Convert.ToDouble(dr["Acc1"].ToString()) + Convert.ToDouble(dr["MAcc1"].ToString())) * 100 / (Convert.ToDouble(dr["Acc1"].ToString()) + Convert.ToDouble(dr["MAcc1"].ToString()) + Convert.ToDouble(dr["Rej1"].ToString())), 2).ToString() + "%";
...
//            string defectCount = DbTimes_Cust.AsEnumerable().Where(dr => dr["customer"].ToString() == CurWorkshop).Select(dr => dr["DefectCodeCount"]).FirstOrDefault().ToString();
            string defectCount = DbTimes_Ws.AsEnumerable().Where(dr => dr["Workshop"].ToString() == CurWorkshop).Select(dr => dr["DefectCodeCount"]).FirstOrDefault().ToString();
         
           
            xrTableCell69.Text = defectCount;
            ...
            {  
               ...
               // xrTableCell72.Text = GetDispStr(_failqty, _mfailqty);
                ...
                //计算 -- 大
                int _mttl = _MF01 + _MF02 + _MF03 + _MF04 + _MF05 + _MF06 + _MF07 + _MF08 + _MF09 +   _othermain;
                ...
                int _mttl_Minor = (_F01 - _MF01) + (_F02 - _MF02) + (_F03 - _MF03) + (_F04 - _MF04) + (_F05 - _MF05) +
                                        (_F06 - _MF06) + (_F07 - _MF07) + (_F08 - _MF08) + (_F09 - _MF09) + (_other - _othermain);

                //int _mttl = _MF01 + _MF02 + _MF03 + _MF04 + _MF05 + _MF06 + _MF07 + _MF08 + _MF09 + _MF10 + _MF11 + _othermain;
                xrTableCell70_Xi.Text = _mttl_Minor.ToString();
                double aa_Minor = CheckTotal == 0 ? 0 : Convert.ToDouble(_mttl_Minor * 100.00 / CheckTotal);
                this.xrPer_Xi.Text = String.Format("{0:F}", aa_Minor);

                ////计算Group时 ，疵点种类
                //int defectTypeSum = (this.xrTableCell57.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell58.Text.Replace("/", "") == "" ? 0 : 1) +
                //                                 (this.xrTableCell59.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell60.Text.Replace("/", "") == "" ? 0 : 1) +
                //                                 (this.xrTableCell61.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell62.Text.Replace("/", "") == "" ? 0 : 1) +
                //                                 (this.xrTableCell63.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell64.Text.Replace("/", "") == "" ? 0 : 1) +
                //                                 (this.xrTableCell65.Text.Replace("/", "") == "" ? 0 : 1) + (this.xrTableCell68.Text.Replace("/", "") == "" ? 0 : 1);

                //xrTableCell69.Text = defectTypeSum.ToString();
        ...
        private void Detail_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            int _mttl = _current.MF01 + _current.MF02 + _current.MF03 + _current.MF04 + _current.MF05 + _current.MF06 + _current.MF07 + _current.MF08 + _current.MF09 ;
            if (_s1 == "2.1")
            {
                int m = 0;
            }
            ...
            //Total总计(细)
            int TotalQtyMinor = (_current.F01 - _current.MF01) + (_current.F02 - _current.MF02) +
                                            (_current.F03 - _current.MF03) + (_current.F04 - _current.MF04) +
                                            (_current.F05 - _current.MF05) + (_current.F06 - _current.MF06) +
                                            (_current.F07 - _current.MF07) + (_current.F08 - _current.MF08) +
                                            (_current.F09 - _current.MF09) + (_current.OtherTotal - _current.OtherMainTotal);

            double PercentMinor = (_current.ChkTotal == 0 || TotalQtyMinor == 0) ? 0 : (Convert.ToDouble(TotalQtyMinor * 100.00) / Convert.ToDouble(_current.ChkTotal));

            xrTableCell31_xi.Text = TotalQtyMinor.ToString();

            xrTableCell42_Xi.Text =Math.Round( Convert.ToDecimal(PercentMinor),2).ToString();
          
        }


        //private int GetMinor(int ttl, int Major) 
        //{
