...
namespace PH.MobileQC.UI.AQL
{
    public partial class RptAuditAQL_Sub21 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public RptAuditAQL_Sub21(DataTable dbheader,DataTable reportdata,Dictionary<string,int[]> dict_wssum,Dictionary<string,int[]> dict_custsum,
             Dictionary<string, string[]> _defect_dict, string daterange, int prodtotal, int chktotal, string rtype, string  wknum)//int wknum)
        {
            InitializeComponent();
        ...
        private void ReportFooter_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            foreach (var item in Dict_SumCust)
            {
                ...
                {
                    xrCust9.Text = _cust;
                    xrCAcc9.Text = _acc.ToString(); xrCRej9.Text = _rej.ToString();
                    double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    xrPer31.Text = String.Format("{0:F}", aa) + "%";
                    xrCH91.Text = "Cust"; xrCH92.Text = "ACC"; xrCH93.Text = "REJ";
                }
                else if (i == 10)
                {
                    xrCustA.Text = _cust;
                    xrCAcc10.Text = _acc.ToString(); xrCRej10.Text = _rej.ToString();
                    double aa = _ttl == 0 ? 0 : Convert.ToDouble(_rej * 100.00 / _ttl);
                    xrPer31.Text = String.Format("{0:F}", aa) + "%";
                    xrCHA1.Text = "Cust"; xrCHA2.Text = "ACC"; xrCHA3.Text = "REJ";
                }
                i++;
