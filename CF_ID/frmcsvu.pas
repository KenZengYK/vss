...
begin
  ...
  sOdbcDir := 'C:\Temp\Csv';
  sFileName := sOdbcDir + '\cutplan'+dt0+'.csv';
  ...
  with Query1 do begin
    ...
    //DatabaseName := 'csvdata';
    SQL.Clear;
    SQL.Add('select * from cutplan'+dt0+'.csv where name='''+sMarkerNo+'''');
    Open;
  end;
...
procedure TfrmCsv.FormCreate(Sender: TObject);
begin
  dt0:=formatdatetime('ddhhnnss',now);
end;
...
begin
  ...
  arRst[0]:=Query1.fieldbyname('Width').value;
  arRst[1]:=Query1.fieldbyname('Length').value;
  arRst[2]:=Query1.fieldbyname('Util').value;
  arRst[3]:=Query1.fieldbyname('P_Per').value;
