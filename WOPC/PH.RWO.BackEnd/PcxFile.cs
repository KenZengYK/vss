...
// Copyright (C) 2006 Joshua Bell (inexorabletash@hotmail.com)
...
namespace PH.RWO.BackEnd
{
    public class PcxFile
    {
        ...
        //Image image = new PcxFile().Load(@"D:\Codes\Net\pcx\MARBLES.PCX");
        //Console.WriteLine("Saving...");
        //image.Save(@"D:\Codes\Net\pcx\marbles.png", ImageFormat.Png);
        ...
        private class PcxHeader
        {
            ...
            //    Utility.WriteUInt16( output, xMin ); 
            //    Utility.WriteUInt16( output, yMin ); 
            //    Utility.WriteUInt16( output, xMax ); 
            //    Utility.WriteUInt16( output, yMax ); 
            //    Utility.WriteUInt16( output, hDpi ); 
            //    Utility.WriteUInt16( output, vDpi ); 
            ...
            //    Utility.WriteUInt16( output, bytesPerLine );
            //    Utility.WriteUInt16( output, (ushort) PcxPaletteType.Indexed );
            ...
            {
                int shortRead = ReadUInt16a(input);
                if (shortRead == -1)
                {
                    throw new EndOfStreamException();
                }
            ...
            private int ReadUInt16a(Stream stream)
            {
        ...
        private class PcxPalette
        {
            ...
            public PcxPalette(ColorPalette palette)
            {
                ...
                {
                    //m_palette[ i ] = Color.FromArgb( entries[i] );
        ...
        public Image Load(Stream input)
        {
            //
            ...
            try
            {
                BitmapData bmpData = image.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format24bppRgb);
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        rgbValues[byteNum] = color.R; byteNum += 1;
                        rgbValues[byteNum] = color.G; byteNum += 1;
                        rgbValues[byteNum] = color.B; byteNum += 1;
                    }
                }
