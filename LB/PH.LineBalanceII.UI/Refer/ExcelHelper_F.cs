...
{
    public class ExcelHelper_F
    {
        #region  Field Define
        private string _ExcelFilePath;
        private StyleMasterDetailForm_F _StyleMasterDetailForm;
        ...
        #region 構造函數
        public ExcelHelper_F(string AExcelFilePath, StyleMasterDetailForm_F AStyleMasterDetailForm, GridView AGridView, StyleMaster AMaster, bool APrintMainTable, bool APrintAdjTable,string ALangid)
        {
        ...
        public void ProcessExcelFormat()
        {
            try
            {
                ...
                //_WorkSheet.PageSetup.set_PrintQuality
                ...
                if (this._PrintMainTable)
                {
                    ...
                    while (i < this._GridView.RowCount + 6)  //此處要遊歷兩次，是要處理先拆又夾回來的情況
                    {
                        ...
                        if (CurJFlag != "-19" && CurJFlag != "")
                        {
                            ...
                            if (BeginOpRowIndex != "-19" && BeginOpRowIndex != "" && EndOpRowIndex != "-19" && EndOpRowIndex != "")
                            {
                                ...
                                //this.MergeCell("V" + BeginOpRowIndex, "V" + EndOpRowIndex);      //優化指標線         (合並單元格)
                    ...
                    this.MergeCell("H" + row, "J" + row, this._StyleMasterDetailForm.lblBest.Text);      //優化
                    ...
                    //this.DrawCellWithBorder("AA" + row, "AC" + row, this._StyleMasterDetailForm.ChildStyleProcedureListForm.gridBand13.Caption, true);  //優化LB(二度優化)
                    this.DrawCellWithBorder("Y" + row, "Z" + row, this._StyleMasterDetailForm.ChildStyleProcedureListForm.gridBand14.Caption, true); //優化LB(間時拆)

                    row++;
                    this.MergeCell("A" + row, "D" + (row + 1), "", 38); //拆的顏色
                    this.MergeCell("F" + row, "F" + (row + 1), "", 34); //夾的顏色
                    //this.MergeCell("H" + row, "J" + (row + 1), "", 6);  //優化的顏色
                      this.MergeCell("H" + row, "J" + (row + 1), "", 39); //優化的顏色 Color.FromArgb(148, 0, 211);
                  //  this.MergeCell_RGB("H" + row, "J" + (row + 1), "", Color.Violet); //FromArgb(148, 0, 211)); //優化的顏色用Color方式賦值
        ...
        private void MergeCell_RGB(object ACell1, object ACell2, string ACaption, Color AColor)
        {
            PHExcel.Range range = _WorkSheet.get_Range(ACell1, ACell2);
            range.Select();
            range.Merge(Missing.Value);
            range.FormulaR1C1 = ACaption;
           // range.Font.Color = System.Drawing.ColorTranslator.ToOle(AColor); //AColor;
            range.Interior.Color = System.Drawing.ColorTranslator.ToOle(AColor);
        }
        ...
        private void PrintAdjTable()
        {
            ...
            this.DrawCellWithBorder("S2", "S2", "狀態 ", true);
            string _statu ="";
            switch (this._Master.Statu.GetValueOrDefault())
            {
                case 0: 
                    _statu = "DVP";
                    break;
            ...
            this.DrawCellWithBorder("T2", "T2", _statu, true);
            ...
            this.DrawCellWithBorder("R" + row, "T" + row, this._StyleMasterDetailForm.gridBand5.Caption, true);     //優化LB
            ...
            this.DrawCellWithBorder("R" + row, "T" + row, this._StyleMasterDetailForm.gridBand26.Caption, true);  //優化LB 數量(衣車+衣工台)
            ...
            this.DrawCellWithBorder("R" + row, "R" + row, this._StyleMasterDetailForm.bandedGridColumn13.Caption, true); //優化LB(主區)
            //this.DrawCellWithBorder("S" + row, "S" + row, this._StyleMasterDetailForm.bandedGridColumn14.Caption, true); //優化LB(支援區)
            this.DrawCellWithBorder("S" + row, "S" + row, this._StyleMasterDetailForm.bandedGridColumn15.Caption, true); //優化LB(尖工區)
            this.DrawCellWithBorder("T" + row, "T" + row, this._StyleMasterDetailForm.bandedGridColumn16.Caption, true); //優化LB(總數)
          ...
          //  row++;
           // this.DrawCellWithBorder("A" + row, "D" + row, this._StyleMasterDetailForm.lblBGroup.Text, true); //"B 組 SAH 小計"
            ...
            /////
            //this.DrawCellWithBorder("A" + row, "D" + row, "", true);
            //this.DrawCellWithBorder("E" + row, "E" + row, "", true);
            //this.DrawCellWithBorder("F" + row, "F" + row, "", true);
            //this.DrawCellWithBorder("G" + row, "G" + row, "", true);
           ...
           // row++;
            ...
            //row++;
            //33 this.DrawCellWithBorder("A" + row, "D" + row, this._StyleMasterDetailForm.lblLine.Text, true); // "優化指標線/優化指標線"
           ...
           // row++;
            ...
            this.DrawCellWithBorder("A" + (row + 1), "C" + (row + 2), this._StyleMasterDetailForm.lblPrepareMan.Text, true); //製錶人
           // this.DrawCellWithBorder("D" + (row + 1), "G" + (row + 2), "", true);
            this.DrawCellWithBorder("D" + (row + 1), "G" + (row + 2), this._Master.CreateBy, true);
            this.DrawCellWithBorder("H" + (row + 1), "K" + (row + 2), this._StyleMasterDetailForm.lblIEConfirm.Text, true); //IE部確認
          //  this.DrawCellWithBorder("L" + (row + 1), "O" + (row + 2), "", true);
            this.DrawCellWithBorder("L" + (row + 1), "O" + (row + 2), this._Master.Auditor, true);

           
            this.DrawCellWithBorder("P" + (row + 1), "R" + (row + 2), this._StyleMasterDetailForm.lblWorkShopConfirm.Text, true); //生産部確認
            //this.DrawCellWithBorder("S" + (row + 1), "T" + (row + 2), "", true);
            this.DrawCellWithBorder("S" + (row + 1), "T" + (row + 2), _Master.ConfirmByProd,true);
           ...
           // this.DrawCellWithBorder("D" + (row + 3), "G" + (row + 4), "", true);
            this.DrawCellWithBorder("D" + (row + 3), "G" + (row + 4), _Master.CreateTime.HasValue ? _Master.CreateTime.GetValueOrDefault().ToShortDateString() : "", true);
           ...
           // this.DrawCellWithBorder("L" + (row + 3), "O" + (row + 4), "", true);
            this.DrawCellWithBorder("L" + (row + 3), "O" + (row + 4), _Master.ConfirmDate_IE.HasValue ? _Master.ConfirmDate_IE.GetValueOrDefault().ToShortDateString() : "", true);
            ...
            this.DrawCellWithBorder("S" + (row + 3), "T" + (row + 4), _Master.ConfirmDate_Prod.HasValue ? _Master.ConfirmDate_Prod.GetValueOrDefault().ToShortDateString() : "", true);
            ...
            if (this._Master.SummaryList.Count > 0)
            {
                foreach (Summary obj in this._Master.SummaryList)
                {
                    ...
                    this.DrawCellWithBorder("R" + row, "R" + row, obj.BestLBMainAreaYCCount.ToString(), false);     //優化LB(主區)
                    //this.DrawCellWithBorder("S" + row, "S" + row, obj.BestLBSupportAreaYCCount.ToString(), false);  //優化LB(支援區)
                    this.DrawCellWithBorder("S" + row, "S" + row, obj.BestLBFloaterAreaYCCount.ToString(), false);  //優化LB(尖工區)
                    this.DrawCellWithBorder("T" + row, "T" + row, obj.BestLBYCCount.ToString(), true);              //優化LB(總數)
        ...
        private void AdjustMainTableFormat()
        {
            ...
            if (this._PrintMainTable)
            {
                ...
                for (int i = 6; i <= this._GridView.RowCount + 5; i++)
                {
                    ...
                    //{
                    //    _WorkSheet.get_Range("V" + i.ToString(), "V" + i.ToString()).FormulaR1C1 = ""; //優化指標記等於0的不顯示
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["F", Missing.Value]).ColumnWidth = 40; //工序名稱

            ((PHExcel.Range)this._WorkSheet.Columns["O", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["O", Missing.Value]).ColumnWidth) + 2; //拆SAH
            ((PHExcel.Range)this._WorkSheet.Columns["P", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["P", Missing.Value]).ColumnWidth) + 2;//夾SAH
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["D", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["D", Missing.Value]).ColumnWidth) - 6; //重要工序
           ...
           // ((PHExcel.Range)this._WorkSheet.Columns["E", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["E", Missing.Value]).ColumnWidth) + 1;
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["E", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["E", Missing.Value]).ColumnWidth) - 4; //工序代號
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["R", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["R", Missing.Value]).ColumnWidth) - 3;
            ((PHExcel.Range)this._WorkSheet.Columns["S", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["S", Missing.Value]).ColumnWidth) - 3;
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["T", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["T", Missing.Value]).ColumnWidth) - 15; //支援節件數
          ...
          //  ((PHExcel.Range)this._WorkSheet.Columns["U", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["U", Missing.Value]).ColumnWidth) + 2;
            ((PHExcel.Range)this._WorkSheet.Columns["U", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["U", Missing.Value]).ColumnWidth) - 3; //平準化MTM
          ...
          //  ((PHExcel.Range)this._WorkSheet.Columns["V", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["V", Missing.Value]).ColumnWidth) + 3;
            ...
            ((PHExcel.Range)this._WorkSheet.Columns["Y", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["Y", Missing.Value]).ColumnWidth) - 7;
            ((PHExcel.Range)this._WorkSheet.Columns["Z", Missing.Value]).ColumnWidth = Convert.ToDouble(((PHExcel.Range)this._WorkSheet.Columns["Z", Missing.Value]).ColumnWidth) - 9;
        ...
        private void SetMainTablePrintFormat()
        {
            ...
            this._WorkSheet.PageSetup.CenterHeader = @"&""Arial,加粗""&14IE &""宋体,加粗""生產工序清單&""Arial,加粗"" - &""宋体,加粗""三步之優化拉平衡&""Arial,加粗"" / IEBOO (Bill of Operations) - 3rd Step for Optimum Line Balance";
        ...
        private void AdjustAdjTableFormat()
        {
            ...
            if (this._PrintAdjTable)
            {
                ...
                //16/10/25 update to below (remove B組&C組)
                this._WorkSheet.get_Range("E7", "F8").NumberFormatLocal = "0.0000_ ";    //GSDSAH、A組、B組、C組合計
                this._WorkSheet.get_Range("E9", "I9").NumberFormatLocal = "0.00_ ";     //拉節時產量(件)
                this._WorkSheet.get_Range("E10", "I10").NumberFormatLocal = "G/通用格式"; //勞動力合計
                this._WorkSheet.get_Range("E11", "I11").NumberFormatLocal = "0.00_ ";     //Floater合計
                this._WorkSheet.get_Range("E12", "I12").NumberFormatLocal = "0.00_ ";     //工作位(衣車+手工台)合計
                this._WorkSheet.get_Range("E13", "I15").NumberFormatLocal = "0.0000_ ";   //最大樽頸 、 最小樽頸SAH、 最小SAH
                this._WorkSheet.get_Range("E16", "I16").NumberFormatLocal = "0.00%";      //LB%拉平衡率
                this._WorkSheet.get_Range("E17", "I18").NumberFormatLocal = "0.0000_ ";   //指標線、SAH中位數(參考)
