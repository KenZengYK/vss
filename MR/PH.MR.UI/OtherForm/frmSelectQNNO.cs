...
namespace PH.MR.UI
{
    public partial class frmSelectQNNO : Form
    {
        public frmSelectQNNO(MRHelper mrHelper, DataTable tb)
        {
            InitializeComponent();
            _mrHelper = mrHelper;
            _tb = tb;
        ...
        private void frmSelectRWONO_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                MessageBox.Show("select one QN NO# at least!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
            }
        }
        ...
        private int UpdateSelectRWO2DB()
        {
            ...
            foreach (DataRow row in this._tb.Rows)
            {
                ...
                qn = Convert.IsDBNull(row["QN"]) ? "" : Convert.ToString(row["QN"]);
                if (isCheck)
                {
                    sSQL = string.Format("insert into SelectRWO_RMR (JOBN,WorkOrder,SWONO,QN) values({0},'{1}','{2}','{3}')", this._mrHelper.JOBNNew, wo, rwo, qn);
                    PH.MR.BO.DB.SQL2000DB.Execute(sSQL);
