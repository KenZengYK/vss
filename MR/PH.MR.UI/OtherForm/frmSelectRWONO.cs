...
namespace PH.MR.UI
{
    public partial class frmSelectRWONO : Form
    {
        public frmSelectRWONO(MRHelper mrHelper, DataTable tb)
        {
            InitializeComponent();
            _mrHelper = mrHelper;
            _tb = tb;
        ...
        private void frmSelectRWONO_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                MessageBox.Show("select one RWO NO# at least!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
            }
        }
        ...
        private int UpdateSelectRWO2DB()
        {
            ...
            foreach (DataRow row in this._tb.Rows)
            {
                ...
                if (isCheck)
                {
                    sSQL = string.Format("insert into SelectRWO_RMR (JOBN,WorkOrder,SWONO) values({0},'{1}','{2}')", this._mrHelper.JOBNNew, wo, rwo);
                    PH.MR.BO.DB.SQL2000DB.Execute(sSQL);
