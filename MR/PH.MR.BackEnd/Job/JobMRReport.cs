...
{
    public partial class JobMRReport : UserControl, PH.Platform.BackEnd.BO.IServerReport, PH.Platform.BackEnd.BO.IParameterSetControl
    {
        public JobMRReport()
        {
            InitializeComponent();
            ...
            this.gridWO.DataSource = WoRwoQns;
        }
        ...
        public DevExpress.XtraReports.UI.XtraReport CreateReport(string parameters)
        {
            ...
            switch (this._reportOption)
            {
                case PHPrintReportOption.SumListing:
                    report = new PH.MR.BackEnd.SumListing();
                    break;
                case PHPrintReportOption.SumBreakDownListing:
                    report = new PH.MR.BackEnd.SumBreakDownListing();
                    break;
                case PHPrintReportOption.DetailListing:
                    report = new PH.MR.BackEnd.DetailListingMR();
                    break;
                default:
                    report = new PH.MR.BackEnd.SumListing();
                    break;
        ...
        {
            List<WoRwoQn> woRwoQns = this.gridWO.DataSource as List<WoRwoQn>;
            string wos = "";
            bool selectAll = false;
            //for (int i = 0; i < this.gridView1.RowCount; i++)
            int i = 0;
            foreach (WoRwoQn item in woRwoQns)
            {
                if (item.SELECT)
                {
        ...
        {
            JobMRReport ctl = new JobMRReport();
        ...
        {
            this.btnGetWO.Visible = label4.Visible = this.gridWO.Visible = ReportType == 3;
        }

        private void btnGetWO_Click(object sender, EventArgs e)
        {
            ...
            WoRwoQns = DB.GetWOsFromAS400(this.Company, this.ProjectNo);
            //this.bindSource.DataSource = WoRwoQns;
            this.gridWO.DataSource = null;
            this.gridWO.DataSource = WoRwoQns;
        }
    }
