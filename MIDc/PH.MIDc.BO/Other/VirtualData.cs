...
namespace PH.MIDc.BO
{
    ...
    public class VirtualList : IBindingList, ITypedList
    {
        ...
        {
            get { return this.ColumnCollection.Count; }
        ...
        public void RemoveValuesBySpecifyRowCol(int rowIndex_from, int rowIndex_to, int colIndex_from, int colIndex_to)
        {
            for (int rowIndex = rowIndex_from; rowIndex <= rowIndex_to; rowIndex++)
            {
                ...
                //RaiseListChanged(new ListChangedEventArgs(ListChangedType.ItemDeleted, -1, rowIndex));
            }
        }
        ...
        public virtual void AddColumn(object value)
        {
            throw new NotImplementedException();
            //int cc = ColumnCount;
        ...
        {
            return this.ColumnCollection[columnIndex].Name;
    ...
    public class VirtualPropertyDescriptorSimple : PropertyDescriptor
    {
        ...
        public string StyleHSCode { get; set; }
        ...
        public VirtualPropertyDescriptorSimple(VirtualList list, int index, string propertyName, Type propertyType, bool isReadOnly, bool hideZero, int seq, string styleName, string styleHSCode)
            : this(list, index, propertyName, propertyType, isReadOnly)
        {
            this.HideZero = hideZero;
            this.Seq = seq;
            this.StyleName = styleName;
            this.StyleHSCode = styleHSCode;
    ...
    public class VirtualWeightList : VirtualList
    {
        ...
        public VirtualWeightList(string name)
        {
            _isModify = false;
            this.Name = name;
        }
        ...
        public void AssignTo(VirtualWeightList DesList)
        {
            DesList.ColumnCollection = this.ColumnCollection;
        ...
        //value must be VirtualPropertyDescriptorSimple
        public override void AddColumn(object value)
        {
            if (!(value is VirtualPropertyDescriptorSimple))
                throw new NotImplementedException("value must be VirtualPropertyDescriptorSimple!");

            VirtualPropertyDescriptorSimple sp = value as VirtualPropertyDescriptorSimple;

            this.ColumnCollection.Add(sp);
            // int cc = ColumnCount;
            //ColumnList.Add( value);
            //ColumnCount++;
            //if (cc != ColumnCount)
            //{
            RaiseListChanged(new ListChangedEventArgs(ListChangedType.PropertyDescriptorAdded, ColumnCount, -1));
            //}
        }
        //value must be VirtualPropertyDescriptorSimple
        public override void AddColumn(int newIndex, object value)
        {
            ...
            this.ColumnCollection.Insert(newIndex, sp);
        ...
        public override void RemoveColumn(int oldIndex)
        {
            ...
            //int cc = ColumnCount;
            RemoveValuesByColumn(oldIndex);
            this.ColumnCollection.RemoveAt(oldIndex);
        ...
        public override int GetPropertyDescriptorIndexByFieldName(string fieldName)
        {
            PropertyDescriptor vpd = this.ColumnCollection[fieldName];
            int index = this.ColumnCollection.IndexOf(vpd);

            //int index = -1;
            //for (int i = 0; i < ColumnCollection.Count; i++)
            //{
            //    VirtualPropertyDescriptorSimple vpd = (VirtualPropertyDescriptorSimple)ColumnCollection[i];
            //    if (vpd.Name.ToUpper() == fieldName.ToUpper())
            //    {
            //        index = i;
            //        break;
            //    }
        ...
        public int GetRowIndexByValue(string fieldName, object keyValue)
        {
            for (int i = 0; i < this.RecordCount; i++)
            {
                object obj = this.GetRowValue(i, fieldName);

                if (keyValue.Equals(obj)) return i;
