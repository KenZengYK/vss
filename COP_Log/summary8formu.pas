...
procedure Tfrmsummary8.ppReport1PreviewFormCreate(Sender: TObject);
begin
  ppReport1.PreviewForm.WindowState:=wsMaximized;
  Tppviewer(ppReport1.PreviewForm.Viewer).Zoompercentage:=100;
end;

procedure Tfrmsummary8.ppSummaryBand1BeforePrint(Sender: TObject);
var
...
begin
  ...
  with query2 do begin
    ...
    commandtext:='select distinct a.seq,b.sqty1,b.audit1,b.audit2,b.audit3,sum(a.major) as q1,sum(a.minor) as q2 from tbl_aql_s4 a,tbl_aql_s0 b '
                +'where b.hun_check=0 and b.endline=0 and a.seq=b.seq and b.wk>=:x1 and b.wk<=:x2 and f_year(b.dt)=:x3';
  ...
  with query2 do begin
    ...
    commandtext:='select count(distinct wk) as q1,count(distinct cstyle) as q2,count(distinct pono) as q3,count(distinct aud) as q4,max(col1) as q5,max(size1) as q6 from tbl_aql_s0 '
                +'where hun_check=0 and endline=0 and wk>=:x1 and wk<=:x2 and f_year(dt)=:x3';
    if frmsummary.CheckBox2.Checked then commandtext:=commandtext+' and dailyaql=1'
    else if frmsummary.CheckBox5.Checked then commandtext:=commandtext+' and dailyaql1=1';
  ...
  with query2 do begin
    ...
    params.createparam(ftinteger,'x2',ptinput);
    params.createparam(ftinteger,'x3',ptinput);
    commandtext:='select count(*) as q1 from tbl_aql_s0 a where hun_check=0 and endline=0 and wk>=:x1 and wk<=:x2 and f_year(dt)=:x3 and not exists (select seq from tbl_aql_s4 b where a.seq=b.seq)';
    if frmsummary.CheckBox2.Checked then commandtext:=commandtext+' and dailyaql=1'
    else if frmsummary.CheckBox5.Checked then commandtext:=commandtext+' and dailyaql1=1';
...
procedure Tfrmsummary8.ppGroupFooterBand1BeforePrint(Sender: TObject);
var
  dft,dft1,x1,x2,i:integer;
begin
  dft:=0;dft1:=0;x1:=0;x2:=0;i:=0;
