unit Carte_Reports2Formu;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ppParameter, ppBands, ppClass, ppCtrls, ppReport, ppSubRpt, ppStrtch,
  ppRichTx, ppVar, ppPrnabl, ppCache, ppComm, ppRelatv, ppProd, StdCtrls,
  Buttons, ppViewr;

type
  TfrmCarte_Reports2 = class(TForm)
    CheckBox1: TCheckBox;
    BitBtn1: TBitBtn;
    ppReport2: TppReport;
...
var
  frmCarte_Reports2: TfrmCarte_Reports2;
...
procedure TfrmCarte_Reports2.BitBtn1Click(Sender: TObject);
begin
...
procedure TfrmCarte_Reports2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
  frmCarte_Reports2:=nil;
end;

procedure TfrmCarte_Reports2.ppDetailBand2BeforePrint(Sender: TObject);
begin
  //showmessage(frmCarte_Ordprocessing.adodataset1.fieldbyname('keycode').value);
  ...
  shpm_2001.caption:='- -';  shpm_2001.font.color:=clBlack;
  if not frmCarte_Ordprocessing.adodataset1.fieldbyname('ex_r').isnull then begin
    if not frmCarte_Ordprocessing.adodataset1.fieldbyname('ex_s').isnull then begin
      shpm_2001.caption:=frmCarte_Ordprocessing.adodataset1.fieldbyname('ex_s').value;
      if frmCarte_Ordprocessing.adodataset1.fieldbyname('ex_s').value='Y' then
      shpm_2001.font.color:=clRed;
    end else shpm_2001.caption:='';
  end;
...
procedure TfrmCarte_Reports2.ppGroupFooterBand3BeforePrint(Sender: TObject);
var
  sqlstr1:string;
begin
  //showmessage(
  ...
  with frmCarte_Ordprocessing.adoquery3 do begin
    close;
    sql.text:='select count(*) as x1 from tbl_carte_sopc1 a where ((pvh_pak1='''') or (pvh_pak1 is null))';
    sql.text:=sql.text+sqlstr1;
    open;
    if not fieldbyname('x1').isnull then begin
      pak1_2001.caption:=fieldbyname('x1').asstring;
    end;
  end;
  with frmCarte_Ordprocessing.adoquery3 do begin
    close;
...
procedure TfrmCarte_Reports2.ppReport2PreviewFormCreate(Sender: TObject);
begin
  ppReport2.PreviewForm.WindowState:=wsMaximized;
  TppViewer(ppReport2.PreviewForm.Viewer).ZoomPercentage:=100;
end;

procedure TfrmCarte_Reports2.ppSummaryBand2BeforePrint(Sender: TObject);
var
  sqlstr1:string;
begin
  sqlstr1:='';
  ...
  with frmCarte_Ordprocessing.adoquery3 do begin
    ...
    open;
    if not fieldbyname('x1').isnull then begin
      pak1_2002.caption:=fieldbyname('x1').asstring;
    end;
  end;
  with frmCarte_Ordprocessing.adoquery3 do begin
    close;
