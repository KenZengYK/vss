...
procedure Tfrmsummary6.ppReport1PreviewFormCreate(Sender: TObject);
begin
  ppReport1.PreviewForm.WindowState:=wsMaximized;
  TppViewer(ppReport1.PreviewForm.Viewer).ZoomPercentage:=100;
end;

procedure Tfrmsummary6.ppDetailBand1BeforePrint(Sender: TObject);
begin
  with query2 do begin
    ...
    commandtext:='select sum(a.major) as q1,sum(a.minor) as q2 from tbl_aql_s4 a where seq in (select seq from tbl_aql_s0 b where pline=:x1 and j2_job=:x2 and j2_job||''-''||rwo||''-''||flag60=:x4 and wk>=:x5 and wk<=:x6 and endline=1)';
    ...
    params[3].asinteger:=frmsummary.SpinEdit2.Value;
    params[4].asinteger:=frmsummary.SpinEdit3.Value;
    open;
...
procedure Tfrmsummary6.ppSummaryBand1BeforePrint(Sender: TObject);
var
...
begin
  ...
  with query2 do begin
    ...
    commandtext:='select distinct a.seq,b.sqty1,b.audit1,b.audit2,b.audit3,sum(a.major) as q1,sum(a.minor) as q2 from tbl_aql_s4 a,tbl_aql_s0 b '
                +'where a.seq=b.seq and b.wk>=:x1 and b.wk<=:x2 and f_year(b.dt)=:x3';// and b.hun_check=:x4 and b.endline=:x5';
    ...
    if frmsummary.ComboBox2.text>'' then commandtext:=commandtext+' and b.fty='''+frmsummary.combobox2.text+'''';
    if frmsummary.Edit1.Text>'' then commandtext:=commandtext+' and b.pline='''+frmsummary.Edit1.text+'''';
  ...
  with query2 do begin
    ...
    //params.createparam(ftboolean,'x4',ptinput);
    //params.createparam(ftboolean,'x5',ptinput);
    commandtext:='select count(distinct wk) as q1,count(distinct cstyle) as q2,count(distinct pline) as q3,count(distinct aud) as q4,max(col1) as q5,max(size1) as q6 from tbl_aql_s0 '
                +'where wk>=:x1 and wk<=:x2 and f_year(dt)=:x3';// and hun_check=:x4 and endline=:x5';
    ...
    if frmsummary.combobox2.text>'' then commandtext:=commandtext+' and fty='''+frmsummary.combobox2.text+'''';
    if frmsummary.Edit1.Text>'' then commandtext:=commandtext+' and pline='''+frmsummary.Edit1.text+'''';
    params.ParamByName('x1').Asinteger:=frmsummary.spinedit2.value;
    params.ParamByName('x2').Asinteger:=frmsummary.spinedit3.value;
    params.ParamByName('x3').AsInteger:=frmsummary.spinedit1.Value;
    //params.ParamByName('x4').AsBoolean:=frmsummary.CheckBox3.Checked;
  ...
  with query2 do begin
    ...
    if frmsummary.combobox2.text>'' then commandtext:=commandtext+' and a.fty='''+frmsummary.combobox2.text+'''';
    if frmsummary.Edit1.Text>'' then commandtext:=commandtext+' and a.pline='''+frmsummary.Edit1.text+'''';
    params.ParamByName('x1').Asinteger:=frmsummary.spinedit2.value;
    params.ParamByName('x2').Asinteger:=frmsummary.spinedit3.value;
    params.ParamByName('x3').AsInteger:=frmsummary.spinedit1.Value;
    //params.ParamByName('x4').AsBoolean:=frmsummary.CheckBox3.Checked;
