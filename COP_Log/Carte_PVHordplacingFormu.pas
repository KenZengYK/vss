...
type
  TfrmCarte_PVHordplacing = class(TForm)
    Panel1: TPanel;
    ...
    ADOQuery1: TADOQuery;
    cxGrid1prd_seq: TcxGridDBBandedColumn;
    cxGrid1prd_name: TcxGridDBBandedColumn;
    cxGrid1ml: TcxGridDBBandedColumn;
    cxGrid1op: TcxGridDBBandedColumn;
    cxGrid1al: TcxGridDBBandedColumn;
...
var
  frmCarte_PVHordplacing: TfrmCarte_PVHordplacing;
...
procedure TfrmCarte_PVHordplacing.BitBtn1Click(Sender: TObject);
begin
  cxgrid1.SetFocus;
  adodataset1.append;
end;

procedure TfrmCarte_PVHordplacing.BitBtn2Click(Sender: TObject);
begin
  if application.MessageBox('Delete this record?','Confirmation',mb_iconquestion+mb_okcancel)=idok then adodataset1.Delete;
end;

procedure TfrmCarte_PVHordplacing.BitBtn3Click(Sender: TObject);
begin
  if (adodataset1.State=dsEdit) or (adodataset1.State=dsInsert) then adodataset1.Post;
end;

procedure TfrmCarte_PVHordplacing.BitBtn5Click(Sender: TObject);
begin
...
procedure TfrmCarte_PVHordplacing.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
  frmCarte_PVHordplacing:=nil;
end;
