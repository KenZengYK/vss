...
begin
  ...
  with adoquery1 do begin
    ...
    sql.text:='select * from tbl_carte_sopc1 a where (case when m_ddt0 is not null then m_ddt0 else m_ddt end<=case when sp_appdt_r is not null then sp_appdt_r else sp_appdt_o end) '
             +'and sp_appdt_a is null and (sp_appdt_o is not null or sp_appdt_r is not null) and (m_ddt is not null or m_ddt0 is not null)';
    sql.text:=sql.text+sqlstr1;
    open;
  end;
  finally
