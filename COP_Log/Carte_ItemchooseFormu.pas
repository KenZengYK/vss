  ...
  if not adoquery1.fieldbyname('code').isnull then begin
    ...
    if label1.Caption='Request' then begin
      if (frmCarte_Requestaction.adodataset2.state=dsedit) or (frmCarte_Requestaction.adodataset2.state=dsinsert) then begin
        frmCarte_Requestaction.adodataset2.fieldbyname('act_item3').value:=adoquery1.fieldbyname('act_tgrp_cde').value+adoquery1.fieldbyname('code').value;
        if frmCarte_Requestaction.adodataset2.fieldbyname('act_subject3').isnull then begin
          if fty1='CL' then
          frmCarte_Requestaction.adodataset2.fieldbyname('act_subject3').value:=tgrp_e+adoquery1.fieldbyname('act_item_e').value
          else frmCarte_Requestaction.adodataset2.fieldbyname('act_subject3').value:=tgrp+adoquery1.fieldbyname('act_item').value;
        end else begin
        ...
        with frmCarte_Requestaction.adodataset2 do begin
          edit;
          fieldbyname('act_item3').value:=adoquery1.fieldbyname('act_tgrp_cde').value+adoquery1.fieldbyname('code').value;
          if frmCarte_Requestaction.adodataset2.fieldbyname('act_subject3').isnull then begin
            if fty1='CL' then
            fieldbyname('act_subject3').value:=tgrp_e+adoquery1.fieldbyname('act_item_e').value
            else fieldbyname('act_subject3').value:=tgrp+adoquery1.fieldbyname('act_item').value;
          end else begin
            if frmCarte_Requestaction.adodataset2.fieldbyname('act_subject3').value='' then begin
              if fty1='CL' then
              fieldbyname('act_subject3').value:=tgrp_e+adoquery1.fieldbyname('act_item_e').value
      ...
      frmCarte_Groupaction.amendcatcde(adoquery1.fieldbyname('act_tgrp_cde').value+adoquery1.fieldbyname('code').value);
...
begin
  with adoquery1 do begin
    close;
    sql.text:='select * from tbl_carte_actitem order by act_tgrp_cde,code';
    open;
  end;
end;
