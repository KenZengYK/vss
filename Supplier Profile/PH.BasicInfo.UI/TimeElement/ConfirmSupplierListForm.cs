...
{
    public partial class ConfirmSupplierListForm : PH.Platform.UI.CS.UI2.ListForm
    {
        public ConfirmSupplierListForm()
        {
            InitializeComponent();
        ...
        public override void DataBind()
        {
            ...
            this.BindingSource.DataSource = SPDataList.GetDataByCondition("isNull(PreparePerson,'')<>'' and isNull(ConfirmPerson,'')='' and (DeleteFlag=0 or DeleteFlag is null) and CheckFlag = 'WaitConfirm'").ToList();
            base.DataBind();
            this.EditorTypeName = typeof(SupplierDetailForm).FullName;
        ...
        private void barButtonItem2_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("你確定要[Confirm]選定內容嗎？", "System Note", MessageBoxButtons.OKCancel) != DialogResult.OK)
                {
                    return;
                }
                ...
                {
                    item.ConfirmPerson = PH.Platform.AuthMgr.BO.Environment.Instance.CurrentUser.UserID;
                    item.ConfirmDate = DateTime.Now;
                    item.CheckFlag = "WaitApproval"; //由David加入 2023-01-16
                }
                this.DataContext.SubmitChanges();

                //发邮件给上次Approval过的人， 由David 增加 2023-02-07
                var ApprovalGroup = SuppProfList.GroupBy(p => p.LastApprovalPerson);
                foreach (var o in ApprovalGroup)
                {
                    Notify.SendEmail(SysParamHelper.Instance.UserID, "Confirm", o.ToList(), o.FirstOrDefault().LastApprovalPerson); //发送邮件，由David增加 2023-02-06
                }
                //Notify.SendEmail(SysParamHelper.Instance.UserID, "Confirm", SuppProfList); //发送邮件，由David增加 2023-02-06

                //重新綁定數據
                DataBind();
                MessageBox.Show("Set the choosed rows to confirm status Success!", "System Note", MessageBoxButtons.OK);
            }
            catch (Exception ex)
            {
